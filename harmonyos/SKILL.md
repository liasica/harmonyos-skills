...
5. 如果本地文件比用户项目要求的版本陈旧（对比 `doc_updated_at`），或本地完全没命中而该话题确属鸿蒙领域，按 [`rules/online-fallback.md`](rules/online-fallback.md) 拉取最新内容，并明确说明走了在线兜底
...
本地没命中或本地文档明显陈旧时，按 [`rules/online-fallback.md`](rules/online-fallback.md) 调华为公开文档接口拉最新正文，不要用 `WebFetch` 或浏览器。
