...
content_hash: sha256:f92f01990d439e8b87aee4cd09a6e9a815ed663a01c3126dcc875784272857e9
---
...
本文主要面向中高级开发者。开始之前，建议先了解[一次开发，多端部署概览](bpta-multi-device-overview.md)、[断点](bpta-multi-device-responsive-layout.md#section1532120147301)等知识点。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/3a/v3/P3-Js209TXebCluVtWymwg/zh-cn_image_0000002506596732.gif "点击放大")
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/9f/v3/wcqUFSAxQn6pbdSlE6JGLw/zh-cn_image_0000002506436906.gif "点击放大")
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/d0/v3/yEaJdlSjS-2MWhrGzhkAZw/zh-cn_image_0000002538396643.png "点击放大")

**适配宽高比为9:16的视频**

当视频宽高比为9:16时，其在断点区间的适配效果图如下图所示，红色区域为推荐的视频显示区域。

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/1e/v3/JmB604RSRgeaemG9enpAdA/zh-cn_image_0000002538316627.png "点击放大")

当横向断点为sm、纵向断点为lg时，由于设备尺寸的差异，存在不同的适配建议。具体如下图所示，其中横坐标为窗口宽高比。

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/13/v3/7HOiqtBHQAia48rlwW-QRg/zh-cn_image_0000002506596740.png "点击放大")对于不满足横向断点为sm、纵向断点为lg的其他窗口尺寸，建议顶部状态栏和底部Tab栏均采用沉浸式设计，内容区高度=窗口高度，内容区宽度=内容区高度×视频宽高比。
...
开合连续指应用在各种屏幕和窗口状态间切换时页面内容连续，切换之前的任务和相关状态能保存、延续，或能够快速恢复，给用户提供连续的体验。主要标准有页面不发生改变和焦点不发生偏移，具体可参考[开合接续](../design-guides/foldable-0000002352875141.md#section5560057912)。应用页面和功能相关的开合连续能力建议使用断点实现，并通过[on('windowSizeChange')](../harmonyos-references/arkts-apis-window-window.md#onwindowsizechange7)接口监听。

display提供了折叠状态监听的接口，这些接口建议使用在某些应用功能的适配上，比如适配设备[悬停态](bpta-multi-device-screen-diff.md#section32851531135)、[设置多设备上相机预览画面比例](bpta-multi-device-camera.md#section882216138497)，但不能用于页面布局的开合连续适配。
...
| [on('windowSizeChange')](../harmonyos-references/arkts-apis-window-window.md#onwindowsizechange7) | 开启窗口尺寸变化的监听 |
| [off('windowSizeChange')](../harmonyos-references/arkts-apis-window-window.md#offwindowsizechange7) | 关闭窗口尺寸变化的监听 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ab/v3/P_BOkA0fTyeAwiCN5PsimA/zh-cn_image_0000002585654510.png "点击放大")
...
* WaterFlow组件
  ...
  WaterFlow组件需要根据场景进行特殊的判断，若双折叠状态改变前后，WaterFlow未改变展示列数，则系统默认使用改变前的滑动偏移量，以确保阅读焦点不发生偏移；若双折叠状态改变前后，WaterFlow改变了列数，则需要将WaterFlowLayoutMode改为SLIDING\_WINDOW（参考：[WaterFlowLayoutMode枚举说明](../harmonyos-references/ts-container-waterflow.md#waterflowlayoutmode12枚举说明)）。修改后，系统将根据WaterFlow可展示区域的最小索引值，自动调整以确保阅读焦点不偏移。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/46/v3/1imawvadTByTRtx7K7u9SQ/zh-cn_image_0000002506436912.png "点击放大")
