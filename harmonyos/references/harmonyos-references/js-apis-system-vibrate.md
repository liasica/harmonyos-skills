...
content_hash: sha256:bb24383ffea2ef6428e785131cc298d30db230a67717ec6f7fc102dfd7d904fb
---
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | [VibrateOptions](js-apis-system-vibrate.md#vibrateoptions) | 否 | 振动配置参数，用于指定振动模式及回调函数。不传时使用默认配置（mode默认为'long'），此时仅触发success和complete回调（说明调用后不会触发fail回调）。 |
...
定义触发设备振动的配置参数，包括振动模式及接口调用的回调函数。开发者调用[Vibrator.vibrate()](js-apis-system-vibrate.md#vibratorvibrate)时，通过VibrateOptions指定振动模式（短振动或长振动）以及监听振动触发成功、失败和完成的回调函数。传入VibrateOptions后，设备将按指定的mode执行相应振动模式，并在振动触发成功时回调success函数，失败时回调fail函数，接口调用结束时回调complete函数。
