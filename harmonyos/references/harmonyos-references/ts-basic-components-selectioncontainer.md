---
url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-basic-components-selectioncontainer
title: SelectionContainer
breadcrumb: API参考 > 应用框架 > ArkUI（方舟UI框架） > ArkTS组件 > 文本与输入 > SelectionContainer
category: harmonyos-references
scraped_at: 2026-09-15T07:05:00+08:00
doc_updated_at: 2026-09-09
content_hash: sha256:e73b213642190e4735c1f5e9d38d5fba23da3d89a1f90612e0e5c6561460e4e9
---
...
* SelectionContainer内选中文本时不显示放大镜，也不支持[getMagnifier](arkts-apis-uicontext-uicontext.md#getmagnifier22)主动设置放大镜。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [SelectionContainerOptions](ts-basic-components-selectioncontainer.md#selectioncontaineroptions) | 否 | 组件初始化配置项。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| isEnabled | [Optional](ts-universal-attributes-custom-property.md#optionalt)<boolean> | 是 | 是否开启触控反馈。  true表示开启触控反馈，false表示不开启触控反馈。 |
...
* 该配置会影响[onWillCopy](ts-basic-components-selectioncontainer.md#onwillcopy)、[onCopy](ts-basic-components-selectioncontainer.md#oncopy)、[bindSelectionMenu](ts-basic-components-selectioncontainer.md#bindselectionmenu)相关回调中返回的文本内容。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| style | [Optional](ts-universal-attributes-custom-property.md#optionalt)<[SelectionContainerTextJoinStyle](ts-basic-components-selectioncontainer.md#selectioncontainertextjoinstyle)> | 是 | 聚合文本拼接方式。 |
...
设置自定义选择菜单。未通过该接口设置时，默认spanType为TextSpanType.TEXT，responseType为TextResponseType.LONG\_PRESS。
...
| options | [Optional](ts-universal-attributes-custom-property.md#optionalt)<[SelectionContainerMenuOptions](ts-basic-components-selectioncontainer.md#selectioncontainermenuoptions)> | 否 | 选择菜单选项，用于配置菜单出现、消失、显示、隐藏等事件的回调。当需要监听这些菜单事件时传入此参数，不传入时默认不监听菜单事件。 |
...
* 当同时为当前场景设置了[bindSelectionMenu](ts-basic-components-selectioncontainer.md#bindselectionmenu)和editMenuOptions时，优先使用bindSelectionMenu，editMenuOptions不生效。bindSelectionMenu用于完全自定义菜单风格和触发条件，由开发者定义所有菜单项；editMenuOptions用于在系统默认菜单基础上添加扩展项，触发条件不变。建议根据自定义程度需求选择。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| editMenu | [Optional](ts-universal-attributes-custom-property.md#optionalt)<[SelectionContainerEditMenuOptions](ts-basic-components-selectioncontainer.md#selectioncontainereditmenuoptions)> | 是 | 自定义编辑菜单配置。 |
...
* 回调参数为按Text组件视觉顺序拼接后的选中文本，拼接方式由[textJoinStyle](ts-basic-components-selectioncontainer.md#textjoinstyle)配置决定。
* 返回false时，会阻止本次跨节点复制及容器级[onCopy](ts-basic-components-selectioncontainer.md#oncopy)回调触发，但不会影响各Text子组件已独立处理完成的复制事件逻辑。
...
* 回调参数为按Text组件视觉顺序拼接后的选中文本，拼接方式由[textJoinStyle](ts-basic-components-selectioncontainer.md#textjoinstyle)配置决定。
* 仅当容器级[onWillCopy](ts-basic-components-selectioncontainer.md#onwillcopy)返回true时，该回调才会触发。
...
| onMenuItemClick | [OnMenuItemClickWithTextCallback](ts-basic-components-selectioncontainer.md#onmenuitemclickwithtextcallback) | 否 | 是 | 点击菜单项时触发，可拦截系统默认菜单执行行为。默认值为空，不触发该回调。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| controller | [SelectionContainerController](ts-basic-components-selectioncontainer.md#selectioncontainercontroller) | 否 | 否 | SelectionContainer控制器。 |

## 示例

### 示例1（跨节点选中文本并复制）

该示例通过[SelectionContainer](ts-basic-components-selectioncontainer.md#接口)、[copyOption](ts-basic-components-selectioncontainer.md#copyoption)、[textJoinStyle](ts-basic-components-selectioncontainer.md#textjoinstyle)、[onTextSelectionChange](ts-basic-components-selectioncontainer.md#ontextselectionchange)、[onWillCopy](ts-basic-components-selectioncontainer.md#onwillcopy)、[onCopy](ts-basic-components-selectioncontainer.md#oncopy)接口展示跨多个Text组件选中文本、拼接选中文本并处理复制回调的能力。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/33/v3/NqQyvOZLQ6u7vRpY6BV4ZA/zh-cn_image_0000002723697136.png)

### 示例2（绑定自定义选择菜单）

该示例通过[bindSelectionMenu](ts-basic-components-selectioncontainer.md#bindselectionmenu)接口实现了跨节点选中文本时绑定自定义菜单的功能。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/e1/v3/RI3rIQt-Q2SzTzo9vHCGUQ/zh-cn_image_0000002753296903.png)

### 示例3（扩展菜单选项）

该示例通过[editMenuOptions](ts-basic-components-selectioncontainer.md#editmenuoptions)接口实现了去除系统菜单中的翻译和搜索菜单项，并添加5个自定义菜单项的功能。同时在[onMenuItemClick](ts-basic-components-selectioncontainer.md#onmenuitemclickwithtextcallback)回调中展示拦截系统复制操作（return true）和不拦截全选操作（return false）的差异。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/17/v3/jqCtna2-Rbi8KGAtamVDHQ/zh-cn_image_0000002753456821.png)

### 示例4（通过控制器关闭选择菜单与清除文本选中）

该示例通过[SelectionContainer](ts-basic-components-selectioncontainer.md#接口)传入[SelectionContainerController](ts-basic-components-selectioncontainer.md#selectioncontainercontroller)，调用[closeSelectionMenu](ts-basic-components-selectioncontainer.md#closeselectionmenu)和[clearTextSelection](ts-basic-components-selectioncontainer.md#cleartextselection)接口展示关闭选择菜单和清除选中文本的能力。

从API版本26.0.0开始，新增[SelectionContainerController](ts-basic-components-selectioncontainer.md#selectioncontainercontroller)和[SelectionContainerOptions](ts-basic-components-selectioncontainer.md#selectioncontaineroptions)接口。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/7a/v3/odxED3VnTou8BrLPdJUiLQ/zh-cn_image_0000002723857056.gif)
