...
| 类型 | 说明 |
| --- | --- |
| [VMMemoryInfo](js-apis-hidebug.md#vmmemoryinfo12) | 返回VM内存信息。 |
...
| 类型 | 说明 |
| --- | --- |
| [ThreadCpuUsage](js-apis-hidebug.md#threadcpuusage12)[] | 返回当前应用进程下所有ThreadCpuUsage数组。 |
...
该接口补充了[hitrace](../harmonyos-guides/hitrace.md)功能，开发者可通过该接口完成指定范围的trace自动化采集。由于该接口中trace采集过程中消耗的性能与需要采集的范围成正相关，建议开发者在使用该接口前，通过hitrace命令抓取应用的trace日志，从中筛选出所需trace采集的关键范围，以提高该接口性能。

startAppTraceCapture()方法的调用需要与'[stopAppTraceCapture()](js-apis-hidebug.md#hidebugstopapptracecapture12)'方法的调用一一对应，重复开启trace采集将导致接口调用异常，由于trace采集过程中会消耗较多性能，开发者应在完成采集后及时关闭。
...
| 类型 | 说明 |
| --- | --- |
| string | 返回trace文件名路径（接口返回真实物理路径，若应用内需要访问，请参考[应用沙箱路径和真实物理路径的对应关系](../harmonyos-guides/app-sandbox-directory.md#应用沙箱路径和真实物理路径的对应关系)进行路径转换）。 |
...
停止应用trace采集。调用前，需先调用'[startAppTraceCapture()](js-apis-hidebug.md#hidebugstartapptracecapture12)'方法开始采集。关闭前未开启或重复关闭会导致接口异常。
...
| 类型 | 说明 |
| --- | --- |
| [MemoryLimit](js-apis-hidebug.md#memorylimit12) | 应用程序进程内存限制。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| type | string | 是 | 泄漏资源类型，共四种：  - pss\_memory（native内存）  - js\_heap（js堆内存）  - fd（文件描述符）  - thread（线程） |
| value | number | 是 | 对应泄漏资源类型的最大值，范围：  - pss\_memory类型：[1024, 4 \* 1024 \* 1024]（单位：KB）  - js\_heap类型：[85, 95]（分配给JS堆内存上限的85%~95%）  - fd类型：[10, 10000]  - thread类型：[1, 1000]。超出范围会导致功能失效。 |
...
| 类型 | 说明 |
| --- | --- |
| [NativeMemInfo](js-apis-hidebug.md#nativememinfo12) | 应用进程内存信息。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<[NativeMemInfo](js-apis-hidebug.md#nativememinfo12)> | promise对象，返回应用进程内存信息。 |
...
| 类型 | 说明 |
| --- | --- |
| [SystemMemInfo](js-apis-hidebug.md#systemmeminfo12) | 系统内存信息。 |
...
获取系统[GC](../harmonyos-guides/gc-introduction.md)统计信息。
...
| 类型 | 说明 |
| --- | --- |
| [GcStats](js-apis-hidebug.md#gcstats12) | 系统GC统计信息。 |
...
根据参数获取指定的系统[GC](../harmonyos-guides/gc-introduction.md)统计信息。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| rssLimit | bigint | 否 | 否 | 应用程序进程可用的物理内存限制，以KB为单位，实际当前系统未对进程可用物理内存做限制，但是进程的可用物理内存仍然不会超过设备的实际最大可用物理内存，当前设备的物理内存使用情况可通过[hidebug.getSystemMemInfo](js-apis-hidebug.md#hidebuggetsystemmeminfo12)获取。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<[GraphicsMemorySummary](js-apis-hidebug.md#graphicsmemorysummary21)> | promise对象，返回应用显存数据。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | [GwpAsanOptions](js-apis-hidebug.md#gwpasanoptions20) | 否 | GWP-ASan配置项。未设置时，使用默认参数。 |
...
该接口影响[dumpJsRawHeapData](js-apis-hidebug.md#hidebugdumpjsrawheapdata18)的结果。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| level | [JsRawHeapTrimLevel](js-apis-hidebug.md#jsrawheaptrimlevel20) | 是 | 转储堆快照的裁剪级别，默认为TRIM\_LEVEL\_1。 |
