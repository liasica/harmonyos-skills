---
url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-rendering-control-lazyforeach
title: LazyForEach
...
开发者指南见：[LazyForEach开发者指南](../harmonyos-guides/arkts-rendering-control-lazyforeach.md)。

LazyForEach是一种懒加载渲染控制组件，从提供的数据源中按需迭代数据并创建相应组件。在大量子组件的场景下，LazyForEach与缓存列表项、动态预加载、组件复用等方法配合使用，可以进一步提升滑动帧率并降低应用内存占用。最佳实践请参考[优化长列表加载慢丢帧问题](../best-practices/bpta-best-practices-long-list.md)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| dataSource | [IDataSource](ts-rendering-control-lazyforeach.md#idatasource) | 是 | LazyForEach数据源，需要开发者实现相关接口。 |
...
从API版本26.0.0开始，LazyForEach支持传入[LazyForEachOptions](ts-rendering-control-lazyforeach.md#lazyforeachoptions)，用于使能自定义组件冻结和配置内存优化策略、资源释放策略。
...
| itemGenerator | (item: any, index: number) => void | 是 | 子组件生成函数，为数组中的每一个数据项创建一个子组件。  **说明：**  - item是当前数据项（可选），index是数据项索引值（可选）。  - 建议item的数据类型与数据源的数据类型保持一致，否则，当itemGenerator中存在与数据类型强相关的操作时，会导致子组件无法正常渲染，甚至运行时崩溃。  - itemGenerator的函数体必须使用大括号{...}。  - itemGenerator每次迭代只能并且必须生成一个子组件。  - itemGenerator中可以使用if语句，但是必须保证if语句每个分支都会创建一个相同类型的子组件。 |
| keyGenerator | (item: any, index: number) => string | 否 | 键值生成函数，用于给数据源中的每一个数据项生成唯一且固定的键值。修改数据源中的一个数据项若不影响其生成的键值，则对应组件不会被更新，否则对应组件就会被重建更新。keyGenerator参数是可选的，但是，为了使开发框架能够更好地识别数组更改并正确更新组件，建议提供。  默认使用框架内置的键值生成函数（详见下方说明）。  **说明：**  - item是当前数据项（可选），index是数据项索引值（可选）。  - 建议item的数据类型与数据源的数据类型保持一致，否则，当keyGenerator中存在与数据类型强相关的操作时，会导致子组件无法正常渲染，甚至运行时崩溃。  - keyGenerator缺省时，使用默认的键值生成函数，即(item: Object, index: number) => { return viewId + '-' + index.toString(); }，生成键值仅受索引值index影响（viewId在编译器转换过程中生成，同一个LazyForEach组件内的viewId一致）。  - 为保证LazyForEach正确、高效地更新子组件，避免渲染结果异常、渲染效率降低等问题，键值应满足以下条件。  1. 键值具有唯一性，每个数据项对应的键值互不相同。  2. 键值具有一致性，数据项不变时对应的键值也不变。 |
| options | [LazyForEachOptions](ts-rendering-control-lazyforeach.md#lazyforeachoptions) | 否 | 开发者配置项，用于使能自定义组件冻结和配置内存优化策略、资源释放策略。使用此配置项时，必须设置键值生成函数，否则将编译失败。不传入时使用默认配置（自定义组件冻结模式默认为AUTO，资源释放策略默认为BATCH，内存优化策略默认为DEFAULT）。 |
...
应避免在getData函数中执行耗时操作，以减少应用滑动时的卡顿丢帧，最佳实践请参考[主线程耗时操作优化-循环渲染](../best-practices/bpta-time-optimization-of-the-main-thread.md#section4551193714439)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| listener | [DataChangeListener](ts-rendering-control-lazyforeach.md#datachangelistener) | 是 | 数据变化监听器，用于在数据源发生变化时通知组件刷新。 |
...
从API version 7开始支持，从API version 8开始废弃。建议使用[onDataAdd](ts-rendering-control-lazyforeach.md#ondataadd8)替代。
...
* 从API version 7开始支持，从API version 8开始废弃。建议使用[onDataMove](ts-rendering-control-lazyforeach.md#ondatamove8)替代。
...
* 从API version 7开始支持，从API version 8开始废弃。建议使用[onDataDelete](ts-rendering-control-lazyforeach.md#ondatadelete8)替代。
...
从API version 7开始支持，从API version 8开始废弃。建议使用[onDataChange](ts-rendering-control-lazyforeach.md#ondatachange8)替代。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| dataOperations | [DataOperation](ts-rendering-control-lazyforeach.md#dataoperation12)[] | 是 | 一次批量处理数据的操作集合，开发者将需要处理的数据操作（添加、删除、改变、移动、交换、重载等）放入该数组，组件按照数组中的操作顺序刷新展示内容。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| type | [DataOperationType](ts-rendering-control-lazyforeach.md#dataoperationtype12).ADD | 否 | 否 | 数据添加类型。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| type | [DataOperationType](ts-rendering-control-lazyforeach.md#dataoperationtype12).DELETE | 否 | 否 | 数据删除类型。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| type | [DataOperationType](ts-rendering-control-lazyforeach.md#dataoperationtype12).CHANGE | 否 | 否 | 数据改变类型。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| type | [DataOperationType](ts-rendering-control-lazyforeach.md#dataoperationtype12).MOVE | 否 | 否 | 数据移动类型。 |
| index | [MoveIndex](ts-rendering-control-lazyforeach.md#moveindex12) | 否 | 否 | 移动位置。取值范围是[0, 数据源长度-1]。超出取值范围时渲染异常。 |
| key | string | 否 | 是 | 为被移动的数据分配新的键值，默认使用原键值。 |

### DataExchangeOperation12+
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| type | [DataOperationType](ts-rendering-control-lazyforeach.md#dataoperationtype12).EXCHANGE | 否 | 否 | 数据交换类型。 |
| index | [ExchangeIndex](ts-rendering-control-lazyforeach.md#exchangeindex12) | 否 | 否 | 交换位置。取值范围是[0, 数据源长度-1]。超出取值范围时渲染异常。 |
| key | [ExchangeKey](ts-rendering-control-lazyforeach.md#exchangekey12) | 否 | 是 | 分配新的键值，默认使用原键值。 |

### DataReloadOperation12+
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| type | [DataOperationType](ts-rendering-control-lazyforeach.md#dataoperationtype12).RELOAD | 否 | 否 | 数据全部重载类型。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| customComponentFreezeMode | [LazyForEachCustomComponentFreezeMode](ts-rendering-control-lazyforeach.md#lazyforeachcustomcomponentfreezemode) | 否 | 是 | 选择是否使能自定义组件冻结。仅在LazyForEach下直接使用自定义组件时生效，其他情况不适用。  默认为[AUTO](ts-rendering-control-lazyforeach.md#lazyforeachcustomcomponentfreezemode)。 |
| releaseStrategy | [LazyForEachReleaseStrategy](ts-rendering-control-lazyforeach.md#lazyforeachreleasestrategy) | 否 | 是 | 为LazyForEach配置资源释放策略。  默认使用[BATCH](ts-rendering-control-lazyforeach.md#lazyforeachreleasestrategy)，批量释放节点。 |
| memoryOptimizationStrategy | [LazyForEachMemOptStrategy](ts-rendering-control-lazyforeach.md#lazyforeachmemoptstrategy) | 否 | 是 | LazyForEach的内存优化策略。该参数在创建LazyForEach时设定，不支持动态修改。  默认值：[DEFAULT](ts-rendering-control-lazyforeach.md#lazyforeachmemoptstrategy) |
...
| --- | --- | --- |
| DEFAULT | 0 | 无内存优化策略。 |
| ENABLE\_AUTO\_CACHE\_OPTIMIZATION | 1 << 0 | 自动内存优化策略，当LazyForEach承载的列表项数量较多（例如达到数百项以上）或单项子组件结构复杂（例如包含多层嵌套或数十个子节点），导致内存占用偏高（可通过性能分析工具检测内存占用情况）时，建议使用此策略以降低内存使用量。  当应用退后台时、LazyForEach所在组件不可见时（[visibility](ts-universal-attributes-visibility.md#visibility)属性设置为[Visible](ts-appendix-enums.md#visibility)以外的值，或组件面积为0，不考虑遮挡）、整机低内存时（[MemoryLevel](js-apis-app-ability-abilityconstant.md#memorylevel)达到MEMORY\_LEVEL\_LOW或MEMORY\_LEVEL\_CRITICAL），对于内存大于6G的设备，释放[预加载区域](../harmonyos-guides/arkts-rendering-control-overview.md#基本概念)内的部分节点，直至上下预加载区域内的节点数量均不超过2；对于内存小于等于6G的设备，释放预加载区域内的全部节点。  当应用恢复前台时、LazyForEach所在组件恢复显示时，LazyForEach发生滑动时，恢复预加载区域内的节点。  在释放和恢复节点时，会触发[自定义组件生命周期](../harmonyos-guides/arkts-page-custom-components-lifecycle.md)。 |

## 示例

### 示例1（使用自动内存优化策略）

以下示例中，通过[LazyForEachOptions](ts-rendering-control-lazyforeach.md#lazyforeachoptions)的memoryOptimizationStrategy属性使用了自动内存优化策略。应用退后台时，清理缓存。应用恢复前台时，恢复缓存。

从API版本26.0.0开始，新增LazyForEachOptions接口。

BasicDataSource代码见LazyForEach开发者指南末尾BasicDataSource示例代码：[string类型数组的BasicDataSource代码](../harmonyos-guides/arkts-rendering-control-lazyforeach.md#string类型数组的basicdatasource代码)。
