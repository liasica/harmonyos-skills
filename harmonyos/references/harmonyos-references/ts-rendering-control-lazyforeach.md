...
scraped_at: 2026-04-28T08:02:45+08:00
doc_updated_at: 2026-04-20
content_hash: sha256:c8bbc7181a92b2b1101f03021f279d9d92483c6dffba445ac9a5c964d1824674
---
...
在大量子组件的场景下，LazyForEach与缓存列表项、动态预加载、组件复用等方法配合使用，可以进一步提升滑动帧率并降低应用内存占用。最佳实践请参考[优化长列表加载慢丢帧问题](../best-practices/bpta-best-practices-long-list.md)。
...
应避免在keyGenerator和itemGenerator函数中执行耗时操作，以此来减少应用滑动时卡顿丢帧问题，最佳实践请参考[主线程耗时操作优化-循环渲染](../best-practices/bpta-time-optimization-of-the-main-thread.md#section4551193714439)。例如，不推荐使用JSON.stringify函数。在复杂的业务场景中，使用JSON.stringify会对item对象进行序列化，该过程会消耗大量时间与计算资源，从而降低页面性能，最佳实践请参考[懒加载优化性能-键值生成规则](../best-practices/bpta-lazyforeach-optimization.md#section68711519072)。
...
应避免在getData函数中执行耗时操作，以此来减少应用滑动时卡顿丢帧问题，最佳实践请参考[主线程耗时操作优化-循环渲染](../best-practices/bpta-time-optimization-of-the-main-thread.md#section4551193714439)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| listener | [DataChangeListener](ts-rendering-control-lazyforeach.md#datachangelistener) | 是 | 数据变化监听器。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| dataOperations | [DataOperation](ts-rendering-control-lazyforeach.md#dataoperation12)[] | 是 | 一次处理数据的操作。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| type | [DataOperationType](ts-rendering-control-lazyforeach.md#dataoperationtype枚举说明).ADD | 否 | 否 | 数据添加类型。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| type | [DataOperationType](ts-rendering-control-lazyforeach.md#dataoperationtype枚举说明).DELETE | 否 | 否 | 数据删除类型。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| type | [DataOperationType](ts-rendering-control-lazyforeach.md#dataoperationtype枚举说明).CHANGE | 否 | 否 | 数据改变类型。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| type | [DataOperationType](ts-rendering-control-lazyforeach.md#dataoperationtype枚举说明).MOVE | 否 | 否 | 数据移动类型。 |
| index | [MoveIndex](ts-rendering-control-lazyforeach.md#moveindex12) | 否 | 否 | 移动位置。取值范围是[0, 数据源长度-1]。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| type | [DataOperationType](ts-rendering-control-lazyforeach.md#dataoperationtype枚举说明).EXCHANGE | 否 | 否 | 数据交换类型。 |
| index | [ExchangeIndex](ts-rendering-control-lazyforeach.md#exchangeindex12) | 否 | 否 | 交换位置。取值范围是[0, 数据源长度-1]。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| type | [DataOperationType](ts-rendering-control-lazyforeach.md#dataoperationtype枚举说明).RELOAD | 否 | 否 | 数据全部重载类型。 |
