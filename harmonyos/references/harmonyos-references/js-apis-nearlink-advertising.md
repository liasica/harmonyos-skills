---
url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-nearlink-advertising
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| advertisingSettings | [AdvertisingSettings](js-apis-nearlink-advertising.md#advertisingsettings) | 否 | 否 | 广播配置参数。 |
| advertisingData | [AdvertisingData](js-apis-nearlink-advertising.md#advertisingdata) | 否 | 否 | 广播数据包。 |
...
| power | [TxPowerMode](js-apis-nearlink-advertising.md#txpowermode) | 否 | 是 | 广播发射功率配置参数。如果不配置，则默认值为ADV\_TX\_POWER\_LOW。 |
...
| manufacturerData | [ManufacturerData](js-apis-nearlink-advertising.md#manufacturerdata)[] | 否 | 是 | 厂商数据。若未配置则默认不携带该字段。 |
| serviceData | [ServiceData](js-apis-nearlink-advertising.md#servicedata)[] | 否 | 是 | 服务数据。若未配置则默认不携带该字段。 |
...
| state | [AdvertisingState](js-apis-nearlink-advertising.md#advertisingstate) | 否 | 否 | 表示当前广播状态。 |
...
发送星闪广播。使用Promise异步回调。适用于设备发现、设备信息广播等需要将本端设备能力或数据对外发布的业务场景，配合[advertising.onAdvertisingStateChange](js-apis-nearlink-advertising.md#advertisingonadvertisingstatechange)可监听广播启停状态。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| advertisingParams | [AdvertisingParams](js-apis-nearlink-advertising.md#advertisingparams) | 是 | 广播相关参数。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| Promise<number> | Promise对象，返回本次开启的广播ID。广播ID是随机分配的唯一标识值，范围[0, 255]，与[advertising.stopAdvertising](js-apis-nearlink-advertising.md#advertisingstopadvertising)参数及[AdvertisingStateChangeInfo](js-apis-nearlink-advertising.md#advertisingstatechangeinfo).advertisingId一致，可用于区分本次广播实例。 |
...
订阅星闪广播状态变化事件。使用callback异步回调。当调用[advertising.startAdvertising](js-apis-nearlink-advertising.md#advertisingstartadvertising)启动广播或[advertising.stopAdvertising](js-apis-nearlink-advertising.md#advertisingstopadvertising)停止广播时，回调函数会被触发，返回对应的广播ID与广播状态。需与[advertising.offAdvertisingStateChange](js-apis-nearlink-advertising.md#advertisingoffadvertisingstatechange)配对使用。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback<[AdvertisingStateChangeInfo](js-apis-nearlink-advertising.md#advertisingstatechangeinfo)> | 是 | 回调函数，返回广播状态变化信息。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback<[AdvertisingStateChangeInfo](js-apis-nearlink-advertising.md#advertisingstatechangeinfo)> | 否 | 回调函数，返回广播启停状态变化信息。  填写该参数则取消当前callback订阅。不填写该参数则取消该事件对应的所有回调。 |
