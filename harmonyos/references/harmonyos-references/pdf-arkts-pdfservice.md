...
content_hash: sha256:7e8bd5460ac2433cbeeae9f5c2c6ebca62cb11b0ea19e8fe6551fdab25fae21d
---
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/fb/v3/T2F5Grk4QbKN5FhnnC2X-A/zh-cn_image_0000002583440953.jpg?HW-CC-KV=V1&HW-CC-Date=20260428T001818Z&HW-CC-Expire=86400&HW-CC-Sign=E591991AD7FD715067873A7A2A8EB1F5807E72C9C95C25896AAAEA2E82E55260)
...
| **参数名** | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| bookmark | [Bookmark](pdf-arkts-pdfservice.md#bookmark) | 是 | Bookmark类型。 |
...
11. let rootBookmark: boolean = pdfDocument.removeBookmark(bookmarks[0]);
12. }
13. }
```
...
searchKey(text: string, listener: SearchKeyCallback, options?: SearchOptions): Promise<void&gt
...
3. const keyword: string = 'watermelon';
4. const options: pdfService.SearchOptions = {
5. startIndex: 0,
6. endIndex: 50,
7. isMatchWholeWord: true,
8. isMatchCase: true,
9. contextStringLength: 80
10. }
11. const listener: pdfService.SearchKeyCallback = (results: pdfService.SearchResultData[]): boolean => {
12. for (let i = 0; i < results.length; i++) {
13. let contextString = results[i].contextString;
14. hilog.info(0x0000, 'searchKey',`Get context string: ${contextString}`);
15. }
16. return false;
17. };
18. let doc: pdfService.PdfDocument | undefined = undefined;
19. doc = new pdfService.PdfDocument();
20. let loadResult = doc.loadDocument(filePath);
21. if (loadResult === pdfService.ParseResult.PARSE_SUCCESS) {
22. await doc.searchKey(keyword, listener, options);
23. }
```
...
| highlightMode | [HighlightMode](pdf-arkts-pdfservice.md#highlightmode) | 否 | 是 | 高亮模式类型。  说明：目前只支持表单注释类型，返回值-1。 |
...
| textAlignment | [AlignmentType](pdf-arkts-pdfservice.md#alignmenttype) | 否 | 是 | 对齐类型。 |
...
15. let annotation = pdfPage.addAnnotation(aInfo);pdfPage.removeAnnotation(annotation);
16. }
```
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| type | [GraphicsObjectType](pdf-arkts-pdfservice.md#graphicsobjecttype) | 否 | 否 | 获取当前图形对象的类型,type是枚举值。 |
...
| charRects | Array<[PdfRect](pdf-arkts-pdfservice.md#pdfrect)> | 否 | 否 | 获取字符的区域。 |
...
| 名称 | 值 | 说明 |
| --- | --- | --- |
| HIGHLIGHT\_INVERT | 1 | 附件注释的内容。 |
| HIGHLIGHT\_OUTLINE | 2 | 反转批注边框。 |
| HIGHLIGHT\_PUSH | 3 | 用于显示注释的压制外观。 |
| HIGHLIGHT\_TOGGLE | 4 | 切换，仅对小工具注释有用。 |
...
| BOX\_BLEED | 2 | 定义在生产环境中输出时页面内容应剪裁到的区域。 |
| BOX\_TRIM | 3 | 修剪后完成页面的预期尺寸。 |
| BOX\_ART | 4 | 定义页面创建者想要的页面有意义内容的范围。 |
...
| OBJECT\_TEXT | 1 | Text. |
| OBJECT\_PATH | 2 | Path. |
| OBJECT\_IMAGE | 3 | Image. |
| OBJECT\_SHADING | 4 | Shading. |
| OBJECT\_FORM | 5 | Form. |
...
| 名称 | 值 | 说明 |
| --- | --- | --- |
| PDF\_FONT\_CID\_FONT\_CHARSET | 0x100000 | CID. |
| PDF\_FONT\_ANSI\_CHARSET | 0 | ANSI. |
| PDF\_FONT\_DEFAULT\_CHARSET | 1 | Default. |
| PDF\_FONT\_SYMBOL\_CHARSET | 2 | Symbol. |
| PDF\_FONT\_SHIFT\_JIS\_CHARSET | 128 | Shift JIS. |
| PDF\_FONT\_HANGUL\_CHARSET | 129 | Hangeul. |
| PDF\_FONT\_GB2312\_CHARSET | 134 | GB2312. |
| PDF\_FONT\_CHINESE\_BIG5\_CHARSET | 136 | Chinese BIG5. |
| PDF\_FONT\_THAI\_CHARSET | 222 | Thai. |
| PDF\_FONT\_EAST\_EUROPE\_CHARSET | 238 | East Europe. |
| PDF\_FONT\_RUSSIAN\_CHARSET | 204 | Russian. |
| PDF\_FONT\_GREEK\_CHARSET | 161 | Greek. |
| PDF\_FONT\_TURKISH\_CHARSET | 162 | Turkish. |
| PDF\_FONT\_VIETNAMESE\_CHARSET | 163 | Vietnamese. |
| PDF\_FONT\_HEBREW\_CHARSET | 177 | Hebrew. |
| PDF\_FONT\_ARABIC\_CHARSET | 178 | Arabic. |
| PDF\_FONT\_BALTIC\_CHARSET | 186 | Baltic. |
