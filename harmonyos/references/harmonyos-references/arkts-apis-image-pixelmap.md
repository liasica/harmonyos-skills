...
scraped_at: 2026-09-10T06:28:27+08:00
doc_updated_at: 2026-09-09
content_hash: sha256:c2c065ccbd233a7d69594a48ebb69964b015827f2f053497987b8eb85436440c
---
...
从API version 11开始，PixelMap支持通过[@ohos.worker (启动一个Worker)](js-apis-worker.md)跨线程调用。当PixelMap通过Worker或[@ohos.taskpool (启动任务池)](js-apis-taskpool.md)跨线程传输后，原线程的PixelMap的所有接口均不能继续调用，否则将抛出501或7600106（详情请参考[Image错误码](errorcode-image.md)）。

开发元服务请通过[ImageSource](arkts-apis-image-imagesource.md)构建PixelMap对象。

图像占用的内存往往较大，当PixelMap对象使用完成后，应主动调用[release](arkts-apis-image-pixelmap.md#release7)方法及时释放内存。释放时应确保该对象的所有异步方法均执行完成，且后续不再使用该对象。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| dst | ArrayBuffer | 是 | 目标缓冲区，获取的像素数据会被拷贝至该缓冲区。缓冲区内像素的格式与PixelMap相同，不包含内存对齐填充字节。缓冲区的大小可通过[getPixelBytesNumber](arkts-apis-image-pixelmap.md#getpixelbytesnumber7)接口获取。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| dst | ArrayBuffer | 是 | 目标缓冲区，获取的像素数据会被拷贝至该缓冲区。缓冲区内像素的格式与PixelMap相同，不包含内存对齐填充字节。缓冲区的大小由[getPixelBytesNumber](arkts-apis-image-pixelmap.md#getpixelbytesnumber7)接口获取。 |
...
从API版本26.0.0开始，建议使用[readAllPixelsToBuffer](arkts-apis-image-pixelmap.md#readallpixelstobuffer)代替，以获得更完善的异常处理能力。
...
从API版本26.0.0开始，建议使用[readAllPixelsToBufferSync](arkts-apis-image-pixelmap.md#readallpixelstobuffersync)代替，以获得更完善的异常处理能力。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| area | [PositionArea](arkts-apis-image-i.md#positionarea7) | 是 | 读取的区域数据。  该区域由[PositionArea](arkts-apis-image-i.md#positionarea7).region指定，获取的像素数据会被拷贝至[PositionArea](arkts-apis-image-i.md#positionarea7).pixels缓冲区。如果PixelMap的像素格式为YUV类型，则获取的像素数据格式与PixelMap相同，否则会被转换为BGRA\_8888格式。 |
...
从API版本26.0.0开始，建议使用[readPixelsToArea](arkts-apis-image-pixelmap.md#readpixelstoarea)代替，以获得更完善的异常处理能力。
...
读取PixelMap指定区域内的像素数据并存入缓冲区。如果PixelMap的像素格式为YUV类型，则按照PixelMap的像素格式存入缓冲区，否则按照BGRA\_8888格式存入缓冲区。使用callback异步回调。
...
从API版本26.0.0开始，建议使用[readPixelsToAreaSync](arkts-apis-image-pixelmap.md#readpixelstoareasync)代替，以获得更完善的异常处理能力。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| area | [PositionArea](arkts-apis-image-i.md#positionarea7) | 是 | 写入的区域数据。  该区域由[PositionArea](arkts-apis-image-i.md#positionarea7).region指定，[PositionArea](arkts-apis-image-i.md#positionarea7).pixels缓冲区内的像素数据会被写入PixelMap的该区域。如果PixelMap的像素格式为YUV类型，则缓冲区内的像素数据格式需与PixelMap相同，否则需要为BGRA\_8888格式。 |
...
从API版本26.0.0开始，建议使用[writePixelsFromArea](arkts-apis-image-pixelmap.md#writepixelsfromarea)代替，以获得更完善的异常处理能力。
...
将缓冲区内的像素数据写入PixelMap的指定区域。如果PixelMap的像素格式为YUV类型，则缓冲区内的数据会按照PixelMap的像素格式解析，否则按照BGRA\_8888格式解析。使用callback异步回调。
...
从API版本26.0.0开始，建议使用[writePixelsFromAreaSync](arkts-apis-image-pixelmap.md#writepixelsfromareasync)代替，以获得更完善的异常处理能力。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| src | ArrayBuffer | 是 | 源数据缓冲区，该缓冲区内的图像像素数据会被写入PixelMap。缓冲区内的像素数据必须是整个PixelMap的像素数据，且像素格式需与PixelMap相同，不包含内存对齐填充字节。缓冲区的大小可通过[getPixelBytesNumber](arkts-apis-image-pixelmap.md#getpixelbytesnumber7)接口获取。 |
...
从API版本26.0.0开始，建议使用[writeAllPixelsFromBuffer](arkts-apis-image-pixelmap.md#writeallpixelsfrombuffer)代替，以获得更完善的异常处理能力。
...
从API版本26.0.0开始，建议使用[writeAllPixelsFromBufferSync](arkts-apis-image-pixelmap.md#writeallpixelsfrombuffersync)代替，以获得更完善的异常处理能力。
...
* YUV格式的图像不支持设置不透明度。
* 从API版本26.0.0开始，建议使用[setOpacity](arkts-apis-image-pixelmap.md#setopacity)代替，以获得更完善的异常处理能力。
...
* YUV格式的图像不支持设置不透明度。
* 从API版本26.0.0开始，建议使用[setOpacitySync](arkts-apis-image-pixelmap.md#setopacitysync)代替，以获得更完善的异常处理能力。
...
提取当前PixelMap的Alpha通道数据，并生成一个仅包含Alpha通道信息的ALPHA\_U8格式的PixelMap。生成的新PixelMap不可编辑，可用于阴影效果。使用Promise异步回调。
...
| 类型 | 说明 |
| --- | --- |
| Promise<PixelMap> | Promise对象，返回仅包含Alpha通道信息的ALPHA\_U8格式的PixelMap。 |
...
提取当前PixelMap的Alpha通道数据，并生成一个仅包含Alpha通道信息的ALPHA\_U8格式的PixelMap。生成的新PixelMap不可编辑，可用于阴影效果。同步返回结果。
...
| 类型 | 说明 |
| --- | --- |
| PixelMap | 返回仅包含Alpha通道信息的ALPHA\_U8格式的PixelMap。 |
...
* YUV格式不支持此接口。
* 如果原PixelMap的格式是ALPHA\_F16，则新生成的PixelMap将维持ALPHA\_F16格式。
* 从API版本26.0.0开始，建议使用[extractAlphaPixelMap](arkts-apis-image-pixelmap.md#extractalphapixelmap)代替，以获得更完善的异常处理能力。
...
| 类型 | 说明 |
| --- | --- |
| Promise<[PixelMap](arkts-apis-image-pixelmap.md)> | Promise对象，返回仅包含Alpha通道信息的ALPHA\_8格式的PixelMap。 |
...
根据Alpha通道的信息，来生成一个仅包含Alpha通道信息的ALPHA\_8格式的PixelMap，生成的新PixelMap不可编辑，可用于阴影效果。使用callback异步回调。
...
* YUV格式不支持此接口。
* 如果原PixelMap的格式是ALPHA\_F16，则新生成的PixelMap将维持ALPHA\_F16格式。
* 从API版本26.0.0开始，建议使用[extractAlphaPixelMapSync](arkts-apis-image-pixelmap.md#extractalphapixelmapsync)代替，以获得更完善的异常处理能力。
...
| 类型 | 说明 |
| --- | --- |
| [PixelMap](arkts-apis-image-pixelmap.md) | 成功同步返回仅包含Alpha通道信息的ALPHA\_8格式的PixelMap对象，失败抛出异常。 |
...
* 从API版本26.0.0开始，建议使用[applyScale](arkts-apis-image-pixelmap.md#applyscale)代替，以获得更完善的异常处理能力。
...
* 宽高的缩放倍数 = 缩放后的图片宽高 / 缩放前的图片宽高。
* 从API版本26.0.0开始，建议使用[applyScaleSync](arkts-apis-image-pixelmap.md#applyscalesync)代替，以获得更完善的异常处理能力。
...
* 平移后的图像尺寸将变为：宽度 = 原宽度 + x，高度 = 原高度 + y。
* 从API版本26.0.0开始，建议使用[applyTranslate](arkts-apis-image-pixelmap.md#applytranslate)代替，以获得更完善的异常处理能力。
...
* 平移后的图像尺寸将变为：宽度 = 原宽度 + x，高度 = 原高度 + y。
* 从API版本26.0.0开始，建议使用[applyTranslateSync](arkts-apis-image-pixelmap.md#applytranslatesync)代替，以获得更完善的异常处理能力。
...
* 从API版本26.0.0开始，建议使用[applyRotate](arkts-apis-image-pixelmap.md#applyrotate)代替，以获得更完善的异常处理能力。
...
* 图像旋转的角度取值范围：[0, 360]。超出取值范围时，根据圆周360°自动矫正。例如，-100°与260°效果相同。
* 如果图片旋转的角度不是90的整数倍，旋转后图片的尺寸会发生改变。
* 从API版本26.0.0开始，建议使用[applyRotateSync](arkts-apis-image-pixelmap.md#applyrotatesync)代替，以获得更完善的异常处理能力。
...
从API版本26.0.0开始，建议使用[applyFlip](arkts-apis-image-pixelmap.md#applyflip)代替，以获得更完善的异常处理能力。
...
从API版本26.0.0开始，建议使用[applyFlipSync](arkts-apis-image-pixelmap.md#applyflipsync)代替，以获得更完善的异常处理能力。
...
从API版本26.0.0开始，建议使用[applyCrop](arkts-apis-image-pixelmap.md#applycrop)代替，以获得更完善的异常处理能力。
...
从API版本26.0.0开始，建议使用[applyCropSync](arkts-apis-image-pixelmap.md#applycropsync)代替，以获得更完善的异常处理能力。
...
设置图像的色彩空间。如需同时对图像的像素颜色进行色彩空间转换，请使用[applyColorSpace](arkts-apis-image-pixelmap.md#applycolorspace11)。
...
* 从API版本12开始，如果要将HDR色彩空间转换为SDR色彩空间，请先调用[toSdr](arkts-apis-image-pixelmap.md#tosdr12)完成HDR到SDR的转换和色调映射，再按需调用本接口在SDR色彩空间之间转换。
...
该接口仅支持DMA内存类型的PixelMap调用。详情请参考[图片解码内存优化(ArkTS)](../harmonyos-guides/image-allocator-type.md)。
...
支持NV12/NV21与RGB\_888/RGBA\_8888/RGB\_565/BGRA\_8888/RGBA\_F16互转，YCRCB\_P010/YCBCR\_P010与RGBA\_1010102互转。

从API version 18开始，可用于ASTC\_4x4类型转为RGBA\_8888类型，目前仅支持ASTC\_4x4转为RGBA\_8888。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| targetPixelFormat | [PixelMapFormat](arkts-apis-image-e.md#pixelmapformat7) | 是 | 目标像素格式，用于YUV和RGB类型互转，或者ASTC\_4x4类型转为RGBA\_8888类型。目前仅支持NV12/NV21与RGB\_888/RGBA\_8888/RGB\_565/BGRA\_8888/RGBA\_F16互转，YCRCB\_P010/YCBCR\_P010与RGBA\_1010102互转，ASTC\_4x4转为RGBA\_8888。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| name | string | 是 | 需要设置的内存标识符。DMA内存标识符的字符串长度取值范围为[1, 255]字节，SHARE\_MEMORY内存标识符的字符串长度取值范围为[1, 244]字节。 |
