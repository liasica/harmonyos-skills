...
content_hash: sha256:256bccee02a83ac6d498e11cfbe79726a55d7f883d11690468d8af9b59198154
---
...
| stopPropagation | () => void | 否 | 否 | 阻塞[事件冒泡](../harmonyos-guides/arkts-interaction-basic-principles.md#事件冒泡)，适用于当前组件已处理鼠标事件且需要阻止事件继续向父组件传递的场景。  **元服务API：** 从API version 11开始，该接口支持在元服务中使用。 |
...
| globalDisplayX20+ | number | 否 | 是 | 鼠标位置在[全局坐标系](../harmonyos-guides/window-terminology.md#global-coordinate-system全局坐标系)中的X坐标。  单位：vp  取值范围：(-∞, +∞)  **元服务API：** 从API version 20开始，该接口支持在元服务中使用。  **模型约束：** 此接口仅可在Stage模型下使用。 |
| globalDisplayY20+ | number | 否 | 是 | 鼠标位置在[全局坐标系](../harmonyos-guides/window-terminology.md#global-coordinate-system全局坐标系)中的Y坐标。  单位：vp  取值范围：(-∞, +∞)  **元服务API：** 从API version 20开始，该接口支持在元服务中使用。  **模型约束：** 此接口仅可在Stage模型下使用。 |
...
该接口仅能在[MouseEvent](ts-universal-mouse-key.md#mouseevent对象说明)中调用，用于获取触发[onMouse](ts-universal-mouse-key.md#onmouse)时当前帧历史点的相关信息，不同设备每帧的鼠标事件上报频率不同，一帧通常只会上报一个鼠标事件，如果当前帧收到的[MouseEvent](ts-universal-mouse-key.md#mouseevent对象说明)数目大于1，会将该帧最后一个点通过[onMouse](ts-universal-mouse-key.md#onmouse)返回，其余点作为历史点。
...
| 类型 | 说明 |
| --- | --- |
| Array<[MouseHistoricalPoint](ts-universal-mouse-key.md#mousehistoricalpoint)> | 当前帧的所有历史点信息组成的数组。 |
...
| globalDisplayX | number | 否 | 否 | 鼠标位置在[全局坐标系](../harmonyos-guides/window-terminology.md#global-coordinate-system全局坐标系)中的X坐标。  单位：vp |
| globalDisplayY | number | 否 | 否 | 鼠标位置在[全局坐标系](../harmonyos-guides/window-terminology.md#global-coordinate-system全局坐标系)中的Y坐标。  单位：vp |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/e4/v3/jbVro4ohSXi1d7L-DmpV_A/zh-cn_image_0000002736434741.gif)

### 示例2（获取当前帧历史点）

该示例通过调用[getHistoricalPoints](ts-universal-mouse-key.md#gethistoricalpoints)接口，获取到触发重采样时的历史点，可以用来实现更平滑的绘制等操作。
...
该示例通过[getCurrentLocalPosition](ts-universal-mouse-key.md#getcurrentlocalposition)方法获取鼠标位置相对于当前组件实时位置左上角的坐标。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/bb/v3/GksQELqXQuOPJp_hJULlEQ/zh-cn_image_0000002706835594.gif)
