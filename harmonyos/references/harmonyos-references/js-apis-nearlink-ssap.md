---
url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-nearlink-ssap
...
| 类型 | 说明 |
| --- | --- |
| [Client](js-apis-nearlink-ssap.md#client) | SSAP客户端实例。 |
...
| 类型 | 说明 |
| --- | --- |
| [Server](js-apis-nearlink-ssap.md#server) | SSAP服务端实例。 |
...
同一应用针对同一远端设备创建一个[Client](js-apis-nearlink-ssap.md#client)实例即可，重复创建会增加不必要的资源开销。
...
关闭客户端，断开与远端服务端的连接。如仅需断开当前连接而保留实例，请使用[disconnect](js-apis-nearlink-ssap.md#disconnect)方法。
...
| 类型 | 说明 |
| --- | --- |
| Promise<[Service](js-apis-nearlink-ssap.md#service)[]> | Promise对象，返回服务端支持的服务列表。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<[Property](js-apis-nearlink-ssap.md#property)> | Promise对象，返回服务端属性。 |
...
| --- | --- | --- | --- |
| property | [Property](js-apis-nearlink-ssap.md#property) | 是 | 服务端属性。 |
| writeType | [PropertyWriteType](js-apis-nearlink-ssap.md#propertywritetype) | 是 | 写类型，支持服务端回复响应和不回复响应两种方式。 |
...
设置[Property](js-apis-nearlink-ssap.md#property)变化通知。需在调用[connect](js-apis-nearlink-ssap.md#connect)成功建立连接后使用。使用Promise异步回调。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| property | [Property](js-apis-nearlink-ssap.md#property) | 是 | 服务端属性。该属性需支持NOTIFY操作（即operation包含NOTIFY，详见[Operation](js-apis-nearlink-ssap.md#operation)）。 |
...
应用需具备ohos.permission.ACCESS\_NEARLINK权限，方可接收此事件上报。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback<[Property](js-apis-nearlink-ssap.md#property)> | 是 | 回调函数，返回服务的Property。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback<[Property](js-apis-nearlink-ssap.md#property)> | 否 | 回调函数，返回服务的Property。  填写该参数则取消当前callback订阅。不填写该参数则取消该事件对应的所有回调。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback<[ConnectionChangeState](js-apis-nearlink-ssap.md#connectionchangestate)> | 是 | 回调函数，返回连接状态上报参数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback<[ConnectionChangeState](js-apis-nearlink-ssap.md#connectionchangestate)> | 否 | 回调函数，返回连接状态上报参数。  填写该参数则取消当前callback订阅。不填写该参数则取消该事件对应的所有回调。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback<number> | 否 | 回调函数，返回协商后的MTU大小。  填写该参数则取消当前callback订阅。不填写该参数则取消该事件对应的所有回调。 |
...
使用该类的方法前，需通过[ssap.createServer](js-apis-nearlink-ssap.md#ssapcreateserver)方法构造该类的实例。

同一应用创建一个[Server](js-apis-nearlink-ssap.md#server)实例即可，重复创建会增加不必要的资源开销。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| service | [Service](js-apis-nearlink-ssap.md#service) | 是 | 服务端提供的服务信息，支持添加多个服务，根据不同的UUID区分。 |
...
| property | [Property](js-apis-nearlink-ssap.md#property) | 是 | 发生值变化的Property。 |
...
回复客户端读写请求。收到[ssap.onPropertyRead](js-apis-nearlink-ssap.md#onpropertyread)或[ssap.onPropertyWrite](js-apis-nearlink-ssap.md#onpropertywrite)上报的请求后，调用本接口向对应客户端回复数据。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| response | [ServerResponse](js-apis-nearlink-ssap.md#serverresponse) | 是 | 回复客户端的响应数据。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback<[PropertyReadRequest](js-apis-nearlink-ssap.md#propertyreadrequest)> | 是 | 回调函数，返回客户端的Property读请求参数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback<[PropertyReadRequest](js-apis-nearlink-ssap.md#propertyreadrequest)> | 否 | 回调函数，返回客户端的Property读请求参数。  填写该参数则取消当前callback订阅。不填写该参数则取消该事件对应的所有回调。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback<[PropertyWriteRequest](js-apis-nearlink-ssap.md#propertywriterequest)> | 是 | 回调函数，返回客户端的Property写请求参数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback<[PropertyWriteRequest](js-apis-nearlink-ssap.md#propertywriterequest)> | 否 | 回调函数，返回客户端的Property写请求参数。  填写该参数则取消当前callback订阅。不填写该参数则取消该事件对应的所有回调。 |
...
| descriptors | [PropertyDescriptor](js-apis-nearlink-ssap.md#propertydescriptor)[] | 否 | 是 | 表示当前Property的描述符列表。若未配置则默认不携带该字段。 |
| operation | number | 否 | 是 | 表示Property支持的操作方式，默认值为READABLE|WRITE\_NO\_RESPONSE，即可读并可写（以无响应方式）。如要使属性支持相应的操作，需要对该字段赋值，例如赋值为：READABLE | WRITE\_NO\_RESPONSE | NOTIFY。取值范围[0, 15]，各比特位对应的操作方式详见[Operation](js-apis-nearlink-ssap.md#operation)。 |
...
| descriptorType | [PropertyDescriptorType](js-apis-nearlink-ssap.md#propertydescriptortype) | 否 | 否 | 表示Property的描述符类型。 |
...
| writeType | [PropertyWriteType](js-apis-nearlink-ssap.md#propertywritetype) | 否 | 否 | 表示客户端写Property类型。 |
...
| --- | --- | --- | --- | --- |
| address | string | 否 | 否 | 表示客户端设备地址。地址格式参考：11:22:33:AA:BB:FF。 |
| requestId | number | 否 | 否 | 表示请求ID。取值范围[0, 65535]。该ID必须与收到的[PropertyReadRequest](js-apis-nearlink-ssap.md#propertyreadrequest)或[PropertyWriteRequest](js-apis-nearlink-ssap.md#propertywriterequest)中的requestId一致，用于关联请求与响应。 |
