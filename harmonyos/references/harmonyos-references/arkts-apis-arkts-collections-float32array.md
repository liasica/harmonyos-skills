---
url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-arkts-collections-float32array
title: Class (Float32Array)
breadcrumb: API参考 > 应用框架 > ArkTS（方舟编程语言） > ArkTS API > @arkts.collections (ArkTS容器集) > Class (Float32Array)
category: harmonyos-references
scraped_at: 2026-04-28T07:59:56+08:00
doc_updated_at: 2026-04-10
content_hash: sha256:50c676dec0374338bda5fc755d56aecb97ac38043f53b06576ce669795499fb9
---
...
| BYTES\_PER\_ELEMENT | number | 是 | 否 | ArkTS Float32Array中每个元素所占用的字节数。 |
...
```
1. let float32Array: collections.Float32Array = new collections.Float32Array();
```
...
```
1. // 以长度参数构造对象
2. let float32Array: collections.Float32Array = new collections.Float32Array(12);
```
...
```
1. // 从一个Iterable构造对象，Float32Array [1, 2, 3]
2. let set: Set<number> = new Set<number>([1, 2, 3]);
3. let array: collections.Float32Array = new collections.Float32Array(set);
```
...
3. let array: collections.Float32Array = new collections.Float32Array(arrayLike);
```

```
1. // 例2 从一个ArrayBuffer构造对象
2. let arrayBuffer: collections.ArrayBuffer = new collections.ArrayBuffer(12);
3. let array: collections.Float32Array = new collections.Float32Array(arrayBuffer);
```

```
1. // 例3 从另一ArkTS Float32Array构造对象
2. let arrayLike = [1, 3, 5];
3. // float32Array1 [1, 3, 5]
4. let float32Array1: collections.Float32Array = new collections.Float32Array(arrayLike);
5. // float32Array2 [1, 3, 5]
6. let float32Array2: collections.Float32Array = new collections.Float32Array(float32Array1);
```
...
```
1. let float32Array: collections.Float32Array = collections.Float32Array.from([1, 2, 3, 4, 5, 6]);
2. console.info("byteLength: " + float32Array.buffer.byteLength); // byteLength: 24
3. // 从float32Array对应buffer第4个字节开始，长度为5
4. let float32Array1: collections.Float32Array = new collections.Float32Array(float32Array.buffer, 4, 5);
5. console.info("[" + float32Array1 + "]"); // [2, 3, 4, 5, 6]
```
...
2. let array: collections.Float32Array = collections.Float32Array.from(arrayLike); // array [1, 3, 5]
```
...
```
1. // 例1 从一个对象创建，Float32Array [0, 1, 2, 3, 4]
2. let array: collections.Float32Array = collections.Float32Array.from<number>(
3. { length: 5 }, (v: Object, k: number) => k);
```

```
1. // 例2 从一个字符数组创建，Float32Array [1, 3, 5]
2. let array: collections.Float32Array = collections.Float32Array.from<string>(
3. ["1", "3", "5"], (v: string, k: number) => parseInt(v));
```

```
1. // 例3 从一个字符串创建，Float32Array [1, 2, 3, 4, 5]
2. let array: collections.Float32Array = collections.Float32Array.from<string>(
...
```
1. // 例1 不指定映射函数，Float32Array [1, 2, 3]
2. let set: Set<number> = new Set<number>([1, 2, 3]);
3. let array: collections.Float32Array = collections.Float32Array.from(set);
```

```
1. // 例2 指定映射函数，Float32Array [1, 3, 5]
2. let set: Set<number> = new Set<number>([1, 2, 3]);
3. let array: collections.Float32Array = collections.Float32Array.from(
...
```
1. let arr: collections.Float32Array = collections.Float32Array.of(1, 2, 3, 4);
...
```
1. let array = new collections.Float32Array([1, 2, 3, 4, 5]);
...
```
1. // 当前应用所在系统为法国地区
2. let array = new collections.Float32Array([1000, 2000, 3000]);
3. let stringArray = array.toLocaleString();
4. console.info(stringArray); // 预期输出：1,000,2,000,3,000
```
...
```
1. let array: collections.Float32Array = collections.Float32Array.from([1, 2, 3, 4, 5, 6, 7, 8]);
2. // Float32Array [1, 2, 3, 2, 3, 6, 7, 8]
3. let copied: collections.Float32Array = array.copyWithin(3, 1, 3);
```
...
2. let float32Array: collections.Float32Array = new collections.Float32Array(arrayLike);
3. float32Array.some((element: number) => element < 0); // true
```
...
1. let arrayLike = [-10, 20, -30, 40, -50];
2. let float32Array: collections.Float32Array = new collections.Float32Array(arrayLike);
3. float32Array.every((element: number) => element > 0);  // false
```
...
2. new collections.Float32Array(arrayLike).fill(4); // Float32Array [4, 4, 4]
3. new collections.Float32Array(arrayLike).fill(4, 1); // Float32Array [1, 4, 4]
4. new collections.Float32Array(arrayLike).fill(4, 1, 2); // Float32Array [1, 4, 3]
```
...
```
1. let array: collections.Float32Array = collections.Float32Array.from([0, 1, 2, 3, 4]);
2. // Float32Array [0, 2, 4]
3. let filtered: collections.Float32Array = array.filter((element: number) => element % 2 == 0);
```
...
```
1. const array: collections.Float32Array = collections.Float32Array.from([1, 2, 3, 4, 5]);
...
```
1. let float32Array: collections.Float32Array = collections.Float32Array.from([1, 2, 3]);
2. float32Array.forEach((value: number, index: number, array: collections.Float32Array) => {
...
```
1. let array: collections.Float32Array = collections.Float32Array.from([3, 5, 9]);
2. array.indexOf(3); // 0
3. array.indexOf(7); // -1
...
```
1. let array: collections.Float32Array = collections.Float32Array.from([1, 2, 3, 4, 5]);
...
```
1. let array: collections.Float32Array = collections.Float32Array.from([25, 36, 49]);
2. const mapped: collections.Float32Array = array.map(Math.sqrt); // Float32Array [5, 6 ,7]
```
...
```
1. let array: collections.Float32Array = collections.Float32Array.from([1, 2, 3, 4, 5]);
2. let reversed: collections.Float32Array = array.reverse(); // Float32Array [5, 4, 3, 2, 1]
```
...
2. let array: collections.Float32Array = new collections.Float32Array(buffer);
3. array.set([1, 2, 3], 3); // Float32Array [0, 0, 0, 1, 2, 3, 0, 0]
```
...
```
1. let array: collections.Float32Array = collections.Float32Array.from([1, 2, 3, 4, 5]);
2. array.slice(); // Float32Array [1, 2, 3, 4, 5]
3. array.slice(1, 3); // Float32Array [2, 3]
4. array.slice(-2); // Float32Array [4, 5]
```
...
```
1. let array: collections.Float32Array = collections.Float32Array.from([1, 3, 5, 4, 2]);
2. array.sort(); // Float32Array [1, 2, 3, 4, 5]
3. array.sort((a: number, b: number) => a - b); // Float32Array [1, 2, 3, 4, 5]
4. array.sort((a: number, b: number) => b - a); // Float32Array [5, 4, 3, 2, 1]
```
...
```
1. let array: collections.Float32Array = collections.Float32Array.from([1, 2, 3, 4, 5]);
2. let subArray: collections.Float32Array = array.subarray(); // Float32Array [1, 2, 3, 4, 5]
3. subArray.set([10, 20, 30]); // Float32Array [10, 20, 30, 4, 5]
```
...
```
1. let array: collections.Float32Array = collections.Float32Array.from([1, 2, 3]);
...
```
1. let array: collections.Float32Array = collections.Float32Array.from([11, 22, 33]);
...
3. for (let item of float32Array) {
...
```
1. let float32Array = collections.Float32Array.from([1, 2, 4]);
2. console.info("Element at index 1: ", float32Array[1]);
```
