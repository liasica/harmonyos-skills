...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| sessionConfiguration | [SessionConfiguration](remote-communication-rcp.md#sessionconfiguration) | 否 | 会话配置，应用于会话请求的设置。 |
...
| configuration | [SessionConfiguration](remote-communication-rcp.md#sessionconfiguration) | undefined | 是 | 否 | 表示会话的配置设置。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<[Response](remote-communication-rcp.md#response)> | Promise对象，返回来自服务器的响应对象。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<[ResponseSendable](remote-communication-rcp.md#responsesendable)> | Promise对象，返回来自服务器的响应对象，该响应对象支持Sendable。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| url | [URLOrString](remote-communication-rcp.md#urlorstring) | 是 | HTTP GET请求资源的URL。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| url | [URLOrString](remote-communication-rcp.md#urlorstring) | 是 | HTTP POST请求资源的URL。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| url | [URLOrString](remote-communication-rcp.md#urlorstring) | 是 | HTTP HEAD请求资源的URL。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| url | [URLOrString](remote-communication-rcp.md#urlorstring) | 是 | HTTP DELETE请求资源的URL。 |
...
| 类型 | 说明 |
| --- | --- |
| [DefaultSession](remote-communication-rcp.md#defaultsession) | 代表一个默认会话，用于发送或取消HTTP请求。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<[Response](remote-communication-rcp.md#response)> | Promise对象。此拦截器对下一个拦截器做拦截、修改、透传等操作后，返回此拦截器提供的[Response](remote-communication-rcp.md#response)对象。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| transfer | [TransferConfiguration](remote-communication-rcp.md#transferconfiguration) | 否 | 是 | 配置与HTTP请求期间的数据传输相关的各个方面，例如自动重定向和超时设置。 |
| tracing | [TracingConfiguration](remote-communication-rcp.md#tracingconfiguration) | 否 | 是 | 使开发者能够在HTTP请求期间捕获详细的跟踪信息，有助于调试和性能分析。 |
| proxy | [ProxyConfiguration](remote-communication-rcp.md#proxyconfiguration) | 否 | 是 | 配置会话的代理设置，允许开发者定义'system'、'no-proxy'或WebProxy配置。 |
| dns | [DnsConfiguration](remote-communication-rcp.md#dnsconfiguration) | 否 | 是 | 指定与DNS相关的配置，包括自定义DNS规则和通过HTTPS使用DNS的选项。 |
| security | [SecurityConfiguration](remote-communication-rcp.md#securityconfiguration) | 否 | 是 | 包括用于处理安全相关方面的设置，如证书和服务器身份验证。 |
| processing | [ProcessingConfiguration](remote-communication-rcp.md#processingconfiguration) | 否 | 是 | 响应处理配置。  **起始版本：** 5.0.0(12) |
| cache | [ResponseCache](remote-communication-rcp.md#responsecache) | 否 | 是 | 指定与HTTP缓存相关的配置。  **起始版本：** 6.0.0(20) |
...
| 类型 | 说明 |
| --- | --- |
| [CookieRepository](remote-communication-rcp.md#cookierepository) | cookie仓库实例。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<[ResponseCookie](remote-communication-rcp.md#responsecookie)[]> | Promise对象，返回[ResponseCookie](remote-communication-rcp.md#responsecookie)列表。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| url | [URL](remote-communication-rcp.md#url) | 是 | HTTP请求的url，用于匹配cookie。匹配规则符合[RFC 6265](https://datatracker.ietf.org/doc/html/rfc6265)协议。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| Promise<[ResponseCookie](remote-communication-rcp.md#responsecookie)[]> | Promise对象，返回匹配url的cookie列表。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| cookies | [ResponseCookie](remote-communication-rcp.md#responsecookie) | [ResponseCookie](remote-communication-rcp.md#responsecookie)[] | 否 | 需要删除的cookie。cookie的匹配规则和抛出错误的规则参考[setCookies](remote-communication-rcp.md#setcookies)。 |
...
| [WebProxy](remote-communication-rcp.md#webproxy) | HTTP代理配置。WebProxy：表示提供自定义代理设置。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| dnsRules | [DnsServers](remote-communication-rcp.md#dnsservers) | [StaticDnsRules](remote-communication-rcp.md#staticdnsrules) | [DynamicDnsRule](remote-communication-rcp.md#dynamicdnsrule) | 否 | 是 | 配置DNS规则，默认值为undefined。  DnsServers：表示优先使用指定的DNS服务器解析主机名。  StaticDnsRules：静态DNS规则，表示如果hostname匹配，则优先使用指定的地址。  DynamicDnsRule：动态DNS规则，表示优先使用函数中返回的地址。  **说明：** 针对同一域名配置多个静态或者动态DNS规则后，默认情况下，仅有最后一个IP地址生效。如果希望多个IP地址同时生效，在6.0.0(20)版本后，支持同一域名配置多个静态或者动态DNS规则，此时将happyEyeballOnDnsRule设置为true可以让多个IP地址同时生效。 |
| dnsOverHttps | [DnsOverHttpsConfiguration](remote-communication-rcp.md#dnsoverhttpsconfiguration) | 否 | 是 | DNS over HTTPS配置。默认值为undefined。  如果设置，则优先使用DNS服务器解析的地址。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| url | [URLOrString](remote-communication-rcp.md#urlorstring) | 否 | 否 | DOH端点的URL或字符串表示形式。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| credential | [Credential](remote-communication-rcp.md#credential) | 否 | 否 | 服务器的凭证。默认值为undefined。 |
| authenticationType | [AuthenticationType](remote-communication-rcp.md#authenticationtype) | 否 | 是 | 服务器的认证类型。如果没有设置，需与服务器协商。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| min | [TlsVersion](remote-communication-rcp.md#tlsversion) | 否 | 是 | TLS可用最小版本，默认不设置，由系统选择。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| info | [AuthenticationChallengeInfo](remote-communication-rcp.md#authenticationchallengeinfo)[] | 是 | 认证挑战参数。系统解析响应头中的WWW-Authenticate信息，并将信息转化为键值对。 |
| response | [Response](remote-communication-rcp.md#response) | 是 | 此次认证挑战的HTTP响应。 |
...
| transferRange | [TransferRange](remote-communication-rcp.md#transferrange) | [TransferRange](remote-communication-rcp.md#transferrange)[] | 否 | 是 | HTTP传输范围。转换为HTTP Range头。带有range头的HTTP请求要求服务器只返回HTTP响应的一部分。默认值为undefined。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| url | [URLOrString](remote-communication-rcp.md#urlorstring) | 是 | 请求的地址类型，在构造函数中，参数可以是 URLOrString，但 URL 只是 URL，字符串需要转换为URL。 |
| method | [HttpMethod](remote-communication-rcp.md#httpmethod) | 否 | HTTP 请求方法。默认是GET。 |
| headers | [RequestHeaders](remote-communication-rcp.md#requestheaders) | 否 | HTTP请求头。默认值为undefined。 |
| content | [RequestContent](remote-communication-rcp.md#requestcontent) | 否 | HTTP 请求正文。默认值为undefined。 |
| cookies | [RequestCookies](remote-communication-rcp.md#requestcookies) | 否 | HTTP 请求 cookie。该设置将转换为 HTTP Cookies header。 |
| transferRange | [TransferRange](remote-communication-rcp.md#transferrange)| [TransferRange](remote-communication-rcp.md#transferrange)[] | 否 | HTTP 传输范围。该设置将转换为 HTTP Range header。 |
| configuration | [Configuration](remote-communication-rcp.md#configuration) | 否 | HTTP 请求配置。见[Configuration](remote-communication-rcp.md#configuration)。 |
...
请求的地址类型，链接到@[ohos.url](js-apis-url.md)中的URL。
...
文件系统中的文件，链接到@[ohos.file.fs](js-apis-file-fs.md)中的File。
...
| 类型 | 说明 |
| --- | --- |
| [FileDescriptor](remote-communication-rcp.md#filedescriptor) | 表示文件的描述符。文件描述符是一个整数，用于标识一个打开的文件。 |
| [File](remote-communication-rcp.md#file) | 文件系统中的文件，用于获取文件的描述符、路径和名称，以及对文件的锁定和解锁操作。 |
| [RandomAccessFile](remote-communication-rcp.md#randomaccessfile) | 提供随机访问文件的操作功能。用于处理随机访问文件的一种类型。提供了文件描述符、文件指针的读取，以及文件指针的设置和文件的写入操作。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| path | [Path](remote-communication-rcp.md#path) | 否 | 否 | 指定的数据下载路径。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| request | [Request](remote-communication-rcp.md#request) | 是 | 表示入参的类型为一个HTTP请求。 |
| suggestedPath | [Path](remote-communication-rcp.md#path) | 是 | 表示入参的类型为[Path](remote-communication-rcp.md#path)，请求中指定的路径。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| fileOrPath | [Path](remote-communication-rcp.md#path) | [LocalFile](remote-communication-rcp.md#localfile) | [ReadFile](remote-communication-rcp.md#readfile) | 是 | 表示需要上传的文件或者上传文件的路径，参考[应用文件](../harmonyos-guides/app-file.md)。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| stream | [Stream](remote-communication-rcp.md#stream) | [ReadStream](remote-communication-rcp.md#readstream) | [SyncReadStream](remote-communication-rcp.md#syncreadstream) | 是 | 表示需要上传的流。 |
...
| 类型 | 说明 |
| --- | --- |
| { kind: 'stream'; stream: [Stream](remote-communication-rcp.md#stream)| [WriteStream](remote-communication-rcp.md#writestream)| [SyncWriteStream](remote-communication-rcp.md#syncwritestream) } | 表示键kind值为'stream'，键stream的值为  [Stream](remote-communication-rcp.md#stream)或[WriteStream](remote-communication-rcp.md#writestream)或[SyncWriteStream](remote-communication-rcp.md#syncwritestream)的对象。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| receiving | [ReceivingPausePolicy](remote-communication-rcp.md#receivingpausepolicy) | 否 | 是 | 设置暂停响应体接收的策略。 |
| sending | [SendingPausePolicy](remote-communication-rcp.md#sendingpausepolicy) | 否 | 是 | 设置暂停请求体发送的策略。 |
...
| 类型 | 说明 |
| --- | --- |
| [ReceivingPauseByCache](remote-communication-rcp.md#receivingpausebycache) | 策略的缓存大小。 |
| [ReceivingPauseByTimeout](remote-communication-rcp.md#receivingpausebytimeout) | 策略的超时时间。 |
...
创建一个[INetworkOutputQueue](remote-communication-rcp.md#inetworkoutputqueue)。
...
| pausePolicyOverride | [ReceivingPausePolicy](remote-communication-rcp.md#receivingpausepolicy) | 是 | 请求的接收暂停策略。 |
...
| [IncomingDataCallback](remote-communication-rcp.md#incomingdatacallback) | 表示值的类型为[IncomingDataCallback](remote-communication-rcp.md#incomingdatacallback)类型数据。将响应体写到回调函数中。 |
| [DownloadToFile](remote-communication-rcp.md#downloadtofile) | 表示值的类型为[DownloadToFile](remote-communication-rcp.md#downloadtofile)类型数据。将响应体写到文件中。 |
| [DownloadToStream](remote-communication-rcp.md#downloadtostream) | 表示值的类型为[DownloadToStream](remote-communication-rcp.md#downloadtostream)类型对象。将响应体写到自定义的流中。 |
| [INetworkOutputQueue](remote-communication-rcp.md#inetworkoutputqueue) | 表示值的类型为[INetworkOutputQueue](remote-communication-rcp.md#inetworkoutputqueue)类型对象。把响应体写到队列。 |
...
| 类型 | 说明 |
| --- | --- |
| [IpAndPort](remote-communication-rcp.md#ipandport)[] | 表示值的类型为[IpAndPort](remote-communication-rcp.md#ipandport)数组，数组成员为地址和端口键值对。 |
...
| 类型 | 说明 |
| --- | --- |
| { [k: string]: string | string[] | undefined;  'authorization'?: string;  'accept'?: [ContentType](remote-communication-rcp.md#contenttype) | [ContentType](remote-communication-rcp.md#contenttype)[];  'accept-charset'?: string | string[];  'accept-encoding'?: [ContentCoding](remote-communication-rcp.md#contentcoding) | [ContentCoding](remote-communication-rcp.md#contentcoding)[];  'accept-language'?: string | string[];  'cache-control'?: string | string[];  'cookie'?: string | string[];  'range'?: string | string[];  'upgrade'?: string | string[];  'user-agent'?: string;  'content-type'?: [ContentType](remote-communication-rcp.md#contenttype); } | 若键取值为[k: string]: string | string[] | undefined，表示值类型为一个包含一个或多个键值对的对象，其键的类型为字符，可取任意值，其值的类型为字符、字符数组或undefined。  若键取值为'authorization'，值的类型为字符，可取任意值。  若键取值为'accept'，值的类型为[ContentType](remote-communication-rcp.md#contenttype)或[ContentType](remote-communication-rcp.md#contenttype)数组，可取任意值。  若键取值为'accept-charset'，值的类型为字符或字符数组，可取任意值。  若键取值为'accept-encoding'，值的类型为[ContentCoding](remote-communication-rcp.md#contentcoding)或[ContentCoding](remote-communication-rcp.md#contentcoding)数组，可取任意值。  若键取值为'accept-language'，值的类型为字符或字符数组，可取任意值。  若键取值为'cache-control'，值的类型为字符或字符数组，可取任意值。  若键取值为'cookie'，值的类型为字符或字符数组，可取任意值。  若键取值为'range'，值的类型为字符或字符数组，可取任意值。  若键取值为'upgrade'，值的类型为字符或字符数组，可取任意值。  若键取值为'user-agent'，值的类型为字符，可取任意值。  若键取值为'content-type'，值的类型为[ContentType](remote-communication-rcp.md#contenttype)，可取任意值。当此项不存在或为空字符串时，系统将根据[RequestContent](remote-communication-rcp.md#requestcontent)配置默认的内容类型。若不希望使用默认内容类型，可以传入单个空格。 |
...
创建一个[INetworkInputQueue](remote-communication-rcp.md#inetworkinputqueue)。
...
| --- | --- | --- | --- |
| maxSize | number | 是 | 队列大小，表示可写入的最大字节数。 |
| pausePolicyOverride | [SendingPausePolicy](remote-communication-rcp.md#sendingpausepolicy) | 是 | 请求的发送暂停策略。 |
...
| 类型 | 说明 |
| --- | --- |
| [RawDataContent](remote-communication-rcp.md#rawdatacontent) | ArkTs的基本类型的数据。 |
| [Form](remote-communication-rcp.md#form) | HTTP简单的表格数据。 |
| [MultipartForm](remote-communication-rcp.md#multipartform) | HTTP多部分表格数据。 |
| [GetDataCallback](remote-communication-rcp.md#getdatacallback) | 获取任意数据的回调方法的类型，其入参为数据最大字节数。 |
| [UploadFromStream](remote-communication-rcp.md#uploadfromstream) | UploadFromStream表示以流的形式进行上传操作。  **起始版本：** 5.0.0(12) |
| [UploadFromFile](remote-communication-rcp.md#uploadfromfile) | UploadFromFile表示一种文件上传的方式，允许客户端将本地计算机上的文件上传到服务器。  **起始版本：** 5.0.0(12) |
| [INetworkInputQueue](remote-communication-rcp.md#inetworkinputqueue) | INetworkInputQueue是用于写请求体的同步写队列。  **起始版本：** 5.0.0(12) |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| fields | [FormFields](remote-communication-rcp.md#formfields) | 否 | 否 | HTTP简单表单数据字段，类型为表单字段类型。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| fields | [FormFields](remote-communication-rcp.md#formfields) | 是 | HTTP简单表单数据字段，类型为表单字段类型。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| fields | [MultipartFormFields](remote-communication-rcp.md#multipartformfields) | 否 | 否 | 表示多部分表单数据，其中值为多表单字段类型。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| fields | [MultipartFormFields](remote-communication-rcp.md#multipartformfields) | 是 | 表示多部分表单数据，其中值为多表单字段类型。 |
...
| 类型 | 说明 |
| --- | --- |
| [FormFieldValue](remote-communication-rcp.md#formfieldvalue) | 表示值的类型为[FormFieldValue](remote-communication-rcp.md#formfieldvalue)，HTTP简单表单数据字段值。 |
| [FormFieldFileValue](remote-communication-rcp.md#formfieldfilevalue) | 表示值的类型为[FormFieldFileValue](remote-communication-rcp.md#formfieldfilevalue)，文件表单数据接口。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| request | [Request](remote-communication-rcp.md#request) | 是 | 否 | 收到此响应的相关HTTP请求内容。 |
| statusCode | number | 是 | 否 | HTTP请求的结果代码。如果回调函数成功执行，将返回ResponseCode中定义的结果代码。否则，将在AsyncCallback中的error字段中返回错误代码。 |
| headers | [ResponseHeaders](remote-communication-rcp.md#responseheaders) | 是 | 否 | 响应头。 |
| effectiveUrl | [URL](remote-communication-rcp.md#url) | 是 | 是 | 重定向后请求的有效URL。默认值为undefined。 |
| body | ArrayBuffer | 是 | 是 | 响应内容根据响应头中的Content-type返回。响应内容必须与服务器返回的数据类型相同。默认最大下载字节数是50M。如果超过50M，为了防止内存溢出，请通过[OnDataReceive](remote-communication-rcp.md#ondatareceive)来流式接收数据。 |
| downloadedTo | [DownloadedTo](remote-communication-rcp.md#downloadedto) | 是 | 是 | 内容下载的路径。在使用下载至文件对象和文件标识符两种方式时，默认情况下，并未设置回调信息。  **起始版本：** 5.0.0(12) |
| debugInfo | [DebugInfo](remote-communication-rcp.md#debuginfo)[] | 是 | 是 | 与响应相关联的调试信息。默认值为undefined。 |
| timeInfo | [TimeInfo](remote-communication-rcp.md#timeinfo) | 是 | 是 | HTTP请求各阶段的定时信息。默认值为undefined。 |
| cookies | [ResponseCookie](remote-communication-rcp.md#responsecookie)[] | 是 | 是 | 响应中的Cookie数组。 |
| httpVersion | [HttpVersion](remote-communication-rcp.md#httpversion) | 是 | 是 | HTTP的版本。  **起始版本：** 5.0.0(12) |
| reasonPhrase | string | 是 | 是 | HTTP响应状态行的reasonPhrase，提供与数字状态代码相关的文本描述。  **起始版本：** 5.0.0(12) |
| cacheInfo | [CacheInfo](remote-communication-rcp.md#cacheinfo) | 是 | 是 | 缓存信息，包含缓存已存在时长、原始HTTP请求ID信息。当HTTP响应从缓存中获取时，该属性有值，否则为undefined。  **起始版本：** 6.0.0(20) |
...
返回默认的JSON反序列化结果，如需要定制JSON反序列化方式，请参考[JSON解析与生成](js-apis-json.md)。
...
根据解析配置对[toString](remote-communication-rcp.md#tostring)方法返回的字符串进行JSON解析，并返回解析后的结果。详情请参阅[JSON解析与生成](js-apis-json.md)的详细介绍。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| param | [json.ParseOptions](js-apis-json.md#parseoptions) | 是 | 解析配置项。支持定义处理BigInt的模式。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| object | null | 对通过[toString()](remote-communication-rcp.md#tostring)方法转换后的字符串进行JSON解析的结果，如果解析过程中发生异常，则返回null。toJSON方法用于处理body，当body不是JSON格式时，返回值同样为null。 |
...
| downloadedTo | [DownloadedTo](remote-communication-rcp.md#downloadedto) | 是 | 是 | 内容下载的路径，在使用下载至文件对象和文件标识符两种方式时，默认情况下，不设置回调信息。 |
| debugInfo | [DebugInfo](remote-communication-rcp.md#debuginfo)[] | 是 | 是 | 与响应相关联的调试信息，默认值为undefined。 |
| timeInfo | [TimeInfo](remote-communication-rcp.md#timeinfo) | 是 | 是 | HTTP请求各阶段的定时信息，默认值为undefined。 |
| cookies | [ResponseCookie](remote-communication-rcp.md#responsecookie)[] | 是 | 是 | 响应中的Cookie数组。 |
| httpVersion | [HttpVersion](remote-communication-rcp.md#httpversion) | 是 | 是 | HTTP的版本。 |
| reasonPhrase | string | 是 | 是 | 提供HTTP请求结果代码的文本描述。 |
| cacheInfo | [CacheInfo](remote-communication-rcp.md#cacheinfo) | 是 | 是 | 缓存信息，包含缓存已存在时长、原始HTTP请求ID信息。当HTTP响应从缓存中获取时，该属性有值，否则为undefined。 |
...
| x509Certs | [X509Cert](remote-communication-rcp.md#x509cert)[] | 否 | 否 | X509证书对象。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| context | [ValidationContext](remote-communication-rcp.md#validationcontext) | 是 | 表示值的类型为context，是证书上下文。 |
...
| 类型 | 说明 |
| --- | --- |
| [TlsV12CipherSuite](remote-communication-rcp.md#tlsv12ciphersuite) | 表示值的类型为[TlsV12CipherSuite](remote-communication-rcp.md#tlsv12ciphersuite)。 |
| [TlsV13SpecificCipherSuite](remote-communication-rcp.md#tlsv13specificciphersuite) | 表示值的类型为[TlsV13SpecificCipherSuite](remote-communication-rcp.md#tlsv13specificciphersuite)。 |
...
| 类型 | 说明 |
| --- | --- |
| [TlsV11CipherSuite](remote-communication-rcp.md#tlsv11ciphersuite) | 表示值的类型为[TlsV11CipherSuite](remote-communication-rcp.md#tlsv11ciphersuite)。 |
| [TlsV12SpecificCipherSuite](remote-communication-rcp.md#tlsv12specificciphersuite) | 表示值的类型为[TlsV12SpecificCipherSuite](remote-communication-rcp.md#tlsv12specificciphersuite)。 |
...
| 类型 | 说明 |
| --- | --- |
| [TlsV10SpecificCipherSuite](remote-communication-rcp.md#tlsv10specificciphersuite) | 表示值的类型为[TlsV10SpecificCipherSuite](remote-communication-rcp.md#tlsv10specificciphersuite)。 |
...
| cipherSuite | [TlsV12CipherSuite](remote-communication-rcp.md#tlsv12ciphersuite)[] | 否 | 是 | TLS1.2版本对应的加密套件。 |
...
| cipherSuite | [TlsV10CipherSuite](remote-communication-rcp.md#tlsv10ciphersuite)[] | 否 | 是 | TLS1.0版本对应的加密套件。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| response | [Response](remote-communication-rcp.md#response) | 是 | 框架传入的响应，即需要进行校验的响应。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| url | [URLOrString](remote-communication-rcp.md#urlorstring) | 是 | 表示URL对象或表示URL的字符串的并集类型。  表示入参的类型为[URLOrString](remote-communication-rcp.md#urlorstring)，返回值为boolean的方法。为true，代表主机使用代理，false则代表未使用。 |
...
| 类型 | 说明 |
| --- | --- |
| [IpAddress](remote-communication-rcp.md#ipaddress)[] | 一个IP地址数组。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| onDataReceive | [OnDataReceive](remote-communication-rcp.md#ondatareceive) | 否 | 是 | 当接收到HTTP响应正文的一部分时调用的回调。默认为undefined。当开发者需要请求超过50M字节的数据时，使用此回调接受数据。使用后，返回的[Response](remote-communication-rcp.md#response)对象的body字段将不再含有数据。 |
| onUploadProgress | [OnUploadProgress](remote-communication-rcp.md#onuploadprogress) | 否 | 是 | 用于在响应期间处理上传进度的回调。默认为undefined。 |
| onDownloadProgress | [OnDownloadProgress](remote-communication-rcp.md#ondownloadprogress) | 否 | 是 | 用于在响应期间处理下载进度的的回调。默认为undefined。 |
| onHeaderReceive | [OnHeaderReceive](remote-communication-rcp.md#onheaderreceive) | 否 | 是 | 用于在响应期间处理接收到的headers的回调。默认为undefined。 |
| onDataEnd | [OnDataEnd](remote-communication-rcp.md#ondataend) | 否 | 是 | 数据传输完成时触发的回调。默认为undefined。 |
| onCanceled | [OnCanceled](remote-communication-rcp.md#oncanceled) | 否 | 是 | 取消请求/响应时触发的回调。默认为undefined。 |
| onTimeInfo | [OnTimeInfo](remote-communication-rcp.md#ontimeinfo) | 否 | 是 | HTTP请求失败/成功时触发的回调。默认为undefined。  **起始版本：** 5.0.3(15) |
| onStatusCodeReceive | [OnStatusCodeReceive](remote-communication-rcp.md#onstatuscodereceive) | 否 | 是 | 用于在响应期间处理接收到的状态码的回调。默认为undefined。  **起始版本：** 6.0.0(20) |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| onCanceled | [OnCanceled](remote-communication-rcp.md#oncanceled) | 否 | 是 | 会话取消事件的回调。会话取消时调用。默认为undefined。 |
...
此类型表示接收到HTTP body时的回调。当开发者需要请求超过50M字节的数据时，使用此回调接受数据。使用后，返回的[Response](remote-communication-rcp.md#response)对象的body字段将不再含有数据。可以使用Promise异步回调。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| headers | [ResponseHeaders](remote-communication-rcp.md#responseheaders) | 是 | 参数为HTTP响应头，返回为空。 |
| request | [Request](remote-communication-rcp.md#request) | 否 | 触发回调的HTTP请求。  **起始版本：** 6.0.0(20) |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| timeInfo | [TimeInfo](remote-communication-rcp.md#timeinfo) | 是 | HTTP各个阶段耗时。 |
| request | [Request](remote-communication-rcp.md#request) | 否 | 触发回调的HTTP请求。  **起始版本：** 6.0.0(20) |
...
| request | [Request](remote-communication-rcp.md#request) | 否 | 触发回调的HTTP请求。 |
...
| 类型 | 说明 |
| --- | --- |
| [NeverExpirationPolicy](remote-communication-rcp.md#neverexpirationpolicy) | 永不过期类型过期策略。 |
| [TimeLimitedExpirationPolicy](remote-communication-rcp.md#timelimitedexpirationpolicy) | 时间限制类型过期策略。 |
...
| 类型 | 说明 |
| --- | --- |
| [AbsoluteTimeExpirationPolicy](remote-communication-rcp.md#absolutetimeexpirationpolicy) | 绝对时间过期策略。到达指定时间，该缓存记录过期。 |
| [RelativeTimeExpirationPolicy](remote-communication-rcp.md#relativetimeexpirationpolicy) | 相对时间过期策略。经过指定的时间间隔后，该缓存记录过期。 |
| [SlidingTimeExpirationPolicy](remote-communication-rcp.md#slidingtimeexpirationpolicy) | 滑动时间过期策略。指定的时间间隔期间，若无任何访问，该缓存记录过期；若有访问，则重置过期时间。 |
...
| time | [TimeInterval](remote-communication-rcp.md#timeinterval) | 否 | 否 | 指定时间间隔。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| units | [TimeIntervalUnits](remote-communication-rcp.md#timeintervalunits) | 否 | 否 | 时间间隔单位。 |
...
| headers | [rcp.ResponseHeaders](remote-communication-rcp.md#responseheaders) | 否 | 否 | HTTP响应头信息。 |
| body | ArrayBuffer | 否 | 是 | HTTP响应内容。 |
| effectiveUrl | string | 否 | 是 | 重定向后请求的有效URL。默认值为undefined。 |
| httpVersion | [rcp.HttpVersion](remote-communication-rcp.md#httpversion) | 否 | 是 | HTTP的版本。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| expirationPolicy | [ExpirationPolicy](remote-communication-rcp.md#expirationpolicy) | 否 | 否 | 缓存过期策略。 |
| response | [CachedResponse](remote-communication-rcp.md#cachedresponse) | 否 | 否 | 存储的缓存响应。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| persistent | [PersistentStorageConfiguration](remote-communication-rcp.md#persistentstorageconfiguration) | 否 | 否 | 指定持久化缓存配置。 |
| inMemory | [InMemoryCacheConfiguration](remote-communication-rcp.md#inmemorycacheconfiguration) | 否 | 是 | 指定内存缓存配置。 |
...
| defaultExpirationPolicy | [TimeLimitedExpirationPolicy](remote-communication-rcp.md#timelimitedexpirationpolicy) | 否 | 是 | 默认缓存过期策略。  如果设置，它将应用于每个新增的响应记录；  如果未设置，将默认设置为1小时的[SlidingTimeExpirationPolicy](remote-communication-rcp.md#slidingtimeexpirationpolicy)。  [set](remote-communication-rcp.md#set)方法配置的缓存过期策略会覆盖本接口默认缓存过期策略。 |
...
| 类型 | 说明 |
| --- | --- |
| [FileSystemStorageConfiguration](remote-communication-rcp.md#filesystemstorageconfiguration) | 基于文件系统的缓存存储配置。 |
...
| pathToFolder | string | 否 | 否 | 指定存储缓存的文件路径。可以参考[应用文件](../harmonyos-guides/app-file.md)。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| configuration | [CacheConfiguration](remote-communication-rcp.md#cacheconfiguration) | 是 | 缓存配置。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| key | [ResponseCacheKey](remote-communication-rcp.md#responsecachekey) | 是 | 缓存键。 |
| response | [CachedResponse](remote-communication-rcp.md#cachedresponse) | 是 | 需要添加或替换的HTTP缓存响应内容。 |
| expirationPolicy | [ExpirationPolicy](remote-communication-rcp.md#expirationpolicy) | 否 | 缓存过期策略。若传入该参数，则会覆盖[CacheConfiguration](remote-communication-rcp.md#cacheconfiguration)中的默认缓存过期策略，即defaultExpirationPolicy。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<[ResponseCacheRecord](remote-communication-rcp.md#responsecacherecord)> | Promise对象，返回HTTP响应缓存记录。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<[CacheState](remote-communication-rcp.md#cachestate)> | Promise对象，返回缓存状态信息。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| url | [URLOrString](remote-communication-rcp.md#urlorstring) | 是 | URL，用于与缓存键中的URL进行匹配。 |
| matchKind | [URLMatchKind](remote-communication-rcp.md#urlmatchkind) | 是 | URL匹配类型。 |
| method | [HttpMethod](remote-communication-rcp.md#httpmethod) | 否 | HTTP请求方法。 |
...
| 类型 | 说明 |
| --- | --- |
| [Response](remote-communication-rcp.md#response) | HTTP响应。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| response | [Response](remote-communication-rcp.md#response) | 是 | HTTP响应。 |
...
| 类型 | 说明 |
| --- | --- |
| [CachedResponse](remote-communication-rcp.md#cachedresponse) | HTTP响应缓存。 |
