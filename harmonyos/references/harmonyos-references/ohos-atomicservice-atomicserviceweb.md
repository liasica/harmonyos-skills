...
| nestedScroll15+ | [NestedScrollOptions](ts-container-scrollable-common.md#nestedscrolloptions10对象说明) | [NestedScrollOptionsExt](arkts-basic-components-web-i.md#nestedscrolloptionsext14) | 否 | @Prop | 设置嵌套滚动选项。nestedScroll为NestedScrollOptions（向前、向后两个方向）类型时，scrollForward、scrollBackward默认滚动选项为NestedScrollMode.SELF\_FIRST。nestedScroll为NestedScrollOptionsExt（上下左右四个方向）类型时，scrollUp、scrollDown、scrollLeft、scrollRight默认滚动选项为NestedScrollMode.SELF\_FIRST。  **元服务API：** 从API version 15开始，该接口支持在元服务中使用。 |
...
| onHttpErrorReceive | Callback<[OnHttpErrorReceiveEvent](ohos-atomicservice-atomicserviceweb.md#onhttperrorreceiveevent)> | 否 | - | 网页加载资源时遇到HTTP错误（响应码>=400）触发该回调。 |
...
| onProgressChange | Callback<[OnProgressChangeEvent](ohos-atomicservice-atomicserviceweb.md#onprogresschangeevent)> | 否 | - | 网页加载进度变化时触发该回调。 |
...
| 类型 | 说明 |
| --- | --- |
| string | 自定义用户代理。默认User-Agent定义与使用场景请参考[User-Agent开发指导](../harmonyos-guides/web-default-useragent.md)。 |
...
按照历史栈，前进一个页面。可结合[accessForward](ohos-atomicservice-atomicserviceweb.md#accessforward)一起使用。
...
按照历史栈，后退一个页面。可结合[accessBackward](ohos-atomicservice-atomicserviceweb.md#accessbackward)一起使用。
...
当前页面是否可前进，即当前页面是否有前进历史记录。可结合[forward](ohos-atomicservice-atomicserviceweb.md#forward)一起使用，先通过此方法判断是否可前进，再调用forward执行前进操作。
...
当前页面是否可后退，即当前页面是否有返回历史记录。可结合[backward](ohos-atomicservice-atomicserviceweb.md#backward)一起使用，先通过此方法判断是否可后退，再调用backward执行后退操作。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| url | string | [Resource](ts-types.md#resource) | 是 | 需要加载的 URL，需为合法的URL格式。 |
| headers | Array<[WebHeader](ohos-atomicservice-atomicserviceweb.md#webheader)> | 否 | URL的附加HTTP请求头。默认值为空数组，即不添加附加请求头。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| event | [OnLoadInterceptEvent](ohos-atomicservice-atomicserviceweb.md#onloadinterceptevent) | 是 | 当Web组件加载url之前触发的加载拦截事件。 |
