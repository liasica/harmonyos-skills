...
* [URL](js-apis-url.md#url)：用于解析和构造完整URL。
* [URLParams](js-apis-url.md#urlparams9)：用于操作URL查询参数。
* [URLSearchParams](js-apis-url.md#urlsearchparamsdeprecated)：从API version 9开始废弃，建议使用[URLParams](js-apis-url.md#urlparams9)替代。
...
将新的键值对插入到查询字符串。与[set](js-apis-url.md#set9)方法不同，append不会替换已存在的键名对应的值，而是追加一个新的键值对，允许同一键名存在多个值。如需替换已有键值，请使用set方法。
...
URL的构造函数。与[parseURL](js-apis-url.md#parseurl9)方法功能相同，但parseURL为静态工厂方法，推荐使用parseURL来创建URL对象。
...
URL的无参构造函数，不建议直接调用。请使用[parseURL](js-apis-url.md#parseurl9)方法创建URL对象。
...
| 类型 | 说明 |
| --- | --- |
| [URL](js-apis-url.md#url) | 返回解析后的URL对象，包含URL的各组成部分（如协议、主机和路径等属性）。 |
...
从API version 7开始支持，从API version 9开始废弃，建议使用[URLParams](js-apis-url.md#urlparams9)替代。
