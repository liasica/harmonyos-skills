...
content_hash: sha256:3d8cd341957be10912181fb3baab672fa7ed1583166ff37ce2ea0bfab4460b9c
---

安全区域是指页面的显示区域，默认情况下开发者开发的界面都布局在安全区域内，不与系统设置的避让区（如状态栏、导航栏）重叠。提供属性方法允许开发者设置组件绘制内容突破安全区域限制：

* 通过[expandSafeArea](ts-universal-attributes-expand-safe-area.md#expandsafearea)属性支持组件在不改变布局情况下扩展其绘制区域至安全区外。
* 通过设置[setKeyboardAvoidMode](ts-universal-attributes-expand-safe-area.md#setkeyboardavoidmode11)来配置虚拟键盘弹出时页面的避让模式。
* 通过[ignoreLayoutSafeArea](ts-universal-attributes-expand-safe-area.md#ignorelayoutsafearea20)属性扩展组件布局时的安全区域，组件布局位置及大小可能改变。
...
* 滚动类容器内的组件不建议设置expandSafeArea属性。如果设置，需要按照组件嵌套关系，将当前节点到滚动类祖先容器间的所有直接节点设置expandSafeArea属性，否则expandSafeArea属性在滚动后可能会失效。写法参考[示例7](ts-universal-attributes-expand-safe-area.md#示例7滚动类容器扩展安全区)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| types | Array <[SafeAreaType](ts-universal-attributes-expand-safe-area.md#safeareatype)> | 否 | 配置扩展安全区域的类型。默认值包含SafeAreaType.CUTOUT，但未添加[Metadata](js-apis-bundlemanager-metadata.md)配置项时，页面不避让挖孔，CUTOUT类型不生效。  默认值：[SafeAreaType.SYSTEM, SafeAreaType.CUTOUT, SafeAreaType.KEYBOARD]  非法值：按默认值处理。 |
| edges | Array<[SafeAreaEdge](ts-universal-attributes-expand-safe-area.md#safeareaedge)> | 否 | 配置扩展安全区域的边缘，默认扩展至所有避让区域。  默认值：[SafeAreaEdge.TOP, SafeAreaEdge.BOTTOM, SafeAreaEdge.START, SafeAreaEdge.END]。  非法值：按默认值处理。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [KeyboardAvoidMode](arkts-apis-uicontext-e.md#keyboardavoidmode11) | 是 | 虚拟键盘抬起时页面的避让模式。可选值：OFFSET（上抬）、RESIZE（压缩）、NONE（不避让）。  默认值：KeyboardAvoidMode.OFFSET，键盘抬起时默认避让模式为上抬。  setKeyboardAvoidMode传入异常值时，该属性设置不生效。 |
...
在性能敏感/高负载场景下，键盘和应用界面的动效可能不完全同步，导致动效过程中出现键盘与应用界面间露出间隙的现象，此时可以通过配置[expandSafeArea](ts-universal-attributes-expand-safe-area.md#expandsafearea)的方式使应用背景铺满屏幕，避免不良视觉效果。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| types | Array<[LayoutSafeAreaType](ts-universal-attributes-expand-safe-area.md#layoutsafeareatype12)> | 否 | 扩展布局安全区域的类型。  默认值：[LayoutSafeAreaType.SYSTEM]，扩展至系统安全区域，例如：状态栏、导航栏、挖孔区和组件级安全区（[safeAreaPadding](ts-universal-attributes-size.md#safeareapadding14)）。  非法值：按默认值处理。 |
| edges | Array<[LayoutSafeAreaEdge](ts-universal-attributes-expand-safe-area.md#layoutsafeareaedge12)> | 否 | 扩展布局安全区域的边缘，并且支持镜像能力。  默认值：[LayoutSafeAreaEdge.ALL]，扩展组件所有边缘。  非法值：按默认值处理。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/b6/v3/q080aS6eSmOfVAZyL3i1sg/zh-cn_image_0000002712245878.png)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/f8/v3/Fn4qcdckTB-VdfubKvFWyA/zh-cn_image_0000002742004827.png)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/9c/v3/y46PEYDJQdOLwveRUHWP2w/zh-cn_image_0000002712405838.gif)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/f7/v3/Fd9qLV3CTaqxbPgvpb6NEQ/zh-cn_image_0000002742124787.gif)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ea/v3/m86DNjjfTY61MNPjfKscNw/zh-cn_image_0000002712245880.gif)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/44/v3/y7-GmKF0Tt-vRv2_Hhd-PQ/zh-cn_image_0000002742004829.gif)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/fe/v3/nYJZ4wkySfOYf2Y4E42UeA/zh-cn_image_0000002712405840.png)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/6f/v3/ZS2lHiKBS8CLJSmjk1sznQ/zh-cn_image_0000002742124789.jpg)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/89/v3/gVi6HdDtR_Spf3dOSLU2Nw/zh-cn_image_0000002712245882.jpg)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/39/v3/P-RUiyZmQP6_3t3IWd47zA/zh-cn_image_0000002742004831.jpg)
