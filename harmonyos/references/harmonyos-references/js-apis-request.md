...
scraped_at: 2026-09-15T07:07:05+08:00
...
**下载任务错误码**：下载[on('fail')](js-apis-request.md#onfail7)事件callback的错误参数、[getTaskInfo](js-apis-request.md#gettaskinfo9)返回值的failedReason字段取值。

**下载任务暂停原因**：下载相关[getTaskInfo](js-apis-request.md#gettaskinfo9)返回值的pausedReason字段取值。

**下载任务状态码**：下载相关[getTaskInfo](js-apis-request.md#gettaskinfo9)返回值的status字段取值。
...
| config | [UploadConfig](js-apis-request.md#uploadconfig) | 是 | 上传的配置信息。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| Promise<[UploadTask](js-apis-request.md#uploadtask)> | 使用Promise方式，异步返回上传任务UploadTask的Promise对象。 |
...
| context | [BaseContext](js-apis-inner-application-basecontext.md) | 是 | 基于应用程序的上下文。 |
| config | [UploadConfig](js-apis-request.md#uploadconfig) | 是 | 上传的配置信息。 |
| callback | AsyncCallback<[UploadTask](js-apis-request.md#uploadtask)> | 是 | 回调函数，异步返回UploadTask对象。当上传成功，err为undefined，data为获取到的UploadTask对象；否则为错误对象。 |
...
| callback | Callback<Array<[TaskState](js-apis-request.md#taskstate9)>> | 是 | 上传任务完成或失败的回调函数。返回上传任务的任务状态信息。 |
...
| callback | Callback<Array<[TaskState](js-apis-request.md#taskstate9)>> | 否 | 需要取消订阅的回调函数。若无此参数，则取消订阅当前类型的所有回调函数。 |
...
| --- | --- | --- | --- |
| context | [BaseContext](js-apis-inner-application-basecontext.md) | 是 | 基于应用程序的上下文。 |
| config | [DownloadConfig](js-apis-request.md#downloadconfig) | 是 | 下载的配置信息。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| Promise<[DownloadTask](js-apis-request.md#downloadtask)> | 使用Promise方式，异步返回下载任务DownloadTask的Promise对象。 |
...
| context | [BaseContext](js-apis-inner-application-basecontext.md) | 是 | 基于应用程序的上下文。 |
| config | [DownloadConfig](js-apis-request.md#downloadconfig) | 是 | 下载的配置信息。 |
| callback | AsyncCallback<[DownloadTask](js-apis-request.md#downloadtask)> | 是 | 回调函数。当下载任务成功，err为undefined，data为获取到的DownloadTask对象；否则为错误对象。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback<[DownloadInfo](js-apis-request.md#downloadinfo7)> | 是 | 回调函数。当查询下载任务操作成功，err为undefined，data为获取到的DownloadInfo对象；否则为错误对象。 |
...
暂停下载正在运行中的任务，已暂停的任务可被[restore](js-apis-request.md#restore9)恢复，使用Promise异步回调。
...
暂停下载正在运行中的任务，已暂停的任务可被[restore](js-apis-request.md#restore9)恢复，使用callback异步回调。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback<[DownloadInfo](js-apis-request.md#downloadinfo7)> | 是 | 回调函数。当查询下载任务成功，err为undefined，data为获取到的DownloadInfo对象；否则为错误对象。 |
...
| 名称 | 类型 | 值 | 说明 |
| --- | --- | --- | --- |
| VISIBILITY\_COMPLETION21+ | number | 1 | [通知栏](js-apis-request.md#requestagentnotification15)展示类型：显示完成通知 |
...
| value | string | [FileSpec](js-apis-request.md#requestagentfilespec10) | Array<[FileSpec](js-apis-request.md#requestagentfilespec10)> | 否 | 否 | 表单参数值。 |
...
| 名称 | 值 | 说明 |
| --- | --- | --- |
| INITIALIZED | 0x00 | 表示通过配置信息（[Config](js-apis-request.md#requestagentconfig10)）创建的任务已初始化。 |
...
| state | [State](js-apis-request.md#requestagentstate10) | 否 | 是 | 指定任务的状态。如果未填写，则查询所有任务。 |
| action | [Action](js-apis-request.md#requestagentaction10) | 否 | 是 | 任务操作选项。  - UPLOAD表示上传任务。  - DOWNLOAD表示下载任务。  - 如果未填写，则查询所有任务。 |
| mode | [Mode](js-apis-request.md#requestagentmode10) | 否 | 是 | 任务模式。  - FOREGROUND表示前台任务。  - BACKGROUND表示后台任务。  - 如果未填写，则查询所有任务。 |
...
| config | [Config](js-apis-request.md#requestagentconfig10) | 否 | 否 | 任务的配置信息。 |
...
| callback | Callback<[HttpResponse](js-apis-request.md#requestagenthttpresponse12)> | 是 | 回调函数，发生相关的事件时触发该回调方法，返回任务响应头的数据结构。 |
...
| callback | Callback<[Faults](js-apis-request.md#requestagentfaults10)> | 是 | 发生相关的事件时触发该回调方法，返回任务失败的原因。 |
...
| callback | Callback<[WaitingReason](js-apis-request.md#requestagentwaitingreason20)> | 是 | 发生相关的事件时触发该回调方法，返回任务等待的原因。 |
...
| callback | Callback<[HttpResponse](js-apis-request.md#requestagenthttpresponse12)> | 否 | 需要取消订阅的回调函数。若无此参数，则取消订阅当前类型的所有回调函数。 |
...
| --- | --- | --- | --- |
| event | string | 是 | 订阅的事件类型。  - 取值为'faultOccur'，表示任务失败。 |
| callback | Callback<[Faults](js-apis-request.md#requestagentfaults10)> | 否 | 需要取消订阅的回调函数。若无此参数，则默认取消订阅当前类型的所有回调函数。 |
...
| --- | --- | --- | --- |
| event | string | 是 | 订阅的事件类型。  - 取值为'wait'，表示任务等待。 |
| callback | Callback<[WaitingReason](js-apis-request.md#requestagentwaitingreason20)> | 否 | 需要取消订阅的回调函数。若无此参数，则默认取消订阅当前类型的所有回调函数。 |
...
暂停任务，可以暂停正在等待/正在运行/正在重试的任务，已暂停的任务可被[resume](js-apis-request.md#resume10)恢复。使用callback异步回调。
...
暂停任务，可以暂停正在等待/正在运行/正在重试的任务，已暂停的任务可被[resume](js-apis-request.md#resume10)恢复。使用Promise异步回调。
...
停止任务，可以停止正在运行/正在等待/正在重试的任务，已停止的任务可被[start](js-apis-request.md#start10)恢复。使用callback异步回调。
...
停止任务，可以停止正在运行/正在等待/正在重试的任务，已停止的任务可被[start](js-apis-request.md#start10)恢复。使用Promise异步回调。
...
| --- | --- | --- | --- |
| context | [BaseContext](js-apis-inner-application-basecontext.md) | 是 | 基于应用程序的上下文。 |
| config | [Config](js-apis-request.md#requestagentconfig10) | 是 | 上传/下载任务的配置信息。 |
| callback | AsyncCallback<[Task](js-apis-request.md#requestagenttask10)> | 是 | 回调函数。当创建上传或下载任务成功，err为undefined，data为获取到的Task对象；否则为错误对象。 |
...
| --- | --- | --- | --- |
| id | string | 是 | 任务id。 |
| callback | AsyncCallback<[TaskInfo](js-apis-request.md#requestagenttaskinfo10)> | 是 | 回调函数。当查询任务操作成功，err为undefined，data为查询到的任务TaskInfo信息；否则为错误对象。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<[TaskInfo](js-apis-request.md#requestagenttaskinfo10)> | Promise对象。返回任务详细信息TaskInfo的Promise对象。 |
...
根据默认[Filter](js-apis-request.md#requestagentfilter10)过滤条件查找任务id，即查询调用时刻至24小时前的所有任务的任务id。使用callback异步回调。
...
根据[Filter](js-apis-request.md#requestagentfilter10)过滤条件查找任务id。使用callback异步回调。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| filter | [Filter](js-apis-request.md#requestagentfilter10) | 是 | 过滤条件。 |
| callback | AsyncCallback<Array<string>> | 是 | 回调函数。当根据过滤条件查找任务成功，err为undefined，data为满足条件的任务id；否则为错误对象。 |
...
根据[Filter](js-apis-request.md#requestagentfilter10)过滤条件查找任务id。使用Promise异步回调。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| filter | [Filter](js-apis-request.md#requestagentfilter10) | 否 | 过滤条件。默认值为空。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| config | [GroupConfig15+](js-apis-request.md#requestagentgroupconfig15) | 是 | 下载任务分组选项。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| gid | string | 是 | 目标分组id。与创建的任务分组ID保持一致，即使用[request.agent.createGroup](js-apis-request.md#requestagentcreategroup15)接口成功创建任务分组时的返回值。 |
