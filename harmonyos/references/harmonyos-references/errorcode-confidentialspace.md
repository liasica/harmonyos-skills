---
url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/errorcode-confidentialspace
title: ArkTS API错误码
...
1. [runApp](confidentialspace-confidentialspace.md#confidentialspacerunapp)方法抛出此错误，表示传入的数据应用路径不合法，或者找不到该数据应用文件。
2. [sendData](confidentialspace-confidentialspace.md#senddata)方法抛出此错误，表示该句柄不合法。

**可能原因**

1. [runApp](confidentialspace-confidentialspace.md#confidentialspacerunapp)方法抛出此错误，可能是：

   1. 数据应用文件未包含在HAP包中。
   2. 该接口调用参数中的文件路径不正确（当前本接口仅支持从应用沙箱目录中加载文件）。
2. [sendData](confidentialspace-confidentialspace.md#senddata)方法抛出此错误，可能是已经调用过[stop](confidentialspace-confidentialspace.md#stop)方法，导致该句柄已被回收。
...
[runApp](confidentialspace-confidentialspace.md#confidentialspacerunapp)接口调用次数已达到限制，该接口的调用规格限制请参见[在机密空间中运行数据应用 - 约束与限制](../harmonyos-guides/confidentialspace-calling.md#约束与限制)。
...
1. [runApp](confidentialspace-confidentialspace.md#confidentialspacerunapp)方法抛出此错误，表明应用与机密空间管理服务之间的通信失败。
2. [sendData](confidentialspace-confidentialspace.md#senddata)方法抛出此错误，表明由于应用与数据管理服务间通信的socket发生异常，导致通信无法进行。
...
1. [sendData](confidentialspace-confidentialspace.md#senddata)方法抛出此错误，表示传入的数据过长。
2. [onReceiveDataError](confidentialspace-confidentialspace.md#onreceivedataerror)方法注册的回调接收到此类型错误，表示数据应用侧发送的消息过长，无法被应用接收。
...
数据应用产生的原始错误码包含在[DataAppErrorInfo](confidentialspace-confidentialspace.md#dataapperrorinfo)的dataAppErrorCode属性中。开发者可以在[onReceiveDataError](confidentialspace-confidentialspace.md#onreceivedataerror)注册的回调函数中捕获此类错误并获取数据应用原始错误码。
...
检查[runApp](confidentialspace-confidentialspace.md#confidentialspacerunapp)调用逻辑，不要同时运行两个数据应用会话。
...
数据应用中必须包含g\_da\_entry结构体，并且handler字段不为空。
