...
1. 若使用预置振动效果（VibratePreset），建议先调用[vibrator.isSupportEffect](js-apis-vibrator.md#vibratorissupporteffect10)或[vibrator.isSupportEffectSync](js-apis-vibrator.md#vibratorissupporteffectsync12)查询当前设备是否支持该效果；若使用自定义振动配置文件（VibrateFromFile），建议先确认设备支持自定义振动模式（可通过[vibrator.isHdHapticSupported](js-apis-vibrator.md#vibratorishdhapticsupported12)查询是否支持高清振动）；若使用自定义振动模式（VibrateFromPattern），需先通过[VibratorPatternBuilder](js-apis-vibrator.md#vibratorpatternbuilder18)构建振动序列。
2. 调用[vibrator.startVibration](js-apis-vibrator.md#vibratorstartvibration9)启动振动，需同时指定振动效果（VibrateEffect）和振动属性（VibrateAttribute）。振动属性中的usage参数决定了振动的场景类型，不同场景类型受系统振动开关管控规则不同。

停止振动流程：

* 停止指定时长振动或预置效果振动：调用[vibrator.stopVibration](js-apis-vibrator.md#vibratorstopvibration9)（API version 9），传入对应的VibratorStopMode。
* 停止自定义振动（VibrateFromFile或VibrateFromPattern）：调用[vibrator.stopVibration](js-apis-vibrator.md#vibratorstopvibration10-1)（API version 10+，无参数版本）停止所有模式振动。
* 停止所有模式振动：调用[vibrator.stopVibration](js-apis-vibrator.md#vibratorstopvibration10-1)（无参数版本）或[vibrator.stopVibrationSync](js-apis-vibrator.md#vibratorstopvibrationsync12)（同步版本）。
* 停止指定设备的马达振动：调用[vibrator.stopVibration](js-apis-vibrator.md#vibratorstopvibration19)（API version 19+，传入VibratorInfoParam）。

多马达设备场景：

从API version 19开始，支持多设备多马达场景。可通过[vibrator.getVibratorInfoSync](js-apis-vibrator.md#vibratorgetvibratorinfosync19)查询马达信息，通过[vibrator.on](js-apis-vibrator.md#vibratoron19)监听马达上下线事件，以便动态选择合适的马达触发振动。
...
| attribute | [VibrateAttribute](js-apis-vibrator.md#vibrateattribute9) | 是 | 马达振动属性，用于指定马达ID、设备ID和振动使用场景。attribute中的usage参数决定振动的场景类型，不同场景类型受系统振动开关管控规则不同，开发者需根据实际业务场景选择合适的usage值。 |
...
stopMode需与启动振动时的VibrateEffect类型对应：VIBRATOR\_STOP\_MODE\_TIME用于停止VibrateTime类型振动，VIBRATOR\_STOP\_MODE\_PRESET用于停止VibratePreset类型振动，否则停止操作可能无效。调用成功后指定模式振动停止，若无对应振动正在进行也会成功返回。此接口无法停止自定义振动（VibrateFromFile和VibrateFromPattern），如需停止自定义振动或所有模式振动，请使用vibrator.stopVibration (#vibratorstopvibration10)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| stopMode | [VibratorStopMode](js-apis-vibrator.md#vibratorstopmode) | 是 | 指定的停止振动模式，用于停止对应模式的马达振动，支持两种：  VIBRATOR\_STOP\_MODE\_TIME：停止[VibrateTime](js-apis-vibrator.md#vibratetime9)类型的固定时长振动；  VIBRATOR\_STOP\_MODE\_PRESET：停止[VibratePreset](js-apis-vibrator.md#vibratepreset9)类型的预置振动。  此接口无法停止自定义振动（[VibrateFromFile](js-apis-vibrator.md#vibratefromfile10)和[VibrateFromPattern](js-apis-vibrator.md#vibratefrompattern18)），请使用[vibrator.stopVibration10+](js-apis-vibrator.md#vibratorstopvibration10)。stopMode需与启动振动时的VibrateEffect类型对应，否则停止操作可能无效。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| stopMode | [VibratorStopMode](js-apis-vibrator.md#vibratorstopmode) | 是 | 指定的停止振动模式，用于停止对应模式的马达振动，支持两种：  VIBRATOR\_STOP\_MODE\_TIME：停止[VibrateTime](js-apis-vibrator.md#vibratetime9)类型的指定时长振动；  VIBRATOR\_STOP\_MODE\_PRESET：停止[VibratePreset](js-apis-vibrator.md#vibratepreset9)类型的预置振动。  此接口无法停止自定义振动（[VibrateFromFile](js-apis-vibrator.md#vibratefromfile10)和[VibrateFromPattern](js-apis-vibrator.md#vibratefrompattern18)），请使用[vibrator.stopVibration10+](js-apis-vibrator.md#vibratorstopvibration10-1)。stopMode需与启动振动时的VibrateEffect类型对应，否则停止操作可能无效。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| param | [VibratorInfoParam](js-apis-vibrator.md#vibratorinfoparam19) | 否 | 指出需要停止振动的设备和马达信息。不传参时默认停止本地设备的全部马达振动。deviceId默认值为-1（本地设备），vibratorId默认值为0（该设备的全部马达）。deviceId和vibratorId可通过[vibrator.getVibratorInfoSync](js-apis-vibrator.md#vibratorgetvibratorinfosync19)或[vibrator.on](js-apis-vibrator.md#vibratoron19)查询获取。 |
...
当开发者需要立即停止所有振动且不关心异步回调结果时使用此接口。适用于对实时性要求极高的场景（如紧急中断振动）。调用成功后，设备上所有正在进行的马达振动立即停止。调用失败时会抛出异常，需使用try catch捕获。与异步版本[vibrator.stopVibration](js-apis-vibrator.md#vibratorstopvibration10-1)相比，本接口为同步接口，直接返回结果无需回调，但会阻塞主线程。建议在非UI线程中使用，或在UI线程中优先使用异步版本以避免影响交互响应。
...
当开发者需要在触发预置振动前确认当前设备是否支持指定的振动效果时使用此接口。由于不同设备可能预置不同的振动效果，建议在使用[vibrator.startVibration](js-apis-vibrator.md#vibratorstartvibration9)的VibratePreset类型前先调用此接口查询，避免在不支持的设备上触发振动效果不佳。调用成功后，通过callback返回boolean结果：返回true表示设备支持该effectId，可直接用于startVibration；返回false表示不支持，此时使用该effectId触发振动可能效果不佳或无法振动。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| effectId | string | 是 | 待确认的预置振动效果ID。字符串最大长度64，超出部分截取前64个字符。使用场景：不同设备预置的振动效果可能不同，需传入具体的effectId查询是否支持。取值可参考[EffectId](js-apis-vibrator.md#effectid)和[HapticFeedback](js-apis-vibrator.md#hapticfeedback12)中定义的值。 |
| callback | AsyncCallback<boolean> | 是 | 回调函数。返回true表示设备支持该effectId，可用于[startVibration](js-apis-vibrator.md#vibratorstartvibration9)；返回false表示不支持，使用该effectId触发振动可能效果不佳。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<boolean> | Promise对象。返回true表示设备支持该effectId，可用于[startVibration](js-apis-vibrator.md#vibratorstartvibration9-1)；返回false表示不支持，使用该effectId触发振动可能效果不佳。 |
...
当开发者需要在触发预置振动前立即确认当前设备是否支持指定的振动效果时使用此接口。适用于对实时性要求高且查询逻辑简单的场景。返回boolean结果：返回true表示设备支持该effectId，可用于[startVibration](js-apis-vibrator.md#vibratorstartvibration9)；返回false表示不支持，使用该effectId触发振动可能效果不佳或无法振动。与异步版本[vibrator.isSupportEffect](js-apis-vibrator.md#vibratorissupporteffect10)相比，本接口为同步接口，直接返回结果无需回调，但会阻塞主线程。建议在非UI线程中使用，或在UI线程中优先使用异步版本以避免影响交互响应。
...
| 类型 | 说明 |
| --- | --- |
| boolean | 返回对象。返回true表示设备支持该effectId，可用于[startVibration](js-apis-vibrator.md#vibratorstartvibration9)；返回false表示不支持，使用该effectId触发振动可能效果不佳或无法振动。 |
...
| --- | --- | --- | --- |
| effectId | string | 是 | 待确认的预置振动效果ID。字符串最大长度64，超出部分截取前64个字符。使用场景：不同设备预置的振动效果可能不同，需传入具体的effectId查询是否支持。取值可参考[EffectId](js-apis-vibrator.md#effectid)和[HapticFeedback](js-apis-vibrator.md#hapticfeedback12)中定义的值。 |
| param | [VibratorInfoParam](js-apis-vibrator.md#vibratorinfoparam19) | 否 | 指出需要查询的设备和马达信息。不传param时默认查询本地设备。deviceId默认值为-1（本地设备），vibratorId默认值为0（该设备的全部马达）。deviceId和vibratorId可通过[vibrator.getVibratorInfoSync](js-apis-vibrator.md#vibratorgetvibratorinfosync19)或[vibrator.on](js-apis-vibrator.md#vibratoron19)查询获取。 |
...
| 类型 | 说明 |
| --- | --- |
| [EffectInfo](js-apis-vibrator.md#effectinfo19) | 预置振动效果信息。isEffectSupported为true表示支持该效果，可用于[startVibration](js-apis-vibrator.md#vibratorstartvibration9)；为false表示不支持，使用该effectId触发振动可能效果不佳。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| param | [VibratorInfoParam](js-apis-vibrator.md#vibratorinfoparam19) | 否 | 指出需要查询的设备和马达信息。不传param时默认查询所有设备所有马达的信息。deviceId默认值为-1（查询全部设备），vibratorId默认值为0（查询该设备的全部马达）。 |

**返回值**：

| 类型 | 说明 |
| --- | --- |
| Array<[VibratorInfo](js-apis-vibrator.md#vibratorinfo19)> | 马达设备的信息数组。每个元素包含deviceId、vibratorId、deviceName、isHdHapticSupported、isLocalVibrator等属性，可用于选择合适的马达触发振动或判断设备振动能力。 |
...
| callback | Callback<[VibratorStatusEvent](js-apis-vibrator.md#vibratorstatusevent19)> | 是 | 回调函数。当马达设备上线或下线时触发，回调参数为VibratorStatusEvent对象，包含timestamp、deviceId、vibratorCount、isVibratorOnline等信息。回调中获取的deviceId可用于[startVibration](js-apis-vibrator.md#vibratorstartvibration9)和[stopVibration](js-apis-vibrator.md#vibratorstopvibration19)等接口指定目标设备。 |
...
当开发者不再需要监听马达上下线状态变化时使用此接口注销回调。传入callback时注销指定回调；不传callback时注销该类型下所有已注册的回调。注销成功后，不再触发对应的回调函数。若传入的callback未注册过，注销操作无效但不会报错。需先通过[vibrator.on](js-apis-vibrator.md#vibratoron19)注册回调后才能注销。同一type重复注册同一callback不会覆盖，需先off再on。
...
| --- | --- | --- | --- |
| type | string | 是 | 监听类型，该值固定为vibratorStateChange，表示马达上下线状态变化事件。 |
| callback | Callback<[VibratorStatusEvent](js-apis-vibrator.md#vibratorstatusevent19)> | 否 | 回调函数，需要注销的回调函数。不传此参数时注销所有vibratorStateChange类型的回调。使用场景：若仅需注销特定回调则传入对应callback；若需注销全部回调则不传此参数。 |
...
振动设备上线、下线状态事件信息。当马达设备上线或下线时，通过[vibrator.on](js-apis-vibrator.md#vibratoron19)回调传递此对象。
...
| deviceId | number | 否 | 否 | 设备的ID。可用于[startVibration](js-apis-vibrator.md#vibratorstartvibration9)和[stopVibration](js-apis-vibrator.md#vibratorstopvibration19)等接口指定目标设备。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| deviceId | number | 否 | 是 | 设备的ID。默认值：-1，表示本地设备。使用场景：在多设备场景下需指定远程设备时设置此参数；不填此参数时默认控制本地设备。从API version 19开始，设备ID可通过[getVibratorInfoSync](js-apis-vibrator.md#vibratorgetvibratorinfosync19)或[on](js-apis-vibrator.md#vibratoron19)查询获取。 |
| vibratorId | number | 否 | 是 | 马达ID。默认值：0，表示该设备的全部马达。使用场景：在多马达设备上需指定特定马达时设置此参数；不填此参数时默认控制该设备的全部马达。从API version 19开始，马达ID可通过[getVibratorInfoSync](js-apis-vibrator.md#vibratorgetvibratorinfosync19)或[on](js-apis-vibrator.md#vibratoron19)查询获取。 |

## EffectInfo19+

查询的预置效果信息。通过[vibrator.getEffectInfoSync](js-apis-vibrator.md#vibratorgeteffectinfosync19)返回此对象，用于判断预置振动效果是否受指定设备的指定马达支持。

**系统能力**：SystemCapability.Sensors.MiscDevice

| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| isEffectSupported | boolean | 否 | 否 | 预置效果是否受支持。true表示支持该预置振动效果，可用于[startVibration](js-apis-vibrator.md#vibratorstartvibration9)；false表示不支持，使用该effectId触发振动可能效果不佳。 |

## VibratorInfo19+

表示查询的马达信息。通过[vibrator.getVibratorInfoSync](js-apis-vibrator.md#vibratorgetvibratorinfosync19)返回此对象，用于获取设备马达能力和选择合适的马达触发振动。

**系统能力**：SystemCapability.Sensors.MiscDevice

| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| deviceId | number | 否 | 否 | 设备ID。可用于[startVibration](js-apis-vibrator.md#vibratorstartvibration9)和[stopVibration](js-apis-vibrator.md#vibratorstopvibration19)等接口指定目标设备。 |
| vibratorId | number | 否 | 否 | 马达ID。可用于[startVibration](js-apis-vibrator.md#vibratorstartvibration9)和[stopVibration](js-apis-vibrator.md#vibratorstopvibration19)等接口指定目标马达。 |
...
提供添加长振、短振事件和生成VibratorPattern对象的方法。使用流程：先通过[addContinuousEvent](js-apis-vibrator.md#addcontinuousevent18)或[addTransientEvent](js-apis-vibrator.md#addtransientevent18)添加振动事件，再通过[build](js-apis-vibrator.md#build18)方法生成VibratorPattern对象，最后将该对象作为[VibrateFromPattern](js-apis-vibrator.md#vibratefrompattern18)的pattern参数传入[vibrator.startVibration](js-apis-vibrator.md#vibratorstartvibration9)接口触发振动。
...
| options | [ContinuousParam](js-apis-vibrator.md#continuousparam18) | 否 | 可选参数，用于指定长振事件的振动强度、频率、振动调节曲线和通道编号。不填时使用各参数的默认值（intensity默认100，frequency默认50，index默认0）。 |

**返回值**：

| 类型 | 说明 |
| --- | --- |
| VibratorPatternBuilder | 返回已添加连续振动事件的VibratorPatternBuilder对象。可用于继续链式调用addContinuousEvent或addTransientEvent添加更多振动事件，最终通过[build](js-apis-vibrator.md#build18)生成VibratorPattern对象。 |
...
添加短振事件的方法，添加后使用[build](js-apis-vibrator.md#build18)方法生成[VibratorPattern](js-apis-vibrator.md#vibratorpattern18)对象。适用于点击、按键等短促振动反馈场景，返回VibratorPatternBuilder对象，支持链式调用继续添加振动事件。
...
| options | [TransientParam](js-apis-vibrator.md#transientparam18) | 否 | 可选参数，用于指定短振事件的振动强度、频率和通道编号。不填时使用各参数的默认值（intensity默认100，frequency默认50，index默认0）。 |

**返回值**：

| 类型 | 说明 |
| --- | --- |
| VibratorPatternBuilder | 返回已添加短振事件的VibratorPatternBuilder对象。可用于继续链式调用addContinuousEvent或addTransientEvent添加更多振动事件，最终通过[build](js-apis-vibrator.md#build18)生成VibratorPattern对象。 |
...
适用于需要将自定义振动事件组合为振动序列后，通过[VibrateFromPattern](js-apis-vibrator.md#vibratefrompattern18)触发马达振动的场景。需先通过[addContinuousEvent](js-apis-vibrator.md#addcontinuousevent18)或[addTransientEvent](js-apis-vibrator.md#addtransientevent18)添加振动事件后，再调用本方法生成VibratorPattern对象。返回VibratorPattern对象，包含振动序列的起始时间和振动事件数组。该对象可作为VibrateFromPattern的pattern参数传入[startVibration](js-apis-vibrator.md#vibratorstartvibration9)接口触发振动。需先通过[addContinuousEvent](js-apis-vibrator.md#addcontinuousevent18)或[addTransientEvent](js-apis-vibrator.md#addtransientevent18)添加至少一个振动事件后调用本方法，否则生成的VibratorPattern为空序列。
...
| 类型 | 说明 |
| --- | --- |
| [VibratorPattern](js-apis-vibrator.md#vibratorpattern18) | 振动序列对象。包含振动序列的起始时间和振动事件数组，可作为[VibrateFromPattern](js-apis-vibrator.md#vibratefrompattern18)的pattern参数传入[startVibration](js-apis-vibrator.md#vibratorstartvibration9)接口触发振动。 |
...
由于设备存在多样性，不同的设备可能预置不同的效果，建议使用预置效果前先使用[vibrator.isSupportEffect](js-apis-vibrator.md#vibratorissupporteffect10)10+或[vibrator.isSupportEffectSync](js-apis-vibrator.md#vibratorissupporteffectsync12)接口查询当前设备是否支持该预置效果。
...
由于设备存在多样性，建议使用预置效果前先使用[vibrator.isSupportEffect](js-apis-vibrator.md#vibratorissupporteffect10)10+或[vibrator.isSupportEffectSync](js-apis-vibrator.md#vibratorissupporteffectsync12)接口查询当前设备是否支持该预置效果。

**系统能力**：SystemCapability.Sensors.MiscDevice

| 名称 | 值 | 说明 |
| --- | --- | --- |
| EFFECT\_SOFT | 'haptic.effect.soft' | 较松散的振动效果，频率偏低。适用于轻柔触觉反馈场景。 |
| EFFECT\_HARD | 'haptic.effect.hard' | 较沉重的振动效果，频率居中。适用于坚定触觉反馈场景。 |
| EFFECT\_SHARP | 'haptic.effect.sharp' | 较尖锐的振动效果，频率偏高。适用于警示触觉反馈场景。 |
| EFFECT\_NOTICE\_SUCCESS18+ | 'haptic.notice.success' | 表达成功通知的振动效果。适用于操作成功提醒场景。 |
| EFFECT\_NOTICE\_FAILURE18+ | 'haptic.notice.fail' | 表达失败通知的振动效果。适用于操作失败提醒场景。 |
| EFFECT\_NOTICE\_WARNING18+ | 'haptic.notice.warning' | 表达警告通知的振动效果。适用于风险警告提醒场景。 |

## VibratorStopMode

停止振动的模式。在调用[vibrator.stopVibration9+](js-apis-vibrator.md#vibratorstopvibration9)或[vibrator.stopVibration9+](js-apis-vibrator.md#vibratorstopvibration9-1)接口时，需要使用此参数类型指定停止的振动模式。停止模式和[VibrateEffect9+](js-apis-vibrator.md#vibrateeffect9)中下发的模式为对应关系：VIBRATOR\_STOP\_MODE\_TIME对应VibrateTime类型，VIBRATOR\_STOP\_MODE\_PRESET对应VibratePreset类型。

**系统能力**：SystemCapability.Sensors.MiscDevice

| 名称 | 值 | 说明 |
| --- | --- | --- |
| VIBRATOR\_STOP\_MODE\_TIME | 'time' | 停止[VibrateTime](js-apis-vibrator.md#vibratetime9)类型（duration模式）的振动。需与startVibration时使用的VibrateTime类型对应。 |
| VIBRATOR\_STOP\_MODE\_PRESET | 'preset' | 停止[VibratePreset](js-apis-vibrator.md#vibratepreset9)类型（预置EffectId模式）的振动。需与startVibration时使用的VibratePreset类型对应。 |
...
| 类型 | 说明 |
| --- | --- |
| [VibrateTime](js-apis-vibrator.md#vibratetime9) | 按照指定时长触发马达振动。适用于仅需控制振动时长的基础场景。  **元服务API：** 从API version 11开始，该接口支持在元服务中使用。 |
| [VibratePreset](js-apis-vibrator.md#vibratepreset9) | 按照预置振动类型触发马达振动。适用于交互反馈类的短振场景，推荐使用以确保与系统整体振感反馈体验风格一致。 |
| [VibrateFromFile](js-apis-vibrator.md#vibratefromfile10) | 按照自定义振动配置文件触发马达振动。适用于需要精细振动控制的复杂场景。 |
| [VibrateFromPattern18+](js-apis-vibrator.md#vibratefrompattern18) | 按照自定义振动效果触发马达振动。适用于需要灵活组合振动事件的场景。 |
...
预置振动类型。当调用[vibrator.startVibration9+](js-apis-vibrator.md#vibratorstartvibration9)或[vibrator.startVibration9+](js-apis-vibrator.md#vibratorstartvibration9-1)时，[VibrateEffect9+](js-apis-vibrator.md#vibrateeffect9)参数的值可以为VibratePreset，表示触发预置振动类型。适用于交互反馈类的短振场景（如点击、长按、滑动、拖拽等），为确保与系统整体振感反馈体验风格一致，推荐使用此类型。
...
| effectId | string | 否 | 否 | 预置的振动效果ID。字符串最大长度64，超出部分截取前64个字符。使用场景：不同设备预置的振动效果可能不同，建议先通过[vibrator.isSupportEffect](js-apis-vibrator.md#vibratorissupporteffect10)或[vibrator.isSupportEffectSync](js-apis-vibrator.md#vibratorissupporteffectsync12)查询是否支持。取值可参考[EffectId](js-apis-vibrator.md#effectid)和[HapticFeedback](js-apis-vibrator.md#hapticfeedback12)中定义的值。 |
...
自定义振动类型。仅部分设备支持高清振动的设备可用，当设备不支持此振动类型时，返回错误码801。当调用[vibrator.startVibration9+](js-apis-vibrator.md#vibratorstartvibration9)或[vibrator.startVibration9+](js-apis-vibrator.md#vibratorstartvibration9-1)时，[VibrateEffect9+](js-apis-vibrator.md#vibrateeffect9)参数的值可以为VibrateFromFile，表示触发自定义振动类型。适用于匹配复杂场景效果的交互反馈（如表情包触发的拟真效果、游戏场景/操作反馈）。

适用于需要按照振动配置文件定制精细振动效果的交互反馈场景。建议先通过[vibrator.isHdHapticSupported](js-apis-vibrator.md#vibratorishdhapticsupported12)确认设备是否支持高清振动。
...
| hapticFd | [HapticFileDescriptor](js-apis-vibrator.md#hapticfiledescriptor10)10+ | 否 | 否 | 振动配置文件的描述符。需确保文件可用且格式正确，振动序列存储格式请参考[振动效果说明](../harmonyos-guides/vibrator-guidelines.md#振动效果说明)。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| fd | number | 否 | 否 | 资源文件描述符。可通过[fileIo.open](js-apis-file-fs.md#fileioopen)从沙箱路径获取或通过[getRawFd](js-apis-resource-manager.md#getrawfd9)从HAP资源获取。 |
...
振动事件类型。用于[VibratorEvent](js-apis-vibrator.md#vibratorevent18)的eventType字段指定振动事件的类型。
...
相对事件振动强度的增益。用于[ContinuousParam](js-apis-vibrator.md#continuousparam18)和[VibratorEvent](js-apis-vibrator.md#vibratorevent18)的points字段，精细控制振动强度和频率的变化趋势。
...
振动事件。用于[VibratorPattern](js-apis-vibrator.md#vibratorpattern18)的events数组中定义具体的振动事件。
...
| points | Array<[VibratorCurvePoint](js-apis-vibrator.md#vibratorcurvepoint18)> | 否 | 是 | 可选参数，表示振动调节曲线数组。使用场景：适用于需要精细控制振动强度和频率变化趋势的交互反馈场景。数组中元素个数最少设置4个，最大设置16个。 |

## VibratorPattern18+

马达振动序列，每个events代表一个振动事件。通过[VibratorPatternBuilder.build](js-apis-vibrator.md#build18)方法生成，作为[VibrateFromPattern](js-apis-vibrator.md#vibratefrompattern18)的pattern参数传入[startVibration](js-apis-vibrator.md#vibratorstartvibration9)接口触发振动。
...
| events | Array<[VibratorEvent](js-apis-vibrator.md#vibratorevent18)> | 否 | 否 | 振动事件数组。由[VibratorPatternBuilder](js-apis-vibrator.md#vibratorpatternbuilder18)的addContinuousEvent和addTransientEvent方法添加后通过build方法生成。同一VibratorPattern中多个VibratorEvent的time值不能重叠。 |

## ContinuousParam18+

连续振动参数。用于[VibratorPatternBuilder.addContinuousEvent](js-apis-vibrator.md#addcontinuousevent18)的options参数，指定长振事件的振动强度、频率、振动调节曲线和通道编号。
...
| points | [VibratorCurvePoint](js-apis-vibrator.md#vibratorcurvepoint18)[] | 否 | 是 | 可选参数，表示振动调节曲线数组。使用场景：适用于需要精细控制振动强度和频率变化趋势的交互反馈场景。数组中元素个数最少设置4个，最大设置16个。 |
| index | number | 否 | 是 | 可选参数，表示通道编号。取值范围：[0,2]区间内所有整数。默认值：0。使用场景：不同通道对应不同的马达器件，适用于多马达设备的精细控制场景。不填写时默认使用通道0。 |

## TransientParam18+

瞬态振动参数。用于[VibratorPatternBuilder.addTransientEvent](js-apis-vibrator.md#addtransientevent18)的options参数，指定短振事件的振动强度、频率和通道编号。
...
| pattern | VibratorPattern | 否 | 否 | [build](js-apis-vibrator.md#build18)方法返回的[VibratorPattern](js-apis-vibrator.md#vibratorpattern18)对象。需先通过[VibratorPatternBuilder](js-apis-vibrator.md#vibratorpatternbuilder18)添加振动事件后调用build方法生成。 |

## VibrateAttribute9+

马达振动属性。用于[startVibration](js-apis-vibrator.md#vibratorstartvibration9)接口的attribute参数，指定马达ID、设备ID和振动使用场景。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| id | number | 否 | 是 | 马达ID。默认值：0。使用场景：当设备存在多个马达时，可通过指定不同的马达ID来选择特定马达触发振动。马达ID可以通过[getVibratorInfoSync](js-apis-vibrator.md#vibratorgetvibratorinfosync19)查询。不填写时默认使用马达ID为0的马达。 |
| deviceId19+ | number | 否 | 是 | 设备ID。默认值：-1，表示本地设备。使用场景：在多设备场景下需指定远程设备时设置此参数；不填写时默认控制本地设备。从API version 19开始，设备ID可以使用[getVibratorInfoSync](js-apis-vibrator.md#vibratorgetvibratorinfosync19)或[on](js-apis-vibrator.md#vibratoron19)查询。  **元服务API**：从API version 19开始，该接口支持在元服务中使用。 |
| usage | [Usage](js-apis-vibrator.md#usage9) | 否 | 否 | 马达振动的使用场景。默认值：'unknown'。取值范围只允许在[Usage](js-apis-vibrator.md#usage9)提供的类型中选取。不同usage值对应不同的系统振动开关管控规则，开发者需根据实际业务场景选择合适的usage值。  **元服务API**：从API version 11开始，该接口支持在元服务中使用。 |
...
从API version 8 开始支持，从API version 9 开始废弃，建议使用 [vibrator.startVibration](js-apis-vibrator.md#vibratorstartvibration9-1)替代。
...
从API version 8 开始支持，从API version 9 开始废弃，建议使用 [vibrator.startVibration](js-apis-vibrator.md#vibratorstartvibration9)替代。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| effectId | [EffectId](js-apis-vibrator.md#effectid) | 是 | 预置的振动效果ID。字符串最大长度64，超出部分截取前64个字符。建议先通过[vibrator.isSupportEffect](js-apis-vibrator.md#vibratorissupporteffect10)或[vibrator.isSupportEffectSync](js-apis-vibrator.md#vibratorissupporteffectsync12)查询是否支持。 |
...
从API version 8 开始支持，从API version 9 开始废弃，建议使用 [vibrator.stopVibration](js-apis-vibrator.md#vibratorstopvibration9-1)替代。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| stopMode | [VibratorStopMode](js-apis-vibrator.md#vibratorstopmode) | 是 | 马达停止指定的振动模式。需与启动振动时的模式对应：VIBRATOR\_STOP\_MODE\_TIME用于停止时长振动，VIBRATOR\_STOP\_MODE\_PRESET用于停止预置效果振动。 |
...
从API version 8 开始支持，从API version 9 开始废弃，建议使用 [vibrator.stopVibration](js-apis-vibrator.md#vibratorstopvibration9)替代。
