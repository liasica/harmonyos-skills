...
content_hash: sha256:16562a7b5c211de0eb28376b40ed33603d4f7c14e93e8b238f4254eea97a7ef5
---
...
* [XmlSerializer](js-apis-xml.md#xmlserializer)：适用于已知XML文本大小的情况。需要开发者自行创建ArrayBuffer作为缓存区域，需确保缓存区域足以容纳生成的文本内容。
* [XmlDynamicSerializer20+](js-apis-xml.md#xmldynamicserializer20)：适用于未知XML文本大小的情况。无需自行创建ArrayBuffer，程序动态扩容，但序列化结果字符串长度上限为100000。
...
该接口必须在[startElement](js-apis-xml.md#startelement)之后调用，用于为当前已开启的元素设置属性。在元素开始标记写入之前调用此接口将产生无效XML。
...
该接口应在[startElement](js-apis-xml.md#startelement)之前调用，为即将开启的元素设置命名空间前缀。调用顺序：先调用setNamespace设置命名空间，再调用startElement开启元素。
...
该接口必须在[startElement20+](js-apis-xml.md#startelement20)之后调用，用于为当前已开启的元素设置属性。
...
* 调用该接口后须调用[endElement20+](js-apis-xml.md#endelement20)写入元素结束标记，以确保节点正确闭合。
* 该接口对所添加数据不做标准XML校验处理，请确保所添加的数据符合标准XML规范。例如不允许添加数字开头的元素名称。
...
调用该接口前必须先调用[startElement20+](js-apis-xml.md#startelement20)接口写入元素开始标记。
...
该接口应在[startElement20+](js-apis-xml.md#startelement20)之前调用，为即将开启的元素设置命名空间前缀。调用顺序：先调用setNamespace设置命名空间，再调用startElement开启元素。
...
该接口必须在[startElement20+](js-apis-xml.md#startelement20)之后、[endElement20+](js-apis-xml.md#endelement20)之前调用，用于设置当前元素的文本内容。
...
| tokenValueCallbackFunction | (eventType: [EventType](js-apis-xml.md#eventtype), value: [ParseInfo](js-apis-xml.md#parseinfo)) => boolean | 否 | 是 | 解析元素事件类型([EventType](js-apis-xml.md#eventtype))和[ParseInfo](js-apis-xml.md#parseinfo)属性，默认值为undefined，表示不解析。  **元服务API**：从API version 11开始，该接口支持在元服务中使用。 |
...
事件类型枚举，定义了XmlPullParser在解析XML过程中可能触发的各类事件。解析时事件按START\_DOCUMENT→START\_TAG→TEXT/CDSECT→END\_TAG→END\_DOCUMENT等顺序依次触发，开发者可通过tokenValueCallbackFunction回调接收对应事件。
...
* inputStream参数必须传入继承自[Readable](js-apis-stream.md#readable)且实现[doRead](js-apis-stream.md#doread)的类。可以传入其他模块中满足该条件的类，如[ReadStream](js-apis-file-fs.md#readstream12)。
...
* 可以配合自动控制数据的流使用，如[ReadStream](js-apis-file-fs.md#readstream12)，此时用户不再需要手动控制数据。
...
当解析器遇到XML元素的开始标签时触发的回调函数。该回调函数需要开发者自行实现。具体使用示例可见[characters24+](js-apis-xml.md#characters24)。
...
当解析器遇到XML元素的结束标签时触发的回调函数。该回调函数需要开发者自行实现。具体使用示例可见[characters24+](js-apis-xml.md#characters24)。
