...
content_hash: sha256:87713b3be7c07d5b2a55f8071cbf221526c1b50462c29a866e08163d44a53e53
---
...
组件区域变化时触发该回调，可通过[AreaChangeOptions](ts-universal-component-area-change-event.md#areachangeoptions)中的expectedUpdateInterval设置触发回调的间隔。仅会响应由布局变化所导致的组件大小、位置发生变化时的回调。由绘制变化所导致的渲染属性变化不会响应回调，如[translate](ts-universal-attributes-transformation.md#translate)、[offset](ts-universal-attributes-location.md#offset)、[markAnchor](ts-universal-attributes-location.md#markanchor)、[scale](ts-universal-attributes-transformation.md#scale)、[transform](ts-universal-attributes-transformation.md#transform)。若组件自身位置由绘制变化决定也不会响应回调，如[bindSheet](ts-universal-attributes-sheet-transition.md#bindsheet)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| event | [AreaChangeCallback](ts-universal-component-area-change-event.md#areachangecallback) | 是 | onAreaChange事件的回调函数。组件显示的尺寸、位置发生变化时触发该回调。 |
| options | [AreaChangeOptions](ts-universal-component-area-change-event.md#areachangeoptions) | 否 | 区域变化相关的配置参数，用于设置区域变化回调的计算时间间隔。可通过expectedUpdateInterval设置回调触发间隔，单位为ms；未传入options时，expectedUpdateInterval按照0处理。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/74/v3/laBUSIHZTjyve_tKjjnp9w/zh-cn_image_0000002742004803.gif)

### 示例2（使用onAreaChange自定义间隔监听区域变化）

该示例通过设置[expectedUpdateInterval](ts-universal-component-area-change-event.md#areachangeoptions)，当Text布局变化时可以触发[onAreaChange](ts-universal-component-area-change-event.md#onareachange-1)事件，达到间隔回调的效果。

从API版本26.0.0开始，新增[onAreaChange](ts-universal-component-area-change-event.md#onareachange-1)、[AreaChangeCallback](ts-universal-component-area-change-event.md#areachangecallback)和[AreaChangeOptions](ts-universal-component-area-change-event.md#areachangeoptions)。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/a3/v3/MezTzSKhRcSd4Jn2DsxMLg/zh-cn_image_0000002712405814.gif)
