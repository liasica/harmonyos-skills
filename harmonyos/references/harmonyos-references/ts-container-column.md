...
content_hash: sha256:c3d13bcf619115c2b8aabfb9f914f0a32d1c0dc905813be9cadbd28a07bbed22
---
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options18+ | [ColumnOptions](ts-container-column.md#columnoptions18对象说明) | 否 | Column组件的间距配置选项。通过space属性设置纵向布局元素垂直方向间距。当需要为子组件设置固定垂直间距时传入此参数；省略时不设置子组件间距。  **模型约束：** 此接口仅可在Stage模型下使用。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | [ColumnOptions](ts-container-column.md#columnoptions18对象说明) | [ColumnOptionsV2](ts-container-column.md#columnoptionsv218对象说明) | 否 | Column组件的间距配置选项。通过space属性设置纵向布局元素垂直方向间距，space支持设置number、string或Resource类型。当需要为子组件设置固定垂直间距时传入此参数；省略时不设置子组件间距。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| space7+ | string | number | 否 | 是 | 纵向布局元素垂直方向间距。  space为负数或者[justifyContent](ts-container-column.md#justifycontent8)设置为FlexAlign.SpaceBetween、FlexAlign.SpaceAround、FlexAlign.SpaceEvenly时，space不生效。  取值范围：[0, +∞)  默认值：0  非法值：按默认值处理。  单位：vp  **说明：**  space取值是大于等于0的数字，或者可以转换为非负数字的字符串，不支持设置百分比。  **卡片能力：** 从API version 9开始，该接口支持在ArkTS卡片中使用。  **元服务API：** 从API version 11开始，该接口支持在元服务中使用。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| space | [SpaceType](ts-container-column.md#spacetype18) | 否 | 是 | 设置纵向布局元素垂直方向间距。  space为负数或者[justifyContent](ts-container-column.md#justifycontent8)设置为FlexAlign.SpaceBetween、FlexAlign.SpaceAround、FlexAlign.SpaceEvenly时，space不生效。  取值范围：[0, +∞)  默认值：0  单位：vp  非法值：按默认值处理。  **说明：**  space取值是大于等于0的数字，或者可以转换为非负数字的字符串，或者可以转换为数字的Resource类型数据，不支持设置百分比。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [FlexAlign](ts-appendix-enums.md#flexalign) | 是 | 子组件在垂直方向上的对齐格式。  默认值：FlexAlign.Start  **说明：** 若子组件不设置[flexShrink](ts-universal-attributes-flex-layout.md#flexshrink)，FlexAlign.Center和FlexAlign.End可能失效，详见下方说明。设置为FlexAlign.SpaceBetween、FlexAlign.SpaceAround、FlexAlign.SpaceEvenly时，[space](ts-container-column.md#columnoptions18对象说明)属性不生效。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/03/v3/lJ_af6EqT5iTDz4I7eSlhQ/zh-cn_image_0000002706835740.png)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/36/v3/WRtXcYeRT0KP3s4cLheiCg/zh-cn_image_0000002736314845.png)
