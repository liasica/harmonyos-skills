...
* [@InsightIntentLink](js-apis-app-ability-insightintentdecorator.md#insightintentlink)装饰器：使用该装饰器装饰当前应用的URI，可将该URI定义为意图，便于AI入口跳转到当前应用。该装饰器支持的参数参见[LinkIntentDecoratorInfo](js-apis-app-ability-insightintentdecorator.md#linkintentdecoratorinfo)。
...
* [@InsightIntentFunction](js-apis-app-ability-insightintentdecorator.md#insightintentfunction)装饰器与[@InsightIntentFunctionMethod](js-apis-app-ability-insightintentdecorator.md#insightintentfunctionmethod)装饰器：两者必须组合使用。使用[@InsightIntentFunction](js-apis-app-ability-insightintentdecorator.md#insightintentfunction)装饰类，同时使用[@InsightIntentFunctionMethod](js-apis-app-ability-insightintentdecorator.md#insightintentfunctionmethod)装饰类中的静态函数，即可将静态函数定义为意图，便于AI入口快速执行该函数。
...
URI格式需要符合[应用链接说明](../harmonyos-guides/app-uri-config.md)中的要求。
...
| paramCategory | [LinkParamCategory](js-apis-app-ability-insightintentdecorator.md#linkparamcategory) | 否 | 是 | 表示意图参数类别。若取值为[LINK](js-apis-app-ability-insightintentdecorator.md#linkparamcategory)，系统获取paramName对应的映射名称，并以键值对形式拼接到URI末尾。若取值为[WANT](js-apis-app-ability-insightintentdecorator.md#linkparamcategory)，系统获取paramName对应的映射名称及其取值，通过[Want](js-apis-app-ability-want.md)的parameters字段传递。 |
...
| navigationId | string | 否 | 是 | 表示与意图绑定的[Navigation](ts-basic-components-navigation.md)组件的id属性。 |
| navDestinationName | string | 否 | 是 | 表示与意图绑定的[navDestination](ts-basic-components-navigation.md#navdestination10)组件的名称。 |
