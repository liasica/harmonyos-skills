---
url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/push-api-service-noti
title: 服务通知
...
| toOpenId | 是 | String | 接收者（用户）账号登录的openID。使用从端侧上报的openID，或请求华为账号服务器[获取用户信息](account-api-get-user-info-overview.md)。可参考元服务[账号使用规范](../design-guides/accounts-0000001967444380.md)进行华为账号登录的设计和接入。 |
...
| templateParams | 是 | [TemplateParams](push-api-service-noti.md#templateparams) Object | 模板参数，形如{"key\_0": "value","key\_1": "value","key\_2": "value"}的对象。单个参数的长度限制见TemplateParams，模板副标题的总长度不超过1024个字节。参数值不可为空。 |
| clickAction | 否 | [ClickAction](push-api-service-noti.md#clickaction) Object | 点击事件。不传时默认跳转首页。 |

### **TemplateParams**

模板参数与您在服务通知领用模板时勾选的参数有关，请通过服务通知页面确认您的模板中有哪些参数。详情见[选用订阅模板](../atomic-guides/push-as-service-noti.md#section880418143379)。
...
| phone\_number | String | 代表电话号码。数字，符号，电话号码，如：+86-025-85697456。值不可为空。 |
| car\_number | String | 代表车牌号。8位以内，第一位与最后一位可为汉字，其余为字母或数字，例如车牌号码：粤A8Z888挂。值不可为空。 |
...
请根据响应消息中的提示，检查并修改请求[参数内容](push-api-service-noti.md#请求体结构说明)。
...
请按照响应消息中的提示，检查并修改携带的[模板参数](push-api-service-noti.md#request-body)。
...
请求携带的模板ID不正确，查询不到对应模板信息，请参考[选用订阅模板](../atomic-guides/push-as-service-noti.md#section880418143379)查看模板详情，并修改模板ID。
...
1. 请检查url中的项目ID是否正确。
2. 检查appId是否在黑名单中。
3. 请开通对应元服务的[服务通知](../atomic-guides/push-as-service-noti.md#section798882510516)。
...
1. 下发超过订阅次数，请[发起基于账号的订阅请求](../atomic-guides/push-as-request-sub.md)增加订阅次数。
2. 请求消息体超过默认大小，请减小请求体大小并重新下发消息。
3. 用户未订阅消息，请[发起基于账号的订阅请求](../atomic-guides/push-as-request-sub.md)。
4. 元服务未开通推送服务，请登录[AppGallery Connect](https://developer.huawei.com/consumer/cn/service/josp/agc/index.html)网站[开通推送服务](../atomic-guides/push-as-prepare.md#section73941874447)。
...
请按照响应消息中的提示，使用从端侧上报的openID，或请求华为账号服务器[获取用户信息](account-api-get-user-info-overview.md)中的openID，重新下发消息。
...
请按照响应消息中的提示，检查并修改[模板参数](push-api-service-noti.md#request-body)，详情请参见[选用订阅模板](../atomic-guides/push-as-service-noti.md#section880418143379)。
