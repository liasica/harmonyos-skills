...
content_hash: sha256:0ffebcd10266ee7d43b7c5325fd73db5f7dc1f165647ce8e3b5f71a560464bd3
---
...
MultiNavigation组件遵循默认的左起右清栈规则：从主页触发详情页加载时，会清除栈中已有的所有详情页，确保仅展示最新加载的详情页。然而，若在右侧的详情页上再次执行详情页加载操作，系统将不会执行清栈动作。效果可参见[示例](ohos-arkui-advanced-multinavigation.md#示例)。

**说明** 

* 从主页（HOME\_PAGE）点击加载详情页时：栈中已有的右侧详情页全部出栈，新详情页入栈，确保右侧仅展示最新加载的详情页。
* 从详情页（DETAIL\_PAGE）再次点击加载详情页时：不清栈，新详情页直接入栈，原有详情页保留。
* 从全屏页（FULL\_PAGE）加载详情页时：不影响已有详情页栈，新详情页入栈。
...
| 名称 | 类型 | 必填 | 装饰器类型 | 说明 |
| --- | --- | --- | --- | --- |
| multiStack | [MultiNavPathStack](ohos-arkui-advanced-multinavigation.md#multinavpathstack) | 是 | [@State](../harmonyos-guides/arkts-state.md) | 设置路由栈。 |
| navDestination | [NavDestinationBuildFunction](ohos-arkui-advanced-multinavigation.md#navdestinationbuildfunction) | 是 | [@BuilderParam](../harmonyos-guides/arkts-builderparam.md) | 设置加载目标页面的路由规则。 |
| onNavigationModeChange | [OnNavigationModeChangeCallback](ohos-arkui-advanced-multinavigation.md#onnavigationmodechangecallback) | 否 | - | 设置MultiNavigation模式变更时的回调。当需要在导航模式变化时执行特定业务逻辑（如调整页面布局、更新UI状态等）时传入此回调。不传入时不监听导航模式变更事件，导航模式变更时无回调触发。 |
| onHomeShowOnTop | [OnHomeShowOnTopCallback](ohos-arkui-advanced-multinavigation.md#onhomeshowontopcallback) | 否 | - | 设置主页处于栈顶时的回调。不传入时不监听主页栈顶状态变化。 |
...
| policy | [SplitPolicy](ohos-arkui-advanced-multinavigation.md#splitpolicy) | 否 | 当前入栈页面的策略。  默认值：DETAIL\_PAGE |
...
| name | string | 是 | NavDestination页面名称。需要与NavDestinationBuildFunction中注册的页面名称一致。 |
| param | Object | 是 | NavDestination页面详细参数，用于向目标页面传递自定义数据。具体字段规格请参考NavDestination相关文档。 |
| onPop | base.[Callback](js-apis-base.md#callback)<[PopInfo](ts-basic-components-navigation.md#popinfo11)> | 否 | Callback回调，用于页面出栈时触发该回调处理返回结果。省略时不触发回调处理。可通过pop方法、popToName方法、popToIndex方法的result参数传递数据给此回调。 |
| animated | boolean | 否 | 是否支持转场动画。  默认值：true  true：支持转场动画。  false：不支持转场动画。 |
| policy | [SplitPolicy](ohos-arkui-advanced-multinavigation.md#splitpolicy) | 否 | 当前入栈页面的策略。  默认值：DETAIL\_PAGE |
...
| 名称 | 值 | 说明 |
| --- | --- | --- |
| HOME\_PAGE | 0 | 主页页面类型。全屏模式显示。适用于应用主页，作为导航起始页面。 |
| DETAIL\_PAGE | 1 | 详情页页面类型。分栏模式显示。适用于详情页面，在大屏设备上与主页形成左右分栏布局。 |
| FULL\_PAGE | 2 | 全屏页页面类型。全屏模式显示。适用于视频播放、图片浏览等需要全屏展示的页面。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/b3/v3/h704If96RkClUQlJ0timEg/zh-cn_image_0000002733436122.gif)

主页跳转详情页效果演示：

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ad/v3/1q_fO3I9SeGJdBTmM4l3Og/zh-cn_image_0000002762995643.gif)

全屏类型页面效果演示：

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/b1/v3/wq3e3CCzR-SmkPdovOZpGQ/zh-cn_image_0000002762835759.gif)
