---
url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-busmanager-serial
title: "@ohos.busManager.serial (串口通信管理)"
breadcrumb: API参考 > 系统 > 基础功能 > Basic Services Kit（基础服务） > ArkTS API > 设备管理 > @ohos.busManager.serial (串口通信管理)
category: harmonyos-references
scraped_at: 2026-09-02T15:02:01+08:00
doc_updated_at: 2026-08-29
content_hash: sha256:eb47f72b27620cdb6fe4dad91af1e5c7210eb18f5d9dc37a4296153cccafcdb2
---
...
查询串口设备列表，返回[SerialPort](js-apis-busmanager-serial.md#serialport)对象数组。使用Promise异步回调。用于需要识别可用串口设备的场景，如工业设备连接、物联网设备管理、嵌入式系统调试等场景。
...
| 类型 | 说明 |
| --- | --- |
| Promise<[SerialPort](js-apis-busmanager-serial.md#serialport)[]> | Promise对象，返回串口设备列表。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| portInfo | [SerialPortInfo](js-apis-busmanager-serial.md#serialportinfo) | 是 | 否 | 串口设备信息。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| config | [SerialConfigs](js-apis-busmanager-serial.md#serialconfigs) | 否 | 串口通信参数。不传入config参数时，使用SerialConfigs的默认配置打开串口。 |
...
监听串口接收数据事件。使用callback异步回调，返回接收到的数据。需在串口打开后调用，调用[close](js-apis-busmanager-serial.md#close)后，所有回调将被清除。用于实时接收串口设备发送的数据，如传感器数据监测、设备状态反馈、实时数据采集等场景。
...
| dataBits | [DataBits](js-apis-busmanager-serial.md#databits) | 否 | 是 | 数据位。默认值：EIGHT（8数据位，标准通信）。FIVE/SIX/SEVEN用于老旧设备或特殊协议。 |
| stopBits | [StopBits](js-apis-busmanager-serial.md#stopbits) | 否 | 是 | 停止位。默认值：ONE。1个停止位用于标准通信场景；2个停止位用于低速通信或与老旧设备通信时增加信号稳定性。 |
| parity | [Parity](js-apis-busmanager-serial.md#parity) | 否 | 是 | 校验位。默认值：NONE（无校验）。EVEN/ODD用于数据准确性要求高的场景；MARK/SPACE用于特殊通信协议。 |
