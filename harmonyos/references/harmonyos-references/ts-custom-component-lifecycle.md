...
content_hash: sha256:52a09dacade786a49238135926e7d7a9e291e396e05f6e439dcaf3c7608dec19
---
...
aboutToAppear函数在创建自定义组件的新实例后，在其build()函数执行前调用。允许在aboutToAppear函数中改变[状态变量](../harmonyos-guides/arkts-state-management-glossary.md#state-variables状态变量)，更改将在后续执行build()函数中生效。实现[自定义布局](ts-custom-component-layout.md)的自定义组件的aboutToAppear生命周期在布局过程中触发。具体使用说明，详见[自定义组件生命周期指南](../harmonyos-guides/arkts-page-custom-components-lifecycle.md)。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/73/v3/3zjHFI8rQhqZcOijDOoeDg/zh-cn_image_0000002706836258.gif)
...
* [避免对@Link/@ObjectLink/@Prop等自动更新的状态变量，在aboutToReuse()中重复赋值](../harmonyos-guides/arkts-component_reuse.md#避免对linkobjectlinkprop等自动更新的状态变量在abouttoreuse中重复赋值)。
...
组件的生命周期回调，在可复用组件从节点树上被加入到复用缓存之前调用。当该组件后续从复用缓存中被重新复用时，将触发[aboutToReuse](ts-custom-component-lifecycle.md#abouttoreuse10)生命周期回调。在频繁调用场景下，应避免在其中执行耗时操作，否则可能导致丢帧卡顿。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/91/v3/p5mOh4nrRsGBwzgJI2aCZg/zh-cn_image_0000002736315363.png)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/10/v3/0I6V10yhRHGYAkd7QfQStA/zh-cn_image_0000002706676324.png)
