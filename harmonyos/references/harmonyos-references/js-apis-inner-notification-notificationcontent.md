...
content_hash: sha256:966fa62e3e1960fbbf539bd6bafe201702621c863333b50510bcaaea11ec85ce
---
...
| notificationContentType11+ | [notificationManager.ContentType](js-apis-notificationmanager.md#contenttype) | 否 | 是 | 通知内容类型。 |
| normal | [NotificationBasicContent](js-apis-inner-notification-notificationcontent.md#notificationbasiccontent) | 否 | 是 | 基本类型通知内容。 |
| longText | [NotificationLongTextContent](js-apis-inner-notification-notificationcontent.md#notificationlongtextcontent) | 否 | 是 | 长文本类型通知内容。 |
| multiLine | [NotificationMultiLineContent](js-apis-inner-notification-notificationcontent.md#notificationmultilinecontent) | 否 | 是 | 多行类型通知内容。 |
| picture | [NotificationPictureContent](js-apis-inner-notification-notificationcontent.md#notificationpicturecontent) | 否 | 是 | 图片类型通知内容。 |
| systemLiveView11+ | [NotificationSystemLiveViewContent](js-apis-inner-notification-notificationcontent.md#notificationsystemliveviewcontent) | 否 | 是 | 系统实况窗类型通知内容。不支持三方应用直接创建该类型通知，可以由系统代理创建系统实况窗类型通知后，三方应用发布同ID的通知来更新指定内容。 |
...
* 当该类型通知与其他通知形成组通知时，该通知显示默认与[普通文本](js-apis-inner-notification-notificationcontent.md#notificationbasiccontent)相同。展开组通知后，标题显示为展开时的标题longTitle，多行文本内容lines多行显示。
...
| briefText | string | 否 | 否 | 通知概要内容，是对通知内容的总结（不可为空字符串，大小不超过1024字节，超出部分会被截断）。 |
| expandedTitle | string | 否 | 否 | 通知展开时的标题（不可为空字符串，大小不超过1024字节，超出部分会被截断）。 |
| picture | [image.PixelMap](arkts-apis-image-pixelmap.md) | 否 | 否 | 通知的图片内容（图像像素的总字节数不能超过2MB）。 |
...
| capsule11+ | [NotificationCapsule](js-apis-inner-notification-notificationcontent.md#notificationcapsule11) | 否 | 是 | 实况通知的胶囊。默认为空。 |
...
| icon | [image.PixelMap](arkts-apis-image-pixelmap.md) | 否 | 是 | 胶囊图片。图标像素的总字节数不超过192KB（图标像素的总字节数通过[getPixelBytesNumber](arkts-apis-image-pixelmap.md#getpixelbytesnumber7)获取），建议图标像素长宽为128\*128。实际显示效果依赖于设备能力和通知中心UI样式。 |
...
| icons | Array<[image.PixelMap](arkts-apis-image-pixelmap.md)> | 否 | 是 | 按钮图片（最多支持3个）。图标像素的总字节数不超过192KB（图标像素的总字节数通过[getPixelBytesNumber](arkts-apis-image-pixelmap.md#getpixelbytesnumber7)获取），建议图标像素长宽为128\*128。实际显示效果依赖于设备能力和通知中心UI样式。 |
| iconsResource12+ | Array<[Resource](ts-types.md#resource)> | 否 | 是 | 按钮资源（最多支持3个）。 |
