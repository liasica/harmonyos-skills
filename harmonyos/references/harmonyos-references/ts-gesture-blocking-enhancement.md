...
content_hash: sha256:3d5ce422967a8e461a2998520e4860880e1e88468f38bb579e953e4b410c31c8
---
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | [ShouldBuiltInRecognizerParallelWithCallback](ts-gesture-blocking-enhancement.md#shouldbuiltinrecognizerparallelwithcallback) | 是 | 系统内置手势与响应链上其他组件的手势设置并行关系的回调事件，当该组件进行[触摸测试](../harmonyos-guides/arkts-interaction-basic-principles.md#触摸测试)时，会触发用户定义的回调来形成手势并行关系。 |
...
对于不需要暴露ArkUI系统组合组件中内置组件手势的场景，建议采用原有[onGestureRecognizerJudgeBegin](ts-gesture-blocking-enhancement.md#ongesturerecognizerjudgebegin)接口。若需要暴露该手势，建议使用该接口并将exposeInnerGesture设置为true。
...
| exposeInnerGesture | boolean | 是 | 暴露内部手势标识。  默认值：false  **说明：**  如果目标组件是组合组件，将此参数设置为true时，回调中的current参数会包含组合组件内部的手势识别器。  当前仅支持[Tabs](ts-container-tabs.md)，其他组件请不要设置此参数。  设置为false时，功能与原接口[onGestureRecognizerJudgeBegin](ts-gesture-blocking-enhancement.md#ongesturerecognizerjudgebegin)相同。 |
...
| touchRecognizers20+ | Array<[TouchRecognizer](ts-gesture-common.md#touchrecognizer20)> | 否 | 响应链上的Touch识别器对象。未传入时，表示在当前手势绑定组件及其子孙组件没有可响应的Touch识别器。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| event | [BaseGestureEvent](ts-gesture-common.md#basegestureevent11对象说明) | 是 | [触摸测试](../harmonyos-guides/arkts-interaction-basic-principles.md#触摸测试)结束后的基础手势事件的信息。  **说明：**  仅包含BaseGestureEvent的信息，不包含其子类扩展信息。  axisHorizontal和axisVertical的值为0。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | [GestureCollectInterceptCallback](ts-gesture-blocking-enhancement.md#gesturecollectinterceptcallback) | 是 | 组件进行触摸测试时使用的回调函数。在当前节点及更高优先级节点上的事件和手势收集完成后执行，以干预收集结果。 |
...
定义在[onGestureCollectIntercept](ts-gesture-blocking-enhancement.md#ongesturecollectintercept)中使用的回调类型。
...
| touchRecognizers | Array<[TouchRecognizer](ts-gesture-common.md#touchrecognizer20)> | 否 | 响应链上组件的触摸识别器对象。  默认值为null，表示响应链上没有触摸识别器对象。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| [GestureCollectIntervention](ts-appendix-enums.md#gesturecollectintervention) | 手势收集干预结果。  返回值不是GestureCollectIntervention枚举值时，按照GestureCollectIntervention.[CONTINUE](ts-appendix-enums.md#gesturecollectintervention)处理。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | [ShouldRecognizerParallelWithCallback](ts-gesture-blocking-enhancement.md#shouldrecognizerparallelwithcallback) | 是 | 手势与响应链上其他组件的手势设置并行关系的回调事件，当该组件进行[触摸测试](../harmonyos-guides/arkts-interaction-basic-principles.md#触摸测试)时，会触发用户定义的回调来形成手势并行关系。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| current | [GestureRecognizer](ts-gesture-common.md#gesturerecognizer12) | 是 | 当前组件的手势识别器，当前仅支持[GestureType](ts-gesture-common.md#gesturetype11).PAN\_GESTURE类型的手势识别器。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/f0/v3/cHLHkinRRFmj5rmtv_JqYA/zh-cn_image_0000002742124887.gif)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/f5/v3/vG7C8fMqQGS3QZHEJFBp7w/zh-cn_image_0000002712245980.gif)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/3c/v3/EKRCvRL1TFGEY6ANWHQZWg/zh-cn_image_0000002742004929.gif)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/b4/v3/fjZz1O56T9WT8ueQqhAGNw/zh-cn_image_0000002712405940.gif)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/e1/v3/8B-mmXFkRQCNEFXpl3XFxg/zh-cn_image_0000002742124889.gif)

### 示例6（自定义干预事件和手势的收集结果）

该示例通过配置[onGestureCollectIntercept](ts-gesture-blocking-enhancement.md#ongesturecollectintercept)指定手势识别器或者触摸识别器是否透传到其他节点。点击button2时，不透传触摸事件到Column。点击button1时，透传触摸事件到Column，Column变色。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/e3/v3/9wBioSu7QuGAFA8QNz__gA/zh-cn_image_0000002712245982.gif)
...
该示例通过[shouldRecognizerParallelWith](ts-gesture-blocking-enhancement.md#shouldrecognizerparallelwith)和[onGestureRecognizerJudgeBegin](ts-gesture-blocking-enhancement.md#ongesturerecognizerjudgebegin)实现了嵌套滚动的功能。内部组件优先响应滑动手势，当内部组件滑动至顶部或底部时，外部组件能够接替滑动。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/24/v3/a2QtaXmCR4eyJPW298Wc6w/zh-cn_image_0000002742124887.gif)
