...
content_hash: sha256:abb9819dee718ef5d6f224cd6562654629df01489dab7675c22e586ef8131cc4
---
...
| builder | [CustomBuilder](ts-types.md#custombuilder8) | 是 | 配置全屏模态页面内容。builder中的根节点需唯一。  **说明**：在搭载HarmonyOS 7.0.0及以上版本，不管Builder是否有唯一根节点，都可以拉起全模态页面。针对低于该版本的设备，Builder需要有唯一的容器类根节点，如Column，才能拉起全模态页面。 |
| type | [ModalTransition](ts-universal-attributes-sheet-transition.md#modaltransition) | 否 | 全屏模态页面的系统转场方式。  取值原则：DEFAULT-上下切换过渡，NONE-无动画过渡，ALPHA-透明渐变过渡。  默认值：ModalTransition.DEFAULT，即上下切换过渡。  **说明：**  与transition同时设置时，此属性不生效，仅transition生效。 |
...
| options | [ContentCoverOptions](ts-universal-attributes-modal-transition.md#contentcoveroptions) | 否 | 配置全屏模态页面的可选属性。不传入此参数时，各可选属性使用各自默认值。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| modalTransition | [ModalTransition](ts-universal-attributes-sheet-transition.md#modaltransition) | 否 | 是 | 全屏模态页面的系统转场方式。  取值原则：DEFAULT-上下切换过渡，NONE-无动画过渡，ALPHA-透明渐变过渡。  默认值：ModalTransition.DEFAULT。  **说明：**  与transition同时设置时，此属性不生效，仅transition生效。  **元服务API：** 从API version 11开始，该接口支持在元服务中使用。 |
| onWillDismiss12+ | [Callback](ts-types.md#callback12)<[DismissContentCoverAction](ts-universal-attributes-modal-transition.md#dismisscontentcoveraction12类型说明)> | 否 | 是 | 全屏模态页面交互式关闭回调函数。  **说明：**  当用户通过返回键等操作触发关闭全屏模态页面时，如果注册该回调函数，则不会立刻关闭。在回调函数中可以通过reason得到拦截关闭页面的操作类型，从而根据原因选择是否关闭全屏模态页面。如需关闭，须在回调中调用DismissContentCoverAction.dismiss()方法；如不调用dismiss()，全屏模态页面将保持打开状态不会关闭。在onWillDismiss回调中，不能再做onWillDismiss拦截。  **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
| transition12+ | [TransitionEffect](ts-transition-animation-component.md#transitioneffect10对象说明) | 否 | 是 | 全屏模态页面的自定义转场方式。  **说明：**  未设置时默认不使用自定义转场，使用modalTransition的系统转场方式，与modalTransition同时设置时，仅transition生效，modalTransition属性不生效。  **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/97/v3/j7frv023S2eT-zT7M3-ShA/zh-cn_image_0000002757312083.gif)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/74/v3/X8frjW6DRE-TW6PtWuBp-Q/zh-cn_image_0000002757232203.gif)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/a1/v3/uLKOxmi4QWeCKxvRk9N7Eg/zh-cn_image_0000002727592512.gif)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/40/v3/s0FCwlssTTyHvURudC3ivA/zh-cn_image_0000002727752370.gif)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/9b/v3/dCRwgk8pTuCtScT9UFX6LQ/zh-cn_image_0000002757312085.gif)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/53/v3/PZChTiu0RF2Nz69CloWBGw/zh-cn_image_0000002757232205.png)
