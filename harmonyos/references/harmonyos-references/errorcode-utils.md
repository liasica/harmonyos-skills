---
url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/errorcode-utils
title: 语言基础类库错误码
...
确保传输信息属于Worker支持的合法序列化对象，支持的序列化类型详查[ArkTS线程间通信概述](../harmonyos-guides/interthread-communication-overview.md)。
...
确保Worker文件路径符合创建Worker对象的规范，参见API参考中[constructor9+](js-apis-worker.md#constructor9)接口的示例。
...
在调用[executePeriodically](js-apis-taskpool.md#taskpoolexecuteperiodically12)执行周期任务前，该任务已经执行。
...
周期任务再次调用[execute](js-apis-taskpool.md#execute11)、[executeDelayed](js-apis-taskpool.md#taskpoolexecutedelayed11)、[addTask](js-apis-taskpool.md#addtask10-1)和[execute](js-apis-taskpool.md#taskpoolexecute-1)执行。
...
调用了[removeDependency](js-apis-taskpool.md#removedependency11)和[addDependency](js-apis-taskpool.md#adddependency11)接口给周期任务增加或移除依赖关系。
...
调用[asyncRunner.execute](js-apis-taskpool.md#execute18)执行task任务的数量超过等待任务列表容量时，最早入队的task任务就会被丢弃。
...
异步队列任务调用了[removeDependency](js-apis-taskpool.md#removedependency11)或[addDependency](js-apis-taskpool.md#adddependency11)接口来添加或移除依赖关系。
...
异步队列任务只能被[asyncRunner.execute](js-apis-taskpool.md#execute18)接口执行，不能被其他API执行；非异步队列任务，不能被[asyncRunner.execute](js-apis-taskpool.md#execute18)接口执行。

**可能原因**

1. 异步任务再次调用[sequenceRunner.execute](js-apis-taskpool.md#execute11)、[executeDelayed](js-apis-taskpool.md#taskpoolexecutedelayed11)、[addTask](js-apis-taskpool.md#addtask10-1)、[taskpool.execute](js-apis-taskpool.md#taskpoolexecute-1)、[asyncRunner.execute](js-apis-taskpool.md#execute18)、[executePeriodically](js-apis-taskpool.md#taskpoolexecuteperiodically12)或[addDependency](js-apis-taskpool.md#adddependency11)执行。
2. 已经被执行过的任务再次调用异步队列的[execute](js-apis-taskpool.md#execute18)执行。
