...
| identifyPolicies | Array<[Policy](js-apis-identifysensitivecontent.md#policy)> | 是 | 用于识别敏感内容的策略数组。每个Policy定义识别规则（标签、关键字、正则表达式），系统将根据这些规则扫描文件内容并返回匹配结果。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| Promise<Array<[MatchResult](js-apis-identifysensitivecontent.md#matchresult)>> | Promise对象，返回敏感内容识别的结果。成功时返回匹配结果数组，异常返回错误码。 |
