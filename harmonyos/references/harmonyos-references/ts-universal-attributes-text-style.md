---
url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-attributes-text-style
title: 输入框类组件通用接口
...
| 类型 | 说明 |
| --- | --- |
| [CaretOffset](ts-universal-attributes-text-style.md#caretoffset11对象说明) | 光标相对输入框的位置。  当controller未绑定组件或绑定controller的组件被释放时，返回undefined。 |
...
| 类型 | 说明 |
| --- | --- |
| [TextRange](ts-text-common.md#textrange12) | 文本当前的选择范围，未选中返回光标位置。  当controller未绑定组件或绑定controller的组件被释放时，返回undefined。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| range | [TextRange](ts-text-common.md#textrange12) | 否 | 滚动到可视区域的文本范围，包括文本起始位置和终止位置。  起始位置应小于等于结束位置，否则接口调用无效。起始位置小于0视为0，结束位置大于全文长度视为全文长度。  未指定范围时，默认为全部文本。未指定起始位置，默认起始位置为0；未指定结束位置，默认结束位置为全文长度。 |
