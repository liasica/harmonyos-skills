---
url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/errorcode-form
title: 卡片错误码
...
检查当前卡片对应互动卡片配置项[sceneAnimationParams](../harmonyos-guides/arkts-ui-widget-configuration.md#sceneanimationparams标签)是否正确。
...
检查入参卡片尺寸是否在[FormDimension](js-apis-app-form-forminfo.md#formdimension)和[supportDimensions](../harmonyos-guides/arkts-ui-widget-configuration.md#配置文件字段说明)配置列表。
...
4. 空参数错误，比如传入空字符串（''）。
5. 参数格式错误。
6. 参数值错误。输入参数需要与[app.json5](../harmonyos-guides/app-configuration-file.md)和[配置ArkTS卡片的配置文件](../harmonyos-guides/arkts-ui-widget-configuration.md)中对应的配置保持一致。
