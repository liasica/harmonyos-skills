---
url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/payment-agent-prepay
title: 预下单
...
在接入华为支付单次支付时，开发者首先需要调用该接口获取到预支付ID（prepayId），用prepayId构建[orderStr](payment-model.md#orderstr)参数返回给客户端调用支付接口拉起华为支付收银台。
