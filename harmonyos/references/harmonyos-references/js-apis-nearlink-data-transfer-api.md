...
注册端口通道。端口通道注册后方可用于连接远端设备，不再使用时需通过[dataTransfer.destroyPort](js-apis-nearlink-data-transfer-api.md#datatransferdestroyport)销毁。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| params | [ConnectionParams](js-apis-nearlink-data-transfer-api.md#connectionparams) | 是 | 指明端口的连接参数。 |
...
断连远端设备。需在通过[dataTransfer.connect](js-apis-nearlink-data-transfer-api.md#datatransferconnect)成功建立连接后调用，用于断开已建立的远端设备连接。使用Promise异步回调。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback<[ConnectionResult](js-apis-nearlink-data-transfer-api.md#connectionresult)> | 是 | 回调函数，返回与远端设备端口连接参数的协商结果。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback<[ConnectionResult](js-apis-nearlink-data-transfer-api.md#connectionresult)> | 否 | 回调函数，返回与远端设备端口连接参数的协商结果。  填写该参数则取消当前callback订阅。不填写该参数则取消该事件对应的所有回调。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| params | [ConnectionStateParams](js-apis-nearlink-data-transfer-api.md#connectionstateparams) | 是 | 指明端口的连接参数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| params | [DataParams](js-apis-nearlink-data-transfer-api.md#dataparams) | 是 | 指明发送数据的参数，包含远端设备地址、服务UUID以及发送的数据包。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback<[DataParams](js-apis-nearlink-data-transfer-api.md#dataparams)> | 是 | 回调函数，返回端口通道接收到的数据参数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback<[DataParams](js-apis-nearlink-data-transfer-api.md#dataparams)> | 否 | 回调函数，返回端口通道接收到的数据参数。  填写该参数则取消当前callback订阅。不填写该参数则取消该事件对应的所有回调。 |
...
| transferMode | [TransferMode](js-apis-nearlink-data-transfer-api.md#transfermode) | 否 | 是 | 表示和远端设备的数据传输模式。默认值是BASIC。 |
...
| address | string | 否 | 否 | 远端设备的星闪地址。地址格式参考：11:22:33:AA:BB:FF。 |
| uuid | string | 否 | 否 | 星闪服务UUID，长度必须为36个字符，由32个十六进制数字和4个连字符（-）组成，例如： FFFFFFFF-1234-5678-ABCD-000000001234，表示一个128位标识符。 不允许使用星闪标准UUID。 |
| data | ArrayBuffer | 否 | 否 | 数据包。通过[dataTransfer.writeData](js-apis-nearlink-data-transfer-api.md#datatransferwritedata)发送时表示待发送的数据，通过[dataTransfer.onReadData](js-apis-nearlink-data-transfer-api.md#datatransferonreaddata)接收时表示接收到的数据。 |
