...
为保障系统安全性和稳定性，防止VpnExtensionAbility滥用系统资源，系统对其能力进行管控，不支持部分模块的引用，详情请参考[附录](js-apis-net-vpnextension.md#附录)。
...
| 类型 | 说明 |
| --- | --- |
| [VpnObserver](js-apis-net-vpnextension.md#vpnobserver) | 返回一个VPN观察者对象。 |
...
VPN观察者对象。用于监听VPN相关事件。在调用VpnObserver的方法前，需要先通过[vpnExtension.createVpnObserver](js-apis-net-vpnextension.md#vpnextensioncreatevpnobserver)创建VPN连接对象。

### onAuthorizationResult

onAuthorizationResult(callback: Callback<boolean>): void

注册用户授权结果监听器。授权结果在调用[startVpnExtensionAbility](js-apis-net-vpnextension.md#vpnextensionstartvpnextensionability)弹出授权弹窗，用户点击弹窗后通知，仅接收当前VPN的结果。在不需要监听授权结果时可以调用[offAuthorizationResult](js-apis-net-vpnextension.md#offauthorizationresult)接口取消注册。
...
多次调用[onAuthorizationResult](js-apis-net-vpnextension.md#onauthorizationresult)注册监听时，若需取消授权结果监听，需要传最后一次调用时传入的callback，或者不传入参数。
