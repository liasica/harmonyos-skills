---
url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/payment-partner-combined-refund
title: 申请退款
breadcrumb: API参考 > 应用服务 > Payment Kit（鸿蒙支付服务） > REST API > 平台类商户/服务商 > 合单支付（仅支持平台类商户） > 申请退款
category: harmonyos-references
scraped_at: 2026-04-28T08:18:09+08:00
doc_updated_at: 2026-04-20
content_hash: sha256:f33c233ebd5c06a26fdd6b2857a98eb222ba7530c6cda092b558b0129e3b5b3e
---
...
4. 服务商代特约商户退款，需要服务商在[华为支付商户平台](https://petalpay-merchant.cloud.huawei.com/)上[申请API退款授权](../pay-docs/hwzf-apituikuan-0000002371871965.md)。
...
| --- | --- | --- |
| 商户退款订单（mercRefundOrderNo） | 发起退款请求失败，建议根据错误码（400000 RETRY\_TOO\_MANY错误码场景，需要换单重试）排查后，原单重试。 | 1. 400000 RETRY\_TOO\_MANY错误码场景，需要换单重试。  2. 多次部分退款场景下，每次需要换单后发起请求。 |
...
```
1. POST /api/v1/partner/aggr/transactions/refunds HTTP/1.1
...
6. "sign": "MEUCIEhVD6FuZ5iIh41A********************diWp/WVE8SoZOSXWMI0JGRXrj0=",
7. "mercRefundOrderNo": "czl0012024070914***",
...
| 400000 | RETRY\_TOO\_MANY | 重试次数超限，请换单重试 | 使用新的订单号重新请求。 |
| 400000 | ORDER\_CONCURRENT\_ERROR | 订单退款并发错误 | 1. 一笔普通收单多次退款，时间间隔要在1分钟以上。  2. 合单多笔子单退款，时间间隔要在1分钟以上。 |
| 400000 | OPERATION\_NOT\_AUTHORIZED | 操作未授权 | 检查是否已[申请API退款授权](../pay-docs/hwzf-apituikuan-0000002371871965.md)。 |
| 400000 | CUST\_NOT\_EXIST | 用户不存在或已销户 | 检查退款用户。 |
| 400000 | NOT\_IN\_VALIDITY\_PERIOD | 不在有效期内 | 检查退款订单是否已过期。 |
