...
| name | string | 否 | 是 | Worker的名称。  默认值为undefined，此时线程名称为'WorkerThread'。  非默认值情况下，对应的线程名称带有'WorkerThread\_'前缀。比如name为'testName'时，对应的线程名称为'WorkerThread\_testName'。  线程名称可通过[HeapMemoryInfo](js-apis-util.md#heapmemoryinfo24)的threadName获取。  **元服务API**： 从API version 11开始，该接口支持在元服务中使用。 |
...
以下示例展示了在Stage模型的entry模块Index.ets文件中加载Worker线程文件的方法，使用Library加载Worker线程文件的场景参考[文件路径注意事项](../harmonyos-guides/worker-introduction.md#文件路径注意事项)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| message | Object | 是 | 发送至Worker的数据，该数据对象必须是可序列化对象，序列化支持类型见[序列化支持类型](js-apis-worker.md#序列化支持类型)。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| message | Object | 是 | 发送至Worker的数据，该数据对象必须是可序列化对象，序列化支持类型见[序列化支持类型](js-apis-worker.md#序列化支持类型)。如果需要共享数据，支持类型见[Sendable支持的数据类型](../harmonyos-guides/arkts-sendable.md#sendable支持的数据类型)。 |
...
| callback | [WorkerEventListener](js-apis-worker.md#workereventlistener9) | 否 | 需要删除的特定监听器回调函数。如果未传入此参数，则会删除该类型的所有监听器。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| type | string | 是 | 需要删除的监听事件类型。事件类型通过[addEventListener9+](js-apis-worker.md#addeventlistener9-1)设置。 |
| callback | [WorkerEventListener](js-apis-worker.md#workereventlistener9) | 否 | 需要删除的特定监听器回调函数。如果未传入此参数，则会删除该类型的所有监听器。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| messageObject | Object | 是 | 发送至宿主线程的数据，该数据对象必须是可序列化对象，序列化支持类型见[序列化支持类型](js-apis-worker.md#序列化支持类型)。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| message | Object | 是 | 发送至宿主线程的数据，该数据对象必须是可序列化或可共享，序列化支持类型见[序列化支持类型](js-apis-worker.md#序列化支持类型)，共享支持类型见[Sendable支持的数据类型](../harmonyos-guides/arkts-sendable.md#sendable支持的数据类型)。 |
...
Worker线程向宿主线程发送插队消息，消息中的[Sendable对象](../harmonyos-guides/arkts-sendable.md)通过引用传递，非Sendable对象通过拷贝数据的方式传递。
...
| --- | --- | --- | --- |
| message | Object | 是 | 发送至宿主线程的数据，该数据对象必须是可序列化或可共享，序列化支持类型见[序列化支持类型](js-apis-worker.md#序列化支持类型)，共享支持类型见[Sendable支持的数据类型](../harmonyos-guides/arkts-sendable.md#sendable支持的数据类型)。 |
| priority | [Priority](js-apis-worker.md#priority) | 是 | 消息发送的优先级。 |
| transfer | ArrayBuffer[] | 否 | 表示可转移的ArrayBuffer实例对象数组，该数组中对象的所有权会被转移到宿主线程，在Worker线程中将会变为不可用，仅在宿主线程中可用，数组不可传入null。默认值为空数组。 |
...
如果传递的参数是对象字面量的话，需要[显式标注对象字面量的类型](../harmonyos-guides/typescript-to-arkts-migration-guide.md#需要显式标注对象字面量的类型)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| err | [ErrorEvent](js-apis-worker.md#errorevent) | 是 | 错误事件类，表示Worker执行过程中出现的异常信息。 |
...
从API version 7开始支持，从API version 9开始废弃，建议使用[ThreadWorker](js-apis-worker.md#threadworker9)替代。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| onexit(deprecated) | (code: number) => void | 否 | 是 | 回调函数。表示Worker销毁时被调用的事件处理程序，处理程序在宿主线程中执行。其中回调函数中code类型为number，异常退出为1，正常退出为0。默认值为undefined。  **说明**：从API version 7开始支持，从API version 9开始废弃，建议使用[ThreadWorker.onexit](js-apis-worker.md#属性)替代。 |
| onerror(deprecated) | (err: [ErrorEvent](js-apis-worker.md#errorevent)) => void | 否 | 是 | 回调函数。表示Worker在执行过程中发生异常被调用的事件处理程序，处理程序在宿主线程中执行。其中回调函数中err类型为[ErrorEvent](js-apis-worker.md#errorevent)，表示收到的异常数据。默认值为undefined。  **说明**：从API version 7开始支持，从API version 9开始废弃，建议使用[ThreadWorker.onerror](js-apis-worker.md#属性)替代。 |
...
此处以在Stage模型的entry模块Index.ets文件中加载Worker线程文件为例，使用Library加载Worker线程文件的场景参考[文件路径注意事项](../harmonyos-guides/worker-introduction.md#文件路径注意事项)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| message | Object | 是 | 发送至Worker的数据对象必须是可序列化对象，序列化支持类型见[序列化支持类型](js-apis-worker.md#序列化支持类型)。 |
...
| listener | [EventListener](js-apis-worker.md#eventlistenerdeprecated) | 否 | 需要删除的特定监听器回调函数。如果未传入此参数，则会删除该类型的所有监听器。 |
...
| callback | [EventListener](js-apis-worker.md#eventlistenerdeprecated) | 否 | 需要删除的特定监听器回调函数。如果未传入此参数，则会删除该类型的所有监听器。 |
