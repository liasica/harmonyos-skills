...
scraped_at: 2026-04-28T08:01:54+08:00
doc_updated_at: 2026-04-20
content_hash: sha256:868d3a6f6de58dba21d8cd363d09c2c9e4868ab008279d9392ae9d60072bc214
---
...
| color | [ResourceColor](ts-types.md#resourcecolor) | 否 | 是 | 光标颜色。  默认值：'#ff007dff' |
...
2. let layoutManager: LayoutManager = this.controller.getLayoutManager();
```
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| x | number | 是 | 相对于组件的横坐标。  单位：[px](ts-pixel-units.md) |
| y | number | 是 | 相对于组件的纵坐标。  单位：[px](ts-pixel-units.md) |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| [PositionWithAffinity](ts-text-common.md#positionwithaffinity12) | 字形位置信息。 |
...
| 类型 | 说明 |
| --- | --- |
| [LineMetrics](ts-text-common.md#linemetrics12) | 行信息、文本样式信息、以及字体属性信息。  当行号小于0或超出实际行，返回无效值。 |
...
| widthStyle | [RectWidthStyle](ts-text-common.md#rectwidthstyle14) | 是 | 返回的矩形区域的宽度的规格。 |
| heightStyle | [RectHeightStyle](ts-text-common.md#rectheightstyle14) | 是 | 返回的矩形区域的高度的规格。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| Array<[TextBox](ts-text-common.md#textbox14)> | 矩形区域数组。 |
...
| affinity | [Affinity](ts-text-common.md#affinity12) | 否 | 否 | 位置亲和度。 |
...
| phoneNumber20+ | [TextMenuItemId](ts-text-common.md#textmenuitemid12) | 是 | 否 | 呼叫，为一级菜单项。对选中的电话号码跳转服务，拉起电话拨号页面。  **元服务API：** 从API version 20开始，该接口支持在元服务中使用。 |
...
| askAI20+ | [TextMenuItemId](ts-text-common.md#textmenuitemid12) | 是 | 否 | 问问小艺。对选中的文本提供AI问询能力，为一级菜单项。  **元服务API：** 从API version 20开始，该接口支持在元服务中使用。 |
...
| 类型 | 说明 |
| --- | --- |
| [TextMenuItemId](ts-text-common.md#textmenuitemid12) | TextMenuItemId的对象。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| id | [TextMenuItemId](ts-text-common.md#textmenuitemid12) | 是 | TextMenuItemId的id。 |
...
| icon | [ResourceStr](ts-types.md#resourcestr) | 否 | 是 | 菜单图标。  不支持网络图片。  **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
| id | [TextMenuItemId](ts-text-common.md#textmenuitemid12) | 否 | 否 | 菜单id。  **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
| labelInfo15+ | [ResourceStr](ts-types.md#resourcestr) | 否 | 是 | 快捷键提示。  该字段仅2in1设备支持。  **元服务API：** 从API version 15开始，该接口支持在元服务中使用。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| onPrepareMenu20+ | [OnPrepareMenuCallback](ts-text-common.md#onpreparemenucallback20) | 否 | 是 | 当文本选择区域变化后显示菜单之前触发该回调，可在该回调中进行菜单数据设置。  **元服务API：** 从API version 20开始，该接口支持在元服务中使用。 |
...
| color12+ | [ResourceColor](ts-types.md#resourcecolor) | 否 | 是 | 设置文本识别成功后的实体颜色。  默认值：'#ff0a59f7'  **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
...
| enablePreviewMenu20+ | boolean | 否 | 是 | 设置是否开启文本识别长按显示预览菜单。true表示开启，false表示未开启。  默认值：false  当[copyOptions](ts-basic-components-richeditor.md#copyoptions)设置为None时，若enablePreviewMenu设置为true，长按AI实体也不能显示预览菜单。  **设备行为差异：** 该参数在Phone、Tablet中可正常调用，在PC/2in1、TV和Wearable等其他设备类型中无效果。  **元服务API：** 从API version 20开始，该接口支持在元服务中使用。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| onWillChange | Callback<[StyledStringChangeValue](ts-text-common.md#styledstringchangevalue12), boolean> | 否 | 是 | 文本内容将要变化回调函数。 |
| onDidChange | [OnDidChangeCallback](ts-text-common.md#ondidchangecallback12) | 否 | 是 | 文本内容完成变化回调函数。 |
...
| previewText | [StyledString](ts-universal-styled-string.md#styledstring) | 否 | 是 | 预览内容属性字符串。  该属性用于表示语音输入、拍摄输入、输入法预上屏场景下的未提交上屏的临时输入内容。 |
...
| style | [TextDecorationStyle](ts-appendix-enums.md#textdecorationstyle12) | 否 | 是 | 装饰线样式。  **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
...
| 类型 | 说明 |
| --- | --- |
| [LineMetrics](js-apis-graphics-text.md#linemetrics) | 用于描述文本布局中单行文字的度量信息。 |
...
| 类型 | 说明 |
| --- | --- |
| [Affinity](js-apis-graphics-text.md#affinity) | 位置亲和度枚举。 |
...
| 类型 | 说明 |
| --- | --- |
| [TextBox](js-apis-graphics-text.md#textbox) | 文本矩形区域。 |
...
| 类型 | 说明 |
| --- | --- |
| [Paragraph](js-apis-graphics-text.md#paragraph) | 保存文本内容及样式的载体，支持排版与绘制操作。 |
...
| 类型 | 说明 |
| --- | --- |
| [RectHeightStyle](js-apis-graphics-text.md#rectheightstyle) | 矩形区域高度规格枚举。 |
...
| 类型 | 说明 |
| --- | --- |
| [RectWidthStyle](js-apis-graphics-text.md#rectwidthstyle) | 矩形区域宽度规格枚举。 |
...
| previewText | [PreviewText](ts-text-common.md#previewtext12) | 否 | 是 | 预上屏的内容信息。 |
| options | [TextChangeOptions](ts-text-common.md#textchangeoptions15对象说明) | 否 | 是 | 变化的文本内容信息。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | [LinearGradientOptions](ts-universal-attributes-gradient-color.md#lineargradientoptions18对象说明) | 是 | 显示为线性渐变效果。 |
...
| 类型 | 说明 |
| --- | --- |
| [InputMethodExtraConfig](js-apis-inputmethod-extraconfig.md#inputmethodextraconfig) | 输入法扩展信息。 |
