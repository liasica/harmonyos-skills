...
| --- | --- | --- | --- |
| admin | [Want](js-apis-app-ability-want.md) | 是 | 企业设备管理扩展组件。Want中必须包含企业设备管理扩展能力的abilityName和所在应用的bundleName。 |
| profile | [WifiProfile](js-apis-enterprise-wifimanager.md#wifiprofile) | 是 | Wi-Fi配置信息，用于指定要连接的Wi-Fi网络的配置参数，包括SSID、BSSID、密钥、安全类型等。 |
...
本接口通过传入Want查询对应企业设备管理应用设置的策略，如需查询实际生效的策略，请使用[wifiManager.getAllowedWifiList](js-apis-enterprise-wifimanager.md#wifimanagergetallowedwifilist)接口。
...
本接口通过传入Want查询对应企业设备管理应用设置的策略，如需查询实际生效的策略，请使用[wifiManager.getDisallowedWifiList](js-apis-enterprise-wifimanager.md#wifimanagergetdisallowedwifilist)接口。
...
| ipType | [IpType](js-apis-enterprise-wifimanager.md#iptype) | 否 | 是 | IP地址类型，默认值DHCP。 |
...
| WIFI\_SEC\_TYPE\_EAP\_SUITE\_B | 6 | Suite-B 192位加密类型。设置后Wi-Fi将使用Suite-B 192位高强度加密，提供高级别的安全认证，适用于政府和高安全机构。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| eapMethod | [EapMethod](js-apis-enterprise-wifimanager.md#eapmethod) | 否 | 否 | EAP认证方式。 |
