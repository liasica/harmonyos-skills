...
content_hash: sha256:73ccee9d53d1081a956a596d4442842497a568bb5ebc9534b6fad0cc01333c8a
---
...
| --- | --- | --- |
| ALL | 'all' | 查询全量的意图实体。 |
| BY\_PROPERTY | 'byProperty' | 通过[IntentEntityDecoratorInfo](js-apis-app-ability-insightintentdecorator.md#intententitydecoratorinfo)中的supportedQueryProperties属性进行查询。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| queryType | [QueryType](js-apis-app-ability-insightintent.md#querytype) | 否 | 否 | 意图实体查询方式。 |
| parameters | Record<string, Object> | 否 | 是 | 意图实体查询参数。该字段在[QueryType](js-apis-app-ability-insightintent.md#querytype)为[BY\_PROPERTY](js-apis-app-ability-insightintent.md#querytype)时需要填写。 |

## AppIntentEntity

支持查询应用内数据的意图实体定义，继承自[IntentEntity](js-apis-app-ability-insightintent.md#intententity20)。用于定义应用需要开放的信息对象。
...
当系统入口触发意图实体查询时，系统将会加载该类，并触发该回调，开发者可以在该回调中实现需要执行的意图实体操作，根据传入的参数[QueryEntityParam](js-apis-app-ability-insightintent.md#queryentityparam)返回符合条件的实体信息。使用Promise异步回调。

推荐在[queryType](js-apis-app-ability-insightintent.md#querytype)为[ALL](js-apis-app-ability-insightintent.md#querytype)时返回意图实体全部的信息，在[queryType](js-apis-app-ability-insightintent.md#querytype)为[BY\_PROPERTY](js-apis-app-ability-insightintent.md#querytype)时，根据[parameters](js-apis-app-ability-insightintent.md#queryentityparam)中的属性值返回符合条件的信息。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| params | [QueryEntityParam](js-apis-app-ability-insightintent.md#queryentityparam) | 是 | 意图实体查询参数。 |
