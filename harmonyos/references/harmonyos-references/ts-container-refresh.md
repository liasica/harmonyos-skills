...
content_hash: sha256:743250a7f9b1f7bb2b1691c5f53dcdb88783bc56fd6316e8a3ee3988fcc090ed
---
...
* Refresh和内容大小小于组件自身的[List](ts-container-list.md)组件嵌套使用并且中间还有其他组件时，手势可能会被中间组件响应，导致Refresh未产生下拉刷新效果。此时可以将[alwaysEnabled](ts-container-scrollable-common.md#edgeeffectoptions11对象说明)参数设为true，[List](ts-container-list.md)会响应手势并通过嵌套滚动带动Refresh组件产生下拉刷新效果。具体可以参考[示例9（不满一屏场景实现下拉刷新）](ts-container-refresh.md#示例9不满一屏场景实现下拉刷新)。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| refreshing | boolean | 否 | 否 | 组件当前是否处于刷新状态。true表示正在刷新，false表示未在刷新。  默认值：false  该参数支持[$$](../harmonyos-guides/arkts-two-way-sync.md)双向绑定变量。  **元服务API：** 从API version 11开始，该接口支持在元服务中使用。 |
...
| friction(deprecated) | number | string | 否 | 是 | 下拉摩擦系数。  取值范围：[0, 100]，值小于0或大于100时，使用默认值。  默认值：62  - 0表示下拉刷新容器不跟随手势下拉。  - 100表示下拉刷新容器紧紧跟随手势下拉。  - 数值越大，下拉刷新容器跟随手势下拉的反应越灵敏。  **说明：** 从API version 8开始支持，从API version 11开始废弃。建议使用[pullDownRatio](ts-container-refresh.md#pulldownratio12)替代。 |
| builder10+ | [CustomBuilder](ts-types.md#custombuilder8) | 否 | 是 | 自定义刷新区域显示内容。  **说明：**  与refreshingContent参数同时设置时，该参数不生效。使用本参数自定义刷新区域显示内容时，promptText不显示。  API version 10及之前版本，自定义组件的高度限制在64vp之内。API version 11及以后版本没有此限制。  自定义组件设置了固定高度时，自定义组件会以固定高度显示在刷新区域下方；自定义组件未设置高度时，自定义组件高度会自适应刷新区域高度，并随刷新区域高度变化至0。建议对自定义组件设置最小高度约束来避免自定义组件高度小于预期的情况发生，具体可参照[示例3](ts-container-refresh.md#示例3自定义刷新区域显示内容-builder)。  从API version 12开始，建议使用refreshingContent参数替代builder参数自定义刷新区域显示内容，以避免刷新过程中因自定义组件销毁重建造成的动画中断问题。  **元服务API：** 从API version 11开始，该接口支持在元服务中使用。  **模型约束：** 此接口仅可在Stage模型下使用。 |
| promptText12+ | [ResourceStr](ts-types.md#resourcestr) | 否 | 是 | 设置刷新区域底部显示的自定义文本。不设置时，不显示该文本。  **说明：**  输入文本的限制参考Text组件，使用builder或refreshingContent参数自定义刷新区域显示内容时，promptText不显示。  promptText设置有效时，[refreshOffset](ts-container-refresh.md#refreshoffset12)属性默认值为96vp。  自定义文本最大的字体缩放倍数[maxFontScale](ts-basic-components-text.md#maxfontscale12)为2。  自定义文本最大行数为1，超出部分以省略号（...）截断。若需多行提示，请通过builder或refreshingContent参数自定义刷新区域显示内容实现。  **元服务API：** 从API version 12开始，该接口支持在元服务中使用。  **模型约束：** 此接口仅可在Stage模型下使用。 |
| refreshingContent12+ | [ComponentContent](js-apis-arkui-componentcontent.md) | 否 | 是 | 自定义刷新区域显示内容。  **说明：**  与builder参数同时设置时builder参数不生效。使用本参数自定义刷新区域显示内容时，promptText不显示。  自定义组件设置了固定高度时，自定义组件会以固定高度显示在刷新区域下方；自定义组件未设置高度时，自定义组件高度会自适应刷新区域高度，并随刷新区域高度变化至0。建议对自定义组件设置最小高度约束来避免自定义组件高度小于预期的情况发生，具体可参照[示例4](ts-container-refresh.md#示例4自定义刷新区域显示内容-refreshingcontent)。  **元服务API：** 从API version 12开始，该接口支持在元服务中使用。  **模型约束：** 此接口仅可在Stage模型下使用。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | number | 是 | 下拉偏移量，单位vp。  取值范围：(0, +∞)。  默认值：未设置[promptText](ts-container-refresh.md#refreshoptions对象说明)参数时为64vp，设置了[promptText](ts-container-refresh.md#refreshoptions对象说明)参数时为96vp。  如果取值为0或负数的时候此接口采用默认值。 |
...
未通过该接口设置时，当未设置[promptText](ts-container-refresh.md#refreshoptions对象说明)参数时，默认偏移量为64vp；设置了[promptText](ts-container-refresh.md#refreshoptions对象说明)参数时，默认偏移量为96vp。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | number | [Resource](ts-types.md#resource) | 是 | 下拉偏移量。  单位：vp  取值范围：(0, +∞)。  默认值：未设置[promptText](ts-container-refresh.md#refreshoptions对象说明)参数时为64vp，设置了[promptText](ts-container-refresh.md#refreshoptions对象说明)参数时为96vp。  值为0或负数时，按默认值处理。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| ratio | [Optional](ts-universal-attributes-custom-property.md#optionalt)<number> | 是 | 下拉跟手系数。数值越大，跟随手势下拉的反应越灵敏。0表示不跟随手势下拉，1表示等比例跟随手势下拉。  没有设置或设置为undefined时，默认使用动态下拉跟手系数，下拉距离越大，跟手系数越小。  取值范围：[0, 1]。小于0的值会被视为0，大于1的值会被视为1。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| distance | [Optional](ts-universal-attributes-custom-property.md#optionalt)<number> | 是 | 最大下拉距离。  取值范围：[0, +∞)，值小于0时按0处理。当该值小于刷新的下拉偏移量refreshOffset时，Refresh下拉离手不会触发刷新。  undefined和null按没有设置此属性处理。  默认值：undefined  单位：vp |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| distance | number | [Resource](ts-types.md#resource) | undefined | 是 | 最大下拉距离。  默认值：undefined  单位：vp  取值范围：[0, +∞)，值小于0时按0处理。当该值小于刷新的下拉偏移量[refreshOffset](ts-container-refresh.md#refreshoffset12)时，Refresh下拉离手不会触发刷新。  undefined和null按没有设置此属性处理，即没有最大下拉距离限制。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ac/v3/70RdwvggRwuuAhQd9FjT5A/zh-cn_image_0000002742124963.gif)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/a8/v3/OYdDOCJPTZuNlpNUKqpq4g/zh-cn_image_0000002712246056.gif)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/27/v3/U7qskVsMRd28Ay31XAwSvA/zh-cn_image_0000002742005005.gif)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/01/v3/R0At03gxRQ6nXpnRL3haHQ/zh-cn_image_0000002712406016.gif)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/0b/v3/rTEGEeuxRpaewrAYRKgnRA/zh-cn_image_0000002742124965.gif)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/14/v3/bjuKLk6wRGmFg9X5M30qoA/zh-cn_image_0000002712246058.gif)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/12/v3/5npWmP7LSf24_xatVXmp3g/zh-cn_image_0000002742005007.gif)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/f7/v3/ewXh-C1zQtiY3Lg5jSAlIQ/zh-cn_image_0000002712406018.gif)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ba/v3/z-6SWR5MQHSnJsASfgfGYA/zh-cn_image_0000002742124967.gif)

### 示例10（上滑不取消刷新）

该示例通过[pullUpToCancelRefresh](ts-container-refresh.md#pulluptocancelrefresh23)接口设置上滑不取消刷新。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/a8/v3/Pj8p7rdATyiKpXW9nX-ovw/zh-cn_image_0000002712246060.gif)
