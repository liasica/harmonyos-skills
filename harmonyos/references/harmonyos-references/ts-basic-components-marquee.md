...
content_hash: sha256:ce4a1d7177870cbbdc5cf589f01df4ff3aae68a5240649b8a4a4bf992a76b1a7
---
...
| step8+ | number | 否 | 是 | 滚动动画的文本步长。  取值范围：[0, 文本宽度]，当step大于Marquee的文本宽度时，取默认值。  默认值：6  单位：[vp](ts-pixel-units.md#基本像素单位)  **卡片能力：** 从API version 9开始，该接口支持在ArkTS卡片中使用。  **元服务API：** 从API version 11开始，该接口支持在元服务中使用。 |
...
| spacing23+ | [LengthMetrics](js-apis-arkui-graphics.md#lengthmetrics12) | 否 | 是 | 两轮跑马灯之间的间距。当LengthMetrics对象的unit属性为LengthUnit.PERCENT时，当前设置不生效，按默认值处理。  默认值：跑马灯组件宽度。  **卡片能力：** 从API version 23开始，该接口支持在ArkTS卡片中使用。  **元服务API：** 从API version 23开始，该接口支持在元服务中使用。  **模型约束：** 此接口仅可在Stage模型下使用。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [Length](ts-types.md#length) | 是 | 字体大小。fontSize为number类型时，使用fp单位。字体默认大小16fp。不支持设置百分比字符串。  Wearable设备上默认值：15fp  **说明：**  配合[allowScale](ts-basic-components-marquee.md#allowscale)属性使用时，需设置为fp单位。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | boolean | 是 | 是否允许文本缩放。  true：允许文本缩放；false：不允许文本缩放。  **说明：**  仅当[fontSize](ts-basic-components-marquee.md#fontsize)为fp单位时生效。 |
...
* MarqueeUpdateStrategy.PRESERVE\_POSITION：适用于内容动态更新时希望保持当前滚动位置继续滚动的场景，如实时时钟、股价等动态内容展示。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| event | [Callback](ts-types.md#callback12)<void>| undefined | 是 | 跑马灯滚动结束或停止时触发回调。  设置为undefined时不会执行回调。 |

## 示例

### 示例1（跑马灯内容动态更新）

该示例展示了跑马灯内容动态更新时的运行效果，主要涉及start、step、loop、fromStart、src等属性以及[marqueeUpdateStrategy](ts-basic-components-marquee.md#marqueeupdatestrategy12)属性的设置。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/e8/v3/gxm3-0SzSVS4G7X9Snb5Sw/zh-cn_image_0000002736315197.gif)

### 示例2（设置跑马灯停止回调）

该示例通过变更跑马灯状态来触发onStop回调，回调触发后使停止计数器numberStop的值加1。

从API版本26.0.0开始，新增[onStop](ts-basic-components-marquee.md#onstop)接口。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/fe/v3/2GH1g6OPR4OwLHzkkCUp1w/zh-cn_image_0000002706676154.gif)
