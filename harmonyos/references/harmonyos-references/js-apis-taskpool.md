...
* 串行队列任务：对应为[SequenceRunner](js-apis-taskpool.md#sequencerunner11)任务。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| func | Function | 是 | 待执行的函数，必须使用[@Concurrent装饰器](../harmonyos-guides/taskpool-introduction.md#concurrent装饰器)装饰。支持的函数返回值类型请参考[序列化支持类型](js-apis-taskpool.md#序列化支持类型)。 |
| args | Object[] | 否 | 任务执行函数的入参，支持的参数类型请参考[序列化支持类型](js-apis-taskpool.md#序列化支持类型)。默认值为undefined。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| func | (...args: A) => R | Promise<R> | 是 | 待执行的函数，必须使用[@Concurrent装饰器](../harmonyos-guides/taskpool-introduction.md#concurrent装饰器)装饰，支持的函数返回值类型请参考[序列化支持类型](js-apis-taskpool.md#序列化支持类型)。 |
| args | A | 否 | 任务执行函数的入参，支持的参数类型请参考[序列化支持类型](js-apis-taskpool.md#序列化支持类型)。默认值为undefined。 |
...
| --- | --- | --- | --- |
| task | [Task](js-apis-taskpool.md#task) | 是 | 需要在任务池中执行的任务。 |
| configs | [Configs](js-apis-taskpool.md#configs24) | 是 | 该参数可以设置超时时间和任务优先级。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| task | [GenericsTask](js-apis-taskpool.md#genericstask13)<A, R> | 是 | 需要在任务池中执行的泛型任务。 |
| priority | [Priority](js-apis-taskpool.md#priority) | 否 | 等待执行的任务的优先级，默认值为taskpool.Priority.MEDIUM。 |
...
| --- | --- | --- | --- |
| delayTime | number | 是 | 延时时间。单位：ms。delayTime值必须要大于等于0。 |
| task | [GenericsTask](js-apis-taskpool.md#genericstask13)<A, R> | 是 | 需要延时执行的泛型任务。 |
...
| --- | --- | --- | --- |
| period | number | 是 | 周期时长。单位：ms。period值必须要大于等于0。 |
| task | [GenericsTask](js-apis-taskpool.md#genericstask13)<A, R> | 是 | 需要周期执行的泛型任务。 |
| priority | [Priority](js-apis-taskpool.md#priority) | 否 | 周期执行的任务的优先级，该参数默认值为taskpool.Priority.MEDIUM。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| longTask | [LongTask](js-apis-taskpool.md#longtask12) | 是 | 需要终止的长时任务。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| function | Function | 否 | 否 | 待执行的函数，必须使用[@Concurrent装饰器](../harmonyos-guides/taskpool-introduction.md#concurrent装饰器)装饰，支持的函数返回值类型请参考[序列化支持类型](js-apis-taskpool.md#序列化支持类型)。  **元服务API：** 从API version 11开始，该接口支持在元服务中使用。 |
| arguments | Object[] | 否 | 是 | 创建任务传入函数所需的参数，支持的参数类型请参考[序列化支持类型](js-apis-taskpool.md#序列化支持类型)。默认值为undefined。  **元服务API：** 从API version 11开始，该接口支持在元服务中使用。 |
...
* 此接口与[setCloneList](js-apis-taskpool.md#setclonelist11)互斥：同一个ArrayBuffer不能同时设置在transfer列表和clone列表中。
...
* 此接口与[setTransferList](js-apis-taskpool.md#settransferlist10)互斥：同一个ArrayBuffer不能同时设置在transfer列表和clone列表中。
...
* 调用该接口时，请确保处理数据的回调函数在宿主线程通过[onReceiveData](js-apis-taskpool.md#onreceivedata11)注册。
...
* 该方法与[sendData](js-apis-taskpool.md#senddata11)配对使用。
...
* 如果所有任务正常执行，异步执行完毕后返回所有任务结果的数组，数组中元素的顺序与[addTask](js-apis-taskpool.md#addtask10-1)的顺序相同。
* 如果任意任务失败，则会抛出对应异常。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| threadInfos | [ThreadInfo](js-apis-taskpool.md#threadinfo10)[] | 否 | 否 | 工作线程的内部信息。不建议修改此值。 |
| taskInfos | [TaskInfo](js-apis-taskpool.md#taskinfo10)[] | 否 | 否 | 任务的内部信息。不建议修改此值。 |
...
| priority | [Priority](js-apis-taskpool.md#priority) | 否 | 是 | 任务的优先级。默认值为taskpool.Priority.MEDIUM。 |
