---
url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-floatview
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| config | [FloatViewConfiguration](js-apis-floatview.md#floatviewconfiguration) | 是 | 创建闪控窗控制器的参数。该参数及其context字段不能为null或undefined，否则抛出401。其他参数异常情况抛出1300016。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| Promise<[FloatViewController](js-apis-floatview.md#floatviewcontroller)> | Promise对象。返回当前创建的闪控窗控制器。 |
...
绑定闪控窗和闪控球。需要先创建[闪控窗控制器](js-apis-floatview.md#floatviewcontroller)和[闪控球控制器](js-apis-floatingball.md#floatingballcontroller)，且均未启动。使用Promise异步回调。

**说明** 

* 绑定成功后，调用[start()](js-apis-floatview.md#start)或[startFloatingBall()](js-apis-floatingball.md#startfloatingball)均会同时创建闪控窗窗口和闪控球窗口，并触发对应窗口已注册的状态回调。但同一时刻仅展示其中一个窗口，展示顺序取决于先调用哪个控制器的启动接口。
* 绑定成功后，用户可通过点击操作在闪控窗窗口与闪控球之间进行切换。
* 绑定成功后，调用任一控制器的停止接口（[stop()](js-apis-floatview.md#stop)或[stopFloatingBall()](js-apis-floatingball.md#stopfloatingball)）会同时销毁闪控窗窗口和闪控球窗口，并触发对应窗口已注册的状态回调。

**起始版本：** 26.0.0

**模型约束：** 此接口仅可在Stage模型下使用。

**需要权限：** ohos.permission.USE\_FLOAT\_BALL 和 ohos.permission.FLOAT\_VIEW
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| floatViewController | [FloatViewController](js-apis-floatview.md#floatviewcontroller) | 是 | 闪控窗控制器，用于管理闪控窗的启动、停止和状态监听等操作。 |
...
解绑闪控窗和闪控球。需要在[闪控窗控制器](js-apis-floatview.md#floatviewcontroller)和[闪控球控制器](js-apis-floatingball.md#floatingballcontroller)均停止后才可解绑。使用Promise异步回调。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| templateType | [FloatViewTemplateType](js-apis-floatview.md#floatviewtemplatetype) | 是 | 闪控窗模板类型。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| [FloatViewLimits](js-apis-floatview.md#floatviewlimits) | 返回闪控窗窗口的限制，包括最大尺寸、最小尺寸和宽高比的限制范围。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| context | [BaseContext](js-apis-inner-application-basecontext.md) | 否 | 否 | 表示上下文环境，用于创建闪控窗控制器时关联应用主窗口。必须传入有效的UIAbilityContext实例。 |
| templateType | [FloatViewTemplateType](js-apis-floatview.md#floatviewtemplatetype) | 否 | 否 | 闪控窗的模板类型。 |
...
下列API示例中都需先使用[floatView.create()](js-apis-floatview.md#floatviewcreate)方法获取到闪控窗控制器实例（即floatViewController），再通过此实例调用对应方法。
...
本接口不支持加载跨包页面，若有需要请使用[setUIContextByName()](js-apis-floatview.md#setuicontextbyname)接口。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| path | string | 是 | 要加载到窗口中的页面内容的路径，该路径需添加到工程的main\_pages.json文件中。不支持相对路径写法，需与main\_pages.json中的src取值保持一致。若路径无效或不满足上述要求，将抛出错误码1300016。 |
| storage | [LocalStorage](../harmonyos-guides/arkts-localstorage.md) | 否 | 页面级UI状态存储单元，用于为加载到窗口的页面内容传递状态属性。默认值为空。 |
...
设置闪控窗窗口大小。建议先调用[getFloatViewLimits](js-apis-floatview.md#floatviewgetfloatviewlimits)接口获取推荐的宽高范围和宽高比范围，再根据推荐值调用本接口。窗口实际大小变化可通过[onRectChange](js-apis-floatview.md#onrectchange)接口监听。使用Promise异步回调。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| size | [window.Size](arkts-apis-window-i.md#size7) | 是 | 表示窗口的大小，单位为px，宽度和高度必须大于0，超出有效范围时抛出错误码1300016。建议大小满足[getFloatViewLimits](js-apis-floatview.md#floatviewgetfloatviewlimits)接口返回的限制。 |
...
切换闪控窗的模板并改变其窗口尺寸。建议先调用[getFloatViewLimits](js-apis-floatview.md#floatviewgetfloatviewlimits)接口获取目标模板类型推荐的宽高范围和宽高比范围，再根据推荐值调用本接口。窗口实际大小变化可通过[onRectChange](js-apis-floatview.md#onrectchange)接口监听。使用Promise异步回调。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| templateProperty | [TemplateProperty](js-apis-floatview.md#templateproperty) | 是 | 表示需要切换的窗口模板类型及大小。size中的宽度和高度必须大于0，超出有效范围时抛出错误码1300016。建议大小满足[getFloatViewLimits](js-apis-floatview.md#floatviewgetfloatviewlimits)接口返回的限制。 |
...
启动闪控窗窗口。接口返回不表示start流程结束，需要通过[onStateChange](js-apis-floatview.md#onstatechange)接口监听到STARTED回调时判断启动成功。建议在调用[setUIContext()](js-apis-floatview.md#setuicontext)或[setUIContextByName()](js-apis-floatview.md#setuicontextbyname)后调用start()。使用Promise异步回调。

**起始版本：** 26.0.0

**模型约束：** 此接口仅可在Stage模型下使用。

**需要权限：** ohos.permission.FLOAT\_VIEW
...
停止闪控窗窗口。接口返回不表示stop流程结束，需要通过[onStateChange](js-apis-floatview.md#onstatechange)接口监听到STOPPED回调时判断停止成功。使用Promise异步回调。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| wantParameters | Record<string, Object> | 否 | 恢复闪控窗的主窗口时会给主窗口传递的自定义参数，主窗口会在触发[onNewWant](js-apis-app-ability-abilitylifecyclecallback.md#onnewwant12)回调时收到。默认值为空，代表不向主窗口传入任何自定义参数。 |
...
| 类型 | 说明 |
| --- | --- |
| [FloatViewProperties](js-apis-floatview.md#floatviewproperties) | 返回闪控窗窗口的属性。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback<[FloatViewStateChangeInfo](js-apis-floatview.md#floatviewstatechangeinfo)> | 是 | 回调函数。返回当前的闪控窗状态变化信息。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback<[FloatViewStateChangeInfo](js-apis-floatview.md#floatviewstatechangeinfo)> | 否 | 回调函数。返回当前的闪控窗状态变化信息。若传入参数，则停止该监听。若未传入参数，则停止所有闪控窗状态变化的监听。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback<[FloatViewRectChangeInfo](js-apis-floatview.md#floatviewrectchangeinfo)> | 是 | 回调函数。返回当前的闪控窗矩形区域变化信息。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback<[FloatViewRectChangeInfo](js-apis-floatview.md#floatviewrectchangeinfo)> | 否 | 回调函数。返回当前的闪控窗矩形区域变化信息。若传入参数，则停止该监听。若未传入参数，则停止所有闪控窗矩形区域变化的监听。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback<[FloatViewLimits](js-apis-floatview.md#floatviewlimits)> | 是 | 回调函数。返回当前的闪控窗限制变化信息。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback<[FloatViewLimits](js-apis-floatview.md#floatviewlimits)> | 否 | 回调函数。返回当前的闪控窗限制变化信息。若传入参数，则停止该监听。若未传入参数，则停止所有闪控窗限制变化的监听。 |
...
| 名称 | 值 | 说明 |
| --- | --- | --- |
| ROUNDED\_RECTANGLE | 0 | 圆角矩形。 |
| HORIZONTAL\_BAR | 1 | 水平的条状矩形。 |
...
| avoidArea | [window.AvoidArea](arkts-apis-window-i.md#avoidarea7) | 否 | 否 | 闪控窗内容的避让区域。  **注意：**  通过[setUIContext()](js-apis-floatview.md#setuicontext)或[setUIContextByName()](js-apis-floatview.md#setuicontextbyname)加载的页面中，位于避让区域的组件将不响应手势事件，开发者在添加需要手势响应事件的组件时，请注意避让这些区域。 |
...
| ratioLimits | Array<[RatioLimit](js-apis-floatview.md#ratiolimit)> | 否 | 否 | 闪控窗的宽高比限制范围，数组中每个元素包含minRatio（最小宽高比）和maxRatio（最大宽高比）。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| state | [FloatViewState](js-apis-floatview.md#floatviewstate) | 否 | 否 | 闪控窗的状态。 |
| stopReason | string | 否 | 否 | 闪控窗停止的原因。该参数仅在状态为FloatViewState.STOPPED时有效，在其他状态下默认为空字符串。停止原因和对应含义如下：  "APP\_STOP"：应用主动停止  "STOP\_IN\_SIDEBAR"：在侧边栏被关闭  "TITLE\_BAR\_STOP\_CLICK"：标题栏点击关闭按钮  "DUMPSTER\_STOP"：拖入垃圾桶停止  "REPLACE\_STOP"：被其他闪控窗挤占  "FLOATING\_BALL\_STOP"：绑定状态下跟随闪控球停止  "MAIN\_WINDOW\_DESTROY\_STOP"：context关联的主窗被销毁后停止 |
...
| --- | --- | --- |
| STARTED | 1 | 闪控窗已启动并显示。 |
| HIDDEN | 2 | 闪控窗已隐藏。上滑进入多任务界面时触发；或使用[setFloatViewVisibilityInApp](js-apis-floatview.md#setfloatviewvisibilityinapp)接口设置应用在前台时隐藏闪控窗后，应用处于前台时触发。 |
| STOPPED | 3 | 闪控窗已停止。 |
| IN\_SIDEBAR | 4 | 闪控窗在侧边栏中。 |
| IN\_FLOATING\_BALL | 5 | 闪控窗切换为闪控球。 |
| ERROR | 6 | 闪控窗发生异常。 |
...
| windowRect | [window.Rect](arkts-apis-window-i.md#rect7) | 否 | 否 | 闪控窗窗口矩形区域。 |
| windowScale | number | 否 | 否 | 闪控窗窗口缩放比例。 |
| reason | string | 否 | 否 | 闪控窗矩形区域变化的原因。原因和对应含义如下：  "POSITION\_CHANGE"：位置变化  "SIZE\_CHANGE"：大小变化  "RECT\_CHANGE"：位置大小同时变化 |
