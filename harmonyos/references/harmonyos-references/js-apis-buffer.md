---
url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-buffer
...
| 类型 | 说明 |
| --- | --- |
| [Buffer](js-apis-buffer.md#buffer) | 返回一个Buffer对象，该对象与入参对象arrayBuffer共享相同的内存区域。 |
...
| byteOffset | number | 是 | 否 | 当前Buffer所在内存池的偏移量。  - 当Buffer通过内存池创建时（如使用[allocUninitializedFromPool](js-apis-buffer.md#bufferallocuninitializedfrompool)创建Buffer，或使用buffer.from()传入字符串，且字符串长度加当前内存池偏移量小于4kb），返回相对于内存池的偏移量。  - 当Buffer直接分配内存时（如使用[alloc](js-apis-buffer.md#bufferalloc)），返回值为0。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| otherBuffer | Uint8Array | [Buffer](js-apis-buffer.md#buffer) | 是 | 比较的目标对象。 |
...
| 类型 | 说明 |
| --- | --- |
| [Buffer](js-apis-buffer.md#buffer) | 返回填充后的Buffer对象。 |
...
| 类型 | 说明 |
| --- | --- |
| [Buffer](js-apis-buffer.md#buffer) | 返回新的Buffer对象。当start < 0或end < 0时返回空Buffer。 |
...
| 类型 | 说明 |
| --- | --- |
| [Buffer](js-apis-buffer.md#buffer) | 交换之后的Buffer对象。 |
