---
url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-inputeventmonitor
title: 全局基础输入事件监听
breadcrumb: API参考 > 应用框架 > ArkUI（方舟UI框架） > ArkTS组件 > 通用事件 > 交互事件分发控制 > 全局基础输入事件监听
category: harmonyos-references
scraped_at: 2026-09-02T15:00:55+08:00
doc_updated_at: 2026-08-29
content_hash: sha256:be526f7a27a13fecded926786316095cd6e5675dfc61602b64af2676f6ce9971
---
...
* 当前回调参数event仅会封装以下原始输入事件类型：[MouseEvent](ts-universal-mouse-key.md#mouseevent对象说明)、[TouchEvent](ts-universal-events-touch.md#touchevent对象说明)、[KeyEvent](ts-universal-events-key.md#keyevent对象说明)。开发者可通过[asMouseEvent](ts-inputeventmonitor.md#asmouseevent)、[asTouchEvent](ts-inputeventmonitor.md#astouchevent)、[asKeyEvent](ts-inputeventmonitor.md#askeyevent)获取对应事件对象。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| event | [RawInputEventWrapper](ts-inputeventmonitor.md#rawinputeventwrapper) | 是 | 输入事件包装器，系统自动创建和传递，开发者无需手动创建。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| [InputEventInterceptResult](ts-inputeventmonitor.md#inputeventinterceptresult) | 事件拦截结果。 |

## InputEventInterceptResult

输入事件拦截结果接口，用于监听器回调[InputEventListener](ts-inputeventmonitor.md#inputeventlistener)返回是否阻止事件传递的决策。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| action | [InputEventInterceptAction](ts-appendix-enums.md#inputeventinterceptaction) | 否 | 否 | 输入事件拦截动作。  CONTINUE：允许事件继续传递到UI框架。  BLOCK：阻止事件传递到UI框架。 |
