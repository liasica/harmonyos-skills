...
content_hash: sha256:b30187e95a0a786d31de9bc5088372d3ef9a3667d05c0415fa0ea4835b12d442
---

本模块提供划词管理能力，包括创建面板、显示面板、移动面板、隐藏面板、销毁面板、监听鼠标/触控板划词事件、获取选中文本等。典型使用流程如下：

1. 调用[on('selectionCompleted')](js-apis-selectioninput-selectionmanager.md#selectionmanageronselectioncompleted)订阅划词完成事件。
2. 在回调中调用[getSelectionContent](js-apis-selectioninput-selectionmanager.md#getselectioncontent)获取选中文本。
3. 调用[createPanel](js-apis-selectioninput-selectionmanager.md#createpanel)创建划词面板。
4. 调用[setUiContent](js-apis-selectioninput-selectionmanager.md#setuicontent)加载页面内容。
5. 调用[moveToGlobalDisplay](js-apis-selectioninput-selectionmanager.md#movetoglobaldisplay)移动面板到指定位置。
6. 调用[show](js-apis-selectioninput-selectionmanager.md#show)显示面板。
7. 调用[destroyPanel](js-apis-selectioninput-selectionmanager.md#destroypanel)销毁面板。
8. 调用[off('selectionCompleted')](js-apis-selectioninput-selectionmanager.md#selectionmanageroffselectioncompleted)取消订阅划词完成事件。
...
订阅划词完成事件，与[off('selectionCompleted')](js-apis-selectioninput-selectionmanager.md#selectionmanageroffselectioncompleted)搭配使用取消订阅。
...
| callback | Callback<[SelectionInfo](js-apis-selectioninput-selectionmanager.md#selectioninfo)> | 是 | 回调函数，返回划词事件信息[SelectionInfo](js-apis-selectioninput-selectionmanager.md#selectioninfo)。该回调仅在用户通过鼠标或触控板选中文本（双击/三击/滑动）后按下Ctrl键时触发。 |
...
取消订阅划词完成事件，与[on('selectionCompleted')](js-apis-selectioninput-selectionmanager.md#selectionmanageronselectioncompleted)搭配使用。
...
| callback | Callback<[SelectionInfo](js-apis-selectioninput-selectionmanager.md#selectioninfo)> | 否 | 需要取消的回调函数（即之前通过on方法订阅时的回调实例）。参数不填写时，取消订阅type对应的所有回调事件。 |
...
获取选中文本的内容。使用Promise异步回调。需在[on('selectionCompleted')](js-apis-selectioninput-selectionmanager.md#selectionmanageronselectioncompleted)回调中调用，且仅在划词完成事件触发后有效。
...
创建划词面板，用于向用户展示业务相关的操作界面或文本处理结果，使用完毕后需调用[destroyPanel](js-apis-selectioninput-selectionmanager.md#destroypanel)销毁面板释放资源。使用Promise异步回调。

单个划词应用仅允许创建一个[MENU\_PANEL](js-apis-selectioninput-selectionpanel.md#paneltype)和一个[MAIN\_PANEL](js-apis-selectioninput-selectionpanel.md#paneltype)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| ctx | [Context](js-apis-inner-application-context.md#context) | 是 | 当前划词面板依赖的上下文信息，需使用SelectionExtensionAbility提供的上下文。 |
| info | [PanelInfo](js-apis-selectioninput-selectionpanel.md#panelinfo) | 是 | 划词面板的配置信息，用于指定面板类型、位置和宽高。单个划词应用仅允许创建一个MENU\_PANEL和一个MAIN\_PANEL。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| Promise<[Panel](js-apis-selectioninput-selectionmanager.md#panel)> | Promise对象，返回当前创建的划词面板对象，可用于面板内容设置、显示、隐藏、移动及事件订阅等管理操作。 |
...
销毁划词面板。与[createPanel](js-apis-selectioninput-selectionmanager.md#createpanel)搭配使用，用于销毁由createPanel()创建的面板对象。使用Promise异步回调。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| selectionType | [SelectionType](js-apis-selectioninput-selectionmanager.md#selectiontype) | 否 | 否 | 划词方式枚举值。 |
...
划词面板对象，通过[createPanel](js-apis-selectioninput-selectionmanager.md#createpanel)创建，提供面板内容设置、显示、隐藏、移动及事件订阅等管理能力，适用于在划词完成后向用户展示自定义操作界面的场景。
...
为当前的划词面板设置界面内容，例如展示翻译结果、搜索建议或自定义操作按钮等。需通过[createPanel](js-apis-selectioninput-selectionmanager.md#createpanel)获取到Panel实例后调用。使用Promise异步回调。
...
显示划词面板，与[hide](js-apis-selectioninput-selectionmanager.md#hide)搭配使用。需通过[createPanel](js-apis-selectioninput-selectionmanager.md#createpanel)获取到Panel实例后调用。使用Promise异步回调。
...
隐藏当前划词面板，与[show](js-apis-selectioninput-selectionmanager.md#show)搭配使用。需通过[createPanel](js-apis-selectioninput-selectionmanager.md#createpanel)获取到Panel实例后调用。使用Promise异步回调。如不主动调用，面板在失焦时会自动隐藏。
...
设置划词面板可随鼠标、触控板或触屏拖动移动位置，指针释放后自动停止移动。需通过[createPanel](js-apis-selectioninput-selectionmanager.md#createpanel)获取到Panel实例后调用。使用Promise异步回调。该接口需在onTouch的回调函数中调用，并且事件类型为TouchType.Down。
...
移动划词面板至屏幕全局坐标系下的指定位置，支持移动到扩展屏上。需通过[createPanel](js-apis-selectioninput-selectionmanager.md#createpanel)获取到Panel实例后调用。使用Promise异步回调。
...
订阅划词面板销毁事件，与[off('destroyed')](js-apis-selectioninput-selectionmanager.md#offdestroyed)搭配使用。需通过[createPanel](js-apis-selectioninput-selectionmanager.md#createpanel)获取到Panel实例后调用。
...
| callback | Callback<void> | 是 | 回调函数，调用[destroyPanel](js-apis-selectioninput-selectionmanager.md#destroypanel)销毁面板时触发。 |
...
取消订阅划词面板销毁事件，与[on('destroyed')](js-apis-selectioninput-selectionmanager.md#ondestroyed)搭配使用。需通过[createPanel](js-apis-selectioninput-selectionmanager.md#createpanel)获取到Panel实例后调用。
...
订阅划词面板隐藏事件，与[off('hidden')](js-apis-selectioninput-selectionmanager.md#offhidden)搭配使用。面板调用[hide](js-apis-selectioninput-selectionmanager.md#hide)隐藏或失焦自动隐藏时触发该事件。需通过[createPanel](js-apis-selectioninput-selectionmanager.md#createpanel)获取到Panel实例后调用。
...
| callback | Callback<void> | 是 | 回调函数，面板隐藏时触发。面板可通过调用[hide](js-apis-selectioninput-selectionmanager.md#hide)主动隐藏，或在失焦时自动隐藏。 |
...
取消订阅划词面板隐藏事件，与[on('hidden')](js-apis-selectioninput-selectionmanager.md#onhidden)搭配使用。需通过[createPanel](js-apis-selectioninput-selectionmanager.md#createpanel)获取到Panel实例后调用。
...
| 名称 | 值 | 说明 |
| --- | --- | --- |
| MOUSE\_MOVE | 1 | 鼠标或触控板滑动划词。 |
| DOUBLE\_CLICK | 2 | 鼠标或触控板双击划词。 |
| TRIPLE\_CLICK | 3 | 鼠标或触控板三击划词。 |
