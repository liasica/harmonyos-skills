...
content_hash: sha256:791735408a17b84a23ffc2aa806570bcaf1ed1f7b92e988632159ccca929de9a
---
...
| 名称 | 值 | 说明 |
| --- | --- | --- |
| Embed | 0 | 侧边栏嵌入到组件内，和内容区并列显示。  整体容器大小不变时，显示侧边栏会导致内容区缩小，隐藏侧边栏会扩大内容区。  组件尺寸小于[minContentWidth](ts-container-sidebarcontainer.md#mincontentwidth10) + [minSideBarWidth](ts-container-sidebarcontainer.md#minsidebarwidth)，并且未设置showSideBar时，侧边栏自动隐藏。  未设置minSideBarWidth或者minContentWidth采用未设置接口的默认值进行计算。  组件在自动隐藏后，如果通过点击控制按钮唤出侧边栏，则侧边栏悬浮在内容区上显示。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [ButtonStyle](ts-container-sidebarcontainer.md#buttonstyle对象说明) | 是 | 侧边栏控制按钮的属性。 |
...
设置侧边栏的宽度。设置为小于0的值时按默认值显示。受最小宽度和最大宽度限制，不在限制区域内取最近的点。与[sideBarWidth](ts-container-sidebarcontainer.md#sidebarwidth)相比，value参数新增了对百分比字符串和其他[像素单位](ts-pixel-units.md)的支持。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [Length](ts-types.md#length) | 是 | 侧边栏的宽度。  默认值：240vp  单位：vp  取值范围：[0, +∞)  **说明：**  API version 9的默认值为200vp，API version 10的默认值为240vp。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [Length](ts-types.md#length) | 是 | 侧边栏最小宽度。  默认值：API version 9及以下版本默认值为200vp，API version 10的默认值为240vp。  取值范围：[0, +∞) |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [Length](ts-types.md#length) | 是 | 设置侧边栏最大宽度。  默认值：280vp  单位：vp  取值范围：[0, +∞) |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [DividerStyle](ts-container-sidebarcontainer.md#dividerstyle10对象说明) | null | 是 | 分割线的样式。  默认为DividerStyle：显示分割线。  - null或undefined：行为不做处理，分割线样式与默认值保持一致。  **说明：**  API version 11及以下版本，null效果为不显示分割线。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [Dimension](ts-types.md#dimension10) | 是 | SideBarContainer组件内容区可显示的最小宽度。  默认值：360vp  单位：vp |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| strokeWidth | [Length](ts-types.md#length) | 否 | 否 | 分割线的线宽。  默认值：1vp  单位：vp  取值范围：[0, +∞)。  **说明**：  分割线的宽度不支持百分比设置。优先级低于[通用属性height](ts-universal-attributes-size.md#height)，超过通用属性设置大小时，按照通用属性进行裁切。部分设备硬件中存在1像素取整后分割线不显示问题，建议使用2像素。 |
| color | [ResourceColor](ts-types.md#resourcecolor) | 否 | 是 | 分割线的颜色。  默认值：#000000，3% |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ec/v3/C6qnvBx4RzGz-dvtD4A8tQ/zh-cn_image_0000002552959604.png?HW-CC-KV=V1&HW-CC-Date=20260428T000126Z&HW-CC-Expire=86400&HW-CC-Sign=DBB77F83257CFF94A79DB1384171BA1B5AB3147A953BD2818AA73717667392F1)
