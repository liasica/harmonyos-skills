---
url: https://developer.huawei.com/consumer/cn/doc/harmonyos-references/payment-partner-combined-app-prepay
title: 预下单
...
开发者接入合单支付，首先需要调用该接口获取到预支付ID（prepayId），用prepayId构建[orderStr](payment-model.md#orderstr)参数返回给客户端调用支付接口拉起华为支付收银台。

## 接口原型

* **承载协议：** HTTPS POST
* **接口方向：** 开发者服务器 -> 华为支付服务器
* **接口URL：** https://petalpay-developer.cloud.huawei.com.cn/api/v2/partner/combined/preorder/create/app

  说明：元服务预下单接口请使用<https://petalpay-developer.cloud.huawei.com.cn/api/v2/partner/combined/preorder/create/fa>
* **数据格式：**
...
| subOrders | 是 | List<[SubMercOrder](payment-model.md#submercorder)> | 合单支付子订单信息。 |
