...
content_hash: sha256:a6a3002df0d6f91c66f5c278fa36c8cdafaf365dbfcdd041c68bd63b186d9262
---
...
2. 可以使用[查询应用信息命令（dump）](../harmonyos-guides/bm-tool.md#查询应用信息命令dump)查看应用是否安装。查看输出的打印信息，应用未安装时，该命令执行会报错。
   ...
   2. hdc shell bm dump -n com.xxx.demo
   ```
...
调用bundleManager模块中的[getBundleNameByUid接口](js-apis-bundlemanager.md#bundlemanagergetbundlenamebyuid14)时，指定的uid无效。
...
```
1. # 需要将com.xxx.demo替换为实际查询的bundleName
2. hdc shell "bm dump -n com.xxx.demo |grep uid"
```
...
调用bundleManager模块中的[canOpenLink接口](js-apis-bundlemanager.md#bundlemanagercanopenlink12)时，指定的link无效。
...
调用bundleManager模块中的[canOpenLink接口](js-apis-bundlemanager.md#bundlemanagercanopenlink12)时，指定link的scheme未在querySchemes字段下配置。
