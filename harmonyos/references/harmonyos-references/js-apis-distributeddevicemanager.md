...
| 类型 | 说明 |
| --- | --- |
| [DeviceManager](js-apis-distributeddevicemanager.md#devicemanager) | 返回设备管理器对象实例，用于获取可信设备列表以及本地设备的名称、类型、标识和网络标识等信息。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| deviceManager | [DeviceManager](js-apis-distributeddevicemanager.md#devicemanager) | 是 | 通过createDeviceManager创建的设备管理器对象实例。 |
...
| 类型 | 说明 |
| --- | --- |
| Array<[DeviceBasicInfo](js-apis-distributeddevicemanager.md#devicebasicinfo)> | 返回可信设备列表，包含设备标识、名称、类型和网络标识等信息。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback<Array<[DeviceBasicInfo](js-apis-distributeddevicemanager.md#devicebasicinfo)>> | 是 | 回调函数。当获取可信设备列表成功时，err为undefined，data为获取到的可信设备列表；失败时，err为错误对象。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<Array<[DeviceBasicInfo](js-apis-distributeddevicemanager.md#devicebasicinfo)>> | Promise对象，resolve时返回分布式设备基本信息列表，reject时返回错误信息。 |
...
发现周边设备，用于在需要建立分布式连接前搜索可用设备。发现状态持续两分钟，超时后自动停止，最大发现数量为99个。使用WiFi进行设备发现时，要求发现方与被发现方处于同一局域网内。调用本方法前，需先通过[on('discoverSuccess')](js-apis-distributeddevicemanager.md#ondiscoversuccess)注册设备发现成功回调以接收发现的设备信息，并通过[on('discoverFailure')](js-apis-distributeddevicemanager.md#ondiscoverfailure)注册设备发现失败回调以接收失败通知。发现完成后可调用stopDiscovering停止发现。
...
| callback | Callback<{ action: [DeviceStateChange](js-apis-distributeddevicemanager.md#devicestatechange); device: [DeviceBasicInfo](js-apis-distributeddevicemanager.md#devicebasicinfo); }> | 是 | 注册设备状态的回调方法，返回设备状态和设备信息。 |
...
| callback | Callback<{ action: [DeviceStateChange](js-apis-distributeddevicemanager.md#devicestatechange); device: [DeviceBasicInfo](js-apis-distributeddevicemanager.md#devicebasicinfo); }> | 否 | 指示要取消注册的设备状态回调，返回设备状态和设备信息。如果指定该参数则取消对应callback，否则取消所有已注册的deviceStateChange回调。 |
...
| callback | Callback<{ device: [DeviceBasicInfo](js-apis-distributeddevicemanager.md#devicebasicinfo); }> | 是 | 注册设备发现的回调方法，返回发现的设备信息（DeviceBasicInfo）。 |
...
| callback | Callback<{ device: [DeviceBasicInfo](js-apis-distributeddevicemanager.md#devicebasicinfo); }> | 否 | 指示要取消注册的设备发现回调，返回发现的设备信息。如果指定该参数则取消对应callback，否则取消所有已注册的discoverSuccess回调。 |
