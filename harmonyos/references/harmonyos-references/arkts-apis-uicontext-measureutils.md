...
content_hash: sha256:82410e1f447d573e3b575875b7b9a626081d66d17c45688440b6e6b483e113d8
---
...
* 如需更多测算文本参数，比如[includeFontPadding](ts-basic-components-text.md#includefontpadding23)和[fallbackLineSpacing](ts-basic-components-text.md#fallbacklinespacing23)，建议使用图形对应测算接口[Paragraph](js-apis-graphics-text.md#paragraph)接口。
* 调用文本计算接口时，应避免同时用[ApplicationContext.setFontSizeScale](js-apis-inner-application-applicationcontext.md#applicationcontextsetfontsizescale13)设置应用字体大小缩放比例。为了确保时序正确性，建议开发者自行监听字体缩放变化，以保证测算结果的准确性。
* 在测算裁剪后的文本时，由于某些Unicode字符（如emoji）的码位长度大于1，直接按字符串长度裁剪会导致不准确的结果。建议基于Unicode码点进行迭代处理，避免错误截断字符，确保测算结果准确，请参考[measureTextSize](arkts-apis-uicontext-measureutils.md#measuretextsize12)的示例2。
...
* 调用此接口时，应避免同时使用[ApplicationContext.setFontSizeScale](js-apis-inner-application-applicationcontext.md#applicationcontextsetfontsizescale13)设置应用字体大小缩放比例。为了确保时序正确性，建议开发者自行监听字体缩放变化，以保证测算结果的准确性。
...
| 类型 | 说明 |
| --- | --- |
| [SizeOptions](ts-types.md#sizeoptions) | 返回文本所占布局宽度和高度。  **说明：**  未设置constraintWidth时，文本宽度返回值会向上取整；传参constraintWidth时，文本宽度返回值不被取整。  文本宽度以及高度返回值单位均为px。  当参数异常或内部计算失败时，会返回undefined。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/a3/v3/Jae3cJSDSWq0ojSNLS8F_w/zh-cn_image_0000002736314631.png)
...
| options | [TextLayoutOptions](ts-text-common.md#textlayoutoptions对象说明20) | 否 | 文本布局选项。省略时使用默认布局配置。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| Array<[Paragraph](js-apis-graphics-text.md#paragraph)> | 根据文本布局选项转换后得到的[Paragraph](js-apis-graphics-text.md#paragraph)对象数组，用于后续的文本布局计算。  **说明：**  当参数异常或内部计算失败时，会返回undefined。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/6b/v3/nRa7pflqQya5p_YDotHtXg/zh-cn_image_0000002706675588.png)
