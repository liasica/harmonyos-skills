...
| [compileSdkVersion](../harmonyos-guides/ide-hvigor-build-profile-app.md#section45865492619) | 6.0.0(20) | 标识编译应用/元服务所使用的SDK版本。 |
| [targetSdkVersion](../harmonyos-guides/ide-hvigor-build-profile-app.md#section45865492619) | 4.0.0(10)~6.0.0(20) | 标识应用/元服务运行所需目标SDK版本，介于compatibleSdkVersion和compileSdkVersion之间。 |
...
* 元服务工程支持Car设备开发。
* 新建元服务工程时，支持在DevEco Studio页面内注册新的元服务APP ID。具体请参考[创建元服务工程](../atomic-guides/atomic-service-create-project.md)。
* 新增OpenHarmony开源中心仓（OHPM Index）入口，帮助开发者高效筛选和管理三方库。具体请参考[使用OpenHarmony开源中心仓管理三方包](../harmonyos-guides/ide-har-import.md#section1579838153916)。

**使用AI智能辅助编程**

* 智能问答新增添加本地的知识库，并根据知识库进行指定风格的上下文问答功能。具体请参考[本地知识库配置](../harmonyos-guides/ide-ocal-knowledge.md)。
...
* 在预览器中使用文件管理相关的API时，仅支持预览/预览调试Stage模型的HAP/HSP，并且需要先打开**Enable file operation**开关。具体请参考[文件管理](../harmonyos-guides/ide-previewer-api-list.md#section157017919348)。
...
* Code Linter新增15条检查规则。具体请参考[规则变更说明](../harmonyos-guides/ide-codelinter-rules-change.md)。
* HAP/HSP引用HSP时支持预览。
...
* DevEco Studio的Settings界面新增一个开关，用于优化增量编译速度。具体请参考[增量判断模块级跳过](../harmonyos-guides/ide-hvigor-experimental-properties.md#section730523404612)。
* 工程级、模块级build-profile.json5的buildOption/arkOptions下新增expandImportPath对象，用于配置import路径展开相关能力，提升应用的运行时性能。具体请参考[工程级build-profile.json5文件](../harmonyos-guides/ide-hvigor-build-profile-app.md)和[模块级build-profile.json5文件](../harmonyos-guides/ide-hvigor-build-profile.md)。
* 工程级build-profile.json5的buildOption下新增preloadSystemSo字段，用于指定是否收集应用入口所使用的系统so，收集的系统so会在应用冷启动时进行预加载，优化应用的冷启动性能。具体请参考[工程级build-profile.json5文件](../harmonyos-guides/ide-hvigor-build-profile-app.md)。
* Hvigor新增线程池相关API，用于向线程池提交并执行worker。具体请参考[submitWorker](../harmonyos-guides/ide-hvigor-api.md#section94763341419)。
* Hvigor新增获取SDK相关信息的API。具体请参考[getSdkDetails](../harmonyos-guides/ide-build-expanding-context.md#section085712212299)。

**优化应用性能**

* 智慧调优能力新增支持对Allocation内存分配问题和Snapshot内存堆快照问题进行分析。具体请参考[智慧调优](../harmonyos-guides/ide-ai-profiler.md)。

* Profiler工具GPU模版支持展示不同GPU硬件块利用率的详细信息。具体请参考[GPU活动分析：GPU分析](../harmonyos-guides/ide-profiler-gpu.md)。
* Frame泳道新增Component Animation子泳道，用于从组件的角度展示应用中包含的各种动画类型。具体请参考[查看组件动画信息](../harmonyos-guides/ide-insight-session-frame.md#section3735165517215)。
...
* 新增生成意图装饰器和添加意图插件能力，以及DevEco Studio新增内嵌的小艺智能平台。具体请参考[意图装饰器生成和智能体创建](../harmonyos-guides/ide-insight-intent2.md)。
...
* 工程级、模块级build-profile.json5新增以下字段。具体请参考[工程级build-profile.json5文件](../harmonyos-guides/ide-hvigor-build-profile-app.md)和[模块级build-profile.json5文件](../harmonyos-guides/ide-hvigor-build-profile.md)。
...
* 模块级build-profile.json5新增excludePackages，用于编译HAP/HSP模块时，指定不参与变量动态import的源码HAR的包名，配置的源码HAR不会参与编译，支持直接/间接依赖。具体请参考[模块级build-profile.json5文件](../harmonyos-guides/ide-hvigor-build-profile.md)。
* 工程级、模块级build-profile.json5的buildOption/nativeLib/filter/select下新增includePattern、excludePattern字段，用于选择打包或排除的native产物，支持glob语法。具体请参考[配置CPP](../harmonyos-guides/ide-hvigor-cpp.md)。
* hvigor-config.json5文件properties下新增以下字段。具体请参考[hvigor-config.json5文件](../harmonyos-guides/ide-hvigor-set-options.md)。
...
* Hvigor的API dependencies、postDependencies支持依赖其他模块的任务，在任务前加上“模块名:”即可。具体请参考[基础构建能力](../harmonyos-guides/ide-hvigor-api.md)。
* 混淆助手新增一个扫描场景：动态导入的名称。具体请参考[扫描任务](../harmonyos-guides/ide-build-obfuscation.md#section18125192133818)。

**命令行工具**

* ohpm支持配置exclusions移除版本号依赖和文件依赖（.har/.tgz依赖）的一个或多个直接子依赖。具体请参考[工程级oh-package.json5 字段说明](../harmonyos-guides/ide-oh-package-json5.md#section02162312018)。
* ohpm config新增encrypt命令，用于使用加密组件加密标准输入的数据，并输出密文到标准输出。具体请参考[ohpm config](../harmonyos-guides/ide-ohpm-config.md)。
* ohpmrc支持配置enable\_boost\_extraction\_speed字段用于ohpm安装解压提速。具体请参考[默认配置项](../harmonyos-guides/ide-ohpmrc.md#zh-cn_topic_0000001792216397_默认配置项)。
...
* DevEco Studio适配IntelliJ 2024.3.3底座升级，提供全新用户界面，带来更简洁的外观及开发体验。具体请参考[使用新UI](../harmonyos-guides/ide-new-ui.md)。
* 新增页面生成功能，支持通过自然语言进行多轮对话，生成应用/元服务可用的页面代码，快速完成页面搭建。具体请参考[页面生成](../harmonyos-guides/ide-page-generation.md)。
...
* 工程级和模块级build-profile.json5文件buildOption/arkOptions下新增skipOhModulesLint字段，用于指定是否跳过工程中oh\_modules目录的[ArkTS规则检查](../harmonyos-guides/typescript-to-arkts-migration-guide.md)。具体请参考[工程级build-profile.json5文件](../harmonyos-guides/ide-hvigor-build-profile-app.md)和[模块级build-profile.json5文件](../harmonyos-guides/ide-hvigor-build-profile.md)。
* Build Analyzer构建分析新增支持超精细化模式，与advanced模式相比，在ArkTS编译阶段记录更详细的打点数据，但开启后可能导致编译构建时间更长。具体请参考[设置构建分析模式](../harmonyos-guides/ide-hvigor-build-analyzer.md#section207890565217)和[hvigorw](../harmonyos-guides/ide-hvigor-commandline.md)。
...
* 应用与元服务体检的规则体检新增七条UX检测规则。具体请参考[规则总览](../harmonyos-guides/ide-app-analyzer-all-rules.md)。
