...
30. // signedToken为鉴权令牌，调用Push Kit REST API时放在Authorization头部来进行鉴权。
...
71. try:
72. # jwt_token 为鉴权令牌，调用Push Kit REST API时放在Authorization头部来进行鉴权。
...
93. // signedToken为鉴权令牌，调用Push Kit REST API时放在Authorization头部来进行鉴权。
