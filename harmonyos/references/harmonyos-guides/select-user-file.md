...
用户需要分享文件、保存图片、视频等用户文件时，开发者可以通过系统预置的[选择器](../harmonyos-references/js-apis-file-picker.md)，实现该能力。通过Picker访问相关文件，将拉起对应的应用，引导用户完成界面操作，接口本身无需申请权限。Picker选择文件或文件夹获取到的URI只具有**临时读写权限**，获取持久化权限需要通过[FilePicker设置永久授权](file-persistpermission.md#通过picker获取临时授权并进行授权持久化)方式获取。
...
2. 需根据实际业务需求配置[文档选择选项](../harmonyos-references/js-apis-file-picker.md#documentselectoptions)。以下代码仅例举各选项的配置参考。
   ...
   // 选择的文档类型，默认值是FILE(文件类型)。从API版本26.0.0开始，当文件类型是FOLDER时，Phone设备支持该参数；当文件类型是MIXED时，该参数在2in1设备中可正常使用，在其他设备中无效果。
...
3. 创建文件选择器[DocumentViewPicker](../harmonyos-references/js-apis-file-picker.md#documentviewpicker)实例。调用[select()](../harmonyos-references/js-apis-file-picker.md#select-3)接口拉起FilePicker应用界面进行文件选择。
   ...
   **注意** 
   ...
   1. 使用Picker获取的[select()](../harmonyos-references/js-apis-file-picker.md#select-3)返回的URI权限是临时读写权限，待退出应用后台后，获取的临时权限就会失效。
