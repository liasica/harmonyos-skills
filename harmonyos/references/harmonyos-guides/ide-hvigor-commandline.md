---
url: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-hvigor-commandline
...
其中taskNames是任务，可同时执行多个任务，options是可选参数，具体的任务和可选参数请参考[常用命令](ide-hvigor-commandline.md#section16300629103)。
...
| -p ohos-test-coverage={true | false} | 执行测试框架代码覆盖率插桩编译，详细使用请参考[使用命令行执行测试](ide-instrument-test.md#section14255191913322)。 |
...
| --syncNative | 在sync阶段执行syncNative，可替换compileNative任务执行，完成并行编译，详细请参考[通过syncNative提升sync阶段C++编译效率](ide-hvigor-experimental-properties.md#section16637112213911)。 |
...
| --config, -c | 指定hvigor-config.json5配置文件中的参数。  当前仅支持设置properties里的参数，具体支持的参数请查看[hvigor-config.json5文件](ide-hvigor-set-options.md#section260315160319)中properties支持的参数。  --config properties.key=value 同 -c properties.key=value |
