...
content_hash: sha256:161364e99232d076aed00fa96c45cfa1ef620152f10b769707cac15ccb6dcdc8
---
...
开始录屏时正在通话中或者录屏过程中来电，录屏将自动停止。因通话中断的录屏会上报OH\_SCREEN\_CAPTURE\_STATE\_STOPPED\_BY\_CALL状态。

录屏过程中发生系统用户切换事件时，录屏将自动停止。因系统用户切换中断的录屏会上报OH\_SCREEN\_CAPTURE\_STATE\_STOPPED\_BY\_USER\_SWITCHES状态。

本开发指导将以完成一次屏幕数据录制的过程为例，向开发者讲解如何使用AVScreenCapture进行录屏，详细的API声明请参考[AVScreenCapture API参考](../harmonyos-references/capi-avscreencapture.md)。

如果配置了采集麦克风音频数据，需申请对应麦克风权限ohos.permission.MICROPHONE和长时任务。配置方式请参考[向用户申请权限](request-user-authorization.md)、[申请长时任务](continuous-task.md)。
...
从API version 22开始，在PC/2in1设备上对应用进行录屏时，可通过申请权限**ohos.permission.CUSTOM\_SCREEN\_RECORDING**，实现在录屏时不再弹出隐私告警弹窗。配置方式请参见[受限开放权限](restricted-permissions.md)。
...
1. 添加头文件。
   ...
   #include <string>
   ```
2. 创建AVScreenCapture实例g\_avCapture。
...
3. 配置录屏参数。
   ...
   创建AVScreenCapture实例g\_avCapture后，可以设置录屏所需要的参数。
   ...
   ```
4. 调用OH\_AVScreenCapture\_StartScreenRecording()方法开始进行录屏。

   ```
   result = OH_AVScreenCapture_StartScreenRecording(g_avCapture);
   ```
5. 调用OH\_AVScreenCapture\_StopScreenRecording()方法停止录制。

   ```
   result = OH_AVScreenCapture_StopScreenRecording(g_avCapture);
   ```
6. 调用OH\_AVScreenCapture\_Release()方法销毁实例，释放资源。
