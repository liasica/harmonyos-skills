...
· 使用上传下载模块，优先推荐使用[request.agent.create](../harmonyos-references/js-apis-request.md#requestagentcreate10)创建上传下载任务。
...
开发者可以使用[ohos.request](../harmonyos-references/js-apis-request.md)的[request.agent.create](../harmonyos-references/js-apis-request.md#requestagentcreate10)接口下载网络资源文件到指定的用户文件目录。
...
开发者可以通过设置配置文件实现HTTP拦截功能。上传下载模块在应用配置文件中禁用HTTP后，无法创建明文HTTP传输的上传下载任务。配置文件在APP中的路径是：src/main/resources/base/profile/network\_config.json。请参考网络管理模块配置文件[网络连接安全配置](network-connection-security-configuration.md)，了解需要配置的具体参数。
