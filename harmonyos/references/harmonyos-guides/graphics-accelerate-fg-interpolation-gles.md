---
url: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/graphics-accelerate-fg-interpolation-gles
title: OpenGL ES平台
...
content_hash: sha256:29469f8626730e868d9eebff1fa0e61049c98d80909016b6012278fb38fe47ec
---
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/4f/v3/SZwzRplVR8azT5pIhAA6kA/zh-cn_image_0000002552799058.png?HW-CC-KV=V1&HW-CC-Date=20260427T234723Z&HW-CC-Expire=86400&HW-CC-Sign=55516F906F0C5FD8270964A3F1A90958ED989CBE66ADCEBAF5A134AB7F43396A)
...
3. 游戏应用调用接口配置超帧实例属性。包括调用[HMS\_FG\_SetAlgorithmMode\_GLES](../harmonyos-references/_graphics_accelerate.md#hms_fg_setalgorithmmode_gles)（必选）设置超帧算法模式并选择内插模式；调用[HMS\_FG\_SetResolution\_GLES](../harmonyos-references/_graphics_accelerate.md#hms_fg_setresolution_gles)（必选）设置超帧输入输出图像分辨率；调用[HMS\_FG\_SetCvvZSemantic\_GLES](../harmonyos-references/_graphics_accelerate.md#hms_fg_setcvvzsemantic_gles)（可选）设置齐次裁剪空间Z/W范围及深度测试函数；调用[HMS\_FG\_SetImageFormat\_GLES](../harmonyos-references/_graphics_accelerate.md#hms_fg_setimageformat_gles)（可选）设置真实渲染帧颜色缓冲区图像格式；如果颜色缓冲区相对深度模板缓冲区基于y轴翻转180度，则调用[HMS\_FG\_SetDepthStencilYDirectionInverted\_GLES](../harmonyos-references/_graphics_accelerate.md#hms_fg_setdepthstencilydirectioninverted_gles)（可选）设置翻转状态。
4. 游戏应用调用[HMS\_FG\_Activate\_GLES](../harmonyos-references/_graphics_accelerate.md#hms_fg_activate_gles)接口激活超帧上下文实例。
5. 游戏应用调用[HMS\_FG\_Dispatch\_GLES](../harmonyos-references/_graphics_accelerate.md#hms_fg_dispatch_gles)接口并传入历史真实渲染帧颜色信息、深度信息、相机矩阵信息，生成预测帧，并更新预测帧缓冲区。当相机视图投影矩阵的平移分量非常大时（如超过10W），预测帧效果下降，画面易出现闪烁。此时可在[HMS\_FG\_Dispatch\_GLES](../harmonyos-references/_graphics_accelerate.md#hms_fg_dispatch_gles)接口调用前调用[HMS\_FG\_SetExtendedCameraInfo\_GLES](../harmonyos-references/_graphics_accelerate.md#hms_fg_setextendedcamerainfo_gles)设置相机扩展信息，从而获取精度更高的预测帧效果。
6. 预测帧绘制UI并送显。
...
10. 用户退出超帧适用的游戏场景。
11. 游戏应用调用[HMS\_FG\_DestroyContext\_GLES](../harmonyos-references/_graphics_accelerate.md#hms_fg_destroycontext_gles)接口销毁超帧上下文实例并释放内存资源。
...
4. 调用超帧实例属性配置接口，超帧算法模式选择内插模式。
   ...
   47. // [可选] 当颜色缓冲区相对深度模板缓冲区基于y轴翻转180度时，设置第二个参数为true，接口不调用时默认为false
...
5. 调用[HMS\_FG\_Activate\_GLES](../harmonyos-references/_graphics_accelerate.md#hms_fg_activate_gles)接口激活超帧上下文实例。
   ...
   5. }
   ```
6. 游戏运行中，真实帧和预测帧交替渲染并送显。渲染真实帧时，缓存颜色信息、深度信息和相机矩阵等属性信息。渲染预测帧时，需调用[HMS\_FG\_Dispatch\_GLES](../harmonyos-references/_graphics_accelerate.md#hms_fg_dispatch_gles)接口并传入上一帧真实帧属性信息，指定预测帧缓冲区索引，生成预测帧，最终更新预测帧缓冲区内存。
   ...
   20. while (true) {
   21. frameNum += 1;
   22. if ((frameNum & 1) != 0) { // 预测帧渲染阶段
   23. // 传入上一帧真实渲染帧颜色缓冲区索引
   24. dispatchDescriptionData_.inputColor = inputColor;
   25. // 传入上一帧真实渲染帧深度模板缓冲区索引
   26. dispatchDescriptionData_.inputDepthStencil = inputDepthStencil;
   27. // 传入预测帧缓冲区索引
   28. dispatchDescriptionData_.outputColor = outputColor;
   29. // 传入上一帧真实渲染帧视图投影矩阵
   30. dispatchDescriptionData_.viewProj = preViewProj;
   31. // 传入上一帧真实渲染帧视图投影逆矩阵
   32. dispatchDescriptionData_.invViewProj= preInvViewProj;

   34. // 生成预测帧，更新预测帧缓冲区的内存
   35. errorCode = HMS_FG_Dispatch_GLES(context_, &dispatchDescriptionData_);
   36. switch (errorCode) {
   37. case FG_SUCCESS: { // 生成预测帧成功
   38. // 绘制预测帧
   39. // ...
   ...
   62. // 渲染当前帧渲染画面，缓存颜色、深度、相机矩阵等信息，用于下一帧预测帧生成
   63. // ...
