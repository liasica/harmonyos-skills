...
1. 在进程启动时，调用接口[createAVMusicTemplate](../harmonyos-references/arkts-apis-avmusictemplate-f.md#avmusictemplatecreateavmusictemplate)创建音频模板实例（每个媒体应用创建一个音频模板实例，无需重复创建）并初始化音频模板。
