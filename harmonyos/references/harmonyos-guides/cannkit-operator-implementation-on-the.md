...
content_hash: sha256:5e73590e8580dcb2aeeaab5fe5267ae3e6981c61ff06f8ffc7e89cf64671147e
---
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/8d/v3/06qwns-EQTiB70QWvGOkeA/zh-cn_image_0000002736314417.png)
...
* 如下示例中，算子中有一个输出依赖计算得出，输出tensor的数据类型为uint32\_t，计算完成后，得到输出的shape为（32, 64），出参shape\_out用于存放该shape信息，值为（2, 32, 64）。代码示例如下。
  ...
  ```
* 如下示例中，算子中有一个输出依赖计算得出，输出tensor的数据类型为uint64\_t，计算完成后，得到输出的shape为（1, 64, 128, 128），出参shape\_out用于存放该shape信息，值为（0x0000000010000000 | 2, 32, 64）。代码示例如下。
