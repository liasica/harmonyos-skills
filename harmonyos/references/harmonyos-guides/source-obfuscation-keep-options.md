---
url: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/source-obfuscation-keep-options
...
content_hash: sha256:f62048536488f94575842cf67139df6bf199c37cfda8a67ba8bc7ea2f9b1e436
---
...
| 功能 | 选项 | 起始API版本 |
| --- | --- | --- |
| 指定保留属性名称 | [-keep-property-name](source-obfuscation-keep-options.md#section-keep-property-name) | 10 |
| 指定保留顶层作用域或导入导出元素名称 | [-keep-global-name](source-obfuscation-keep-options.md#section-keep-global-name) | 10 |
| 指定保留文件/文件夹名称 | [-keep-file-name](source-obfuscation-keep-options.md#section-keep-file-name) | 10 |
| 指定保留注释 | [-keep-comments](source-obfuscation-keep-options.md#section-keep-comments) | 12 |
| 指定保留声明文件中的所有名称 | [-keep-dts](source-obfuscation-keep-options.md#section-keep-dts) | 12 |
| 指定保留源码文件中的所有名称 | [-keep](source-obfuscation-keep-options.md#section-keep) | 12 |
| 名称类和路径类的保留选项支持通配符 | [保留选项支持的通配符](source-obfuscation-keep-options.md#保留选项支持的通配符) | 12 |
| 在代码压缩时排除指定路径的文件 | [-keep-uncompact](source-obfuscation-keep-options.md#section-keep-uncompact) | 26.0.0 |

## -keep-property-name

指定想保留的属性名，支持使用[名称类通配符](source-obfuscation-keep-options.md#名称类通配符)。按如下方式进行配置，表示保留名称为firstName和lastName的属性：
...
1. 该选项在开启[-enable-property-obfuscation](source-obfuscation-rule-options.md#section-enable-property-obfuscation)时生效。
...
1. 如果代码中通过字符串拼接、变量访问或使用defineProperty方法定义对象属性，则这些属性名应被保留。
   ...
   ```
2. 对于间接或直接导出的类或对象的属性名的场景，如果混淆后出现问题，可以使用[-keep-property-name](source-obfuscation-keep-options.md#section-keep-property-name)来保留这些属性名。
...
指定要保留的顶层作用域及导入和导出元素的名称，支持使用[名称类通配符](source-obfuscation-keep-options.md#名称类通配符)。配置方式如下：
...
1. 该选项在开启[-enable-toplevel-obfuscation](source-obfuscation-rule-options.md#section-enable-toplevel-obfuscation)或[-enable-export-obfuscation](source-obfuscation-rule-options.md#section-enable-export-obfuscation)时生效。
2. [-keep-global-name](source-obfuscation-keep-options.md#section-keep-global-name)指定的白名单作用于全局。即代码中出现多个顶层作用域名称或者导出名称，只要与-keep-global-name配置的白名单名称相同，均不会被混淆。
...
指定要保留的文件或文件夹名称（不需要写文件后缀），支持使用[名称类通配符](source-obfuscation-keep-options.md#名称类通配符)。
...
1. 该选项在开启[-enable-filename-obfuscation](source-obfuscation-rule-options.md#section-enable-filename-obfuscation)时生效。
...
1. 在使用require引入文件路径时，由于ArkTS不支持[CommonJS模块](module-principle.md#commonjs模块)语法，因此这种情况下require引入的文件路径应该被保留。
...
2. 对于动态导入的路径名，由于无法识别import函数中的参数是否为路径，因此在这种情况下应保留动态导入的路径名。
   ...
   ```
3. 对于API version 19及之前版本，使用[Navigation跨包路由](arkts-navigation-cross-package.md)进行路由跳转时，传递给动态路由的路径应被保留。动态路由提供系统路由表和自定义路由表两种方式：
...
保留编译生成的声明文件中class、function、namespace、enum、struct、interface、module、type及属性上方的JsDoc注释，支持使用[名称类通配符](source-obfuscation-keep-options.md#名称类通配符)。例如想保留声明文件中Human类上方的JsDoc注释，可进行以下配置：
...
1. 该选项在开启[-remove-comments](source-obfuscation-rule-options.md#section-remove-comments)时生效。
...
filepath仅支持相对路径，./和../为相对于混淆配置文件所在目录，支持使用[路径类通配符](source-obfuscation-keep-options.md#路径类通配符)。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/65/v3/08XaeQ3_Sf2534NMMG0sHQ/zh-cn_image_0000002747209833.png)
