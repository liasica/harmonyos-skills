---
url: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/push-specification
...
所有发布上架到华为应用市场且使用推送通知行为的HarmonyOS应用、联运应用及元服务（以下简称应用）应当遵守本规范，若开发者的行为违反本规范或[《华为应用市场联运服务协议》](../app/20204.md)，华为有权根据本规范和[《华为应用市场联运违规处罚规定》](../app/20204.md)对开发者进行处罚。

## Push Kit使用基本原则

* HarmonyOS应用的推送消息，需通过华为官方的消息推送平台下发。
* 遵守[通知样式规范](push-specification.md#通知样式规范概要)，禁止采用规范外样式，以保证通知体验的一致性。
* 通知开关需由用户授权允许，应用首次启动时需弹窗询问用户是否允许通知。
* 不可出于商业目的强制改变通知属性（如采用进行中通知强制置顶显示），避免损害用户使用体验。
* 遵守[通知内容管理细则](push-detail-rules.md)。

## 通知内容

为保障推送内容信息的合法性、真实客观性、一致性、可读性，以及用户良好的通知使用体验，请您遵守[通知内容管理细则](push-detail-rules.md)。
...
| 描述 | 图片展示 |
| --- | --- |
| 通知文本内容最多可显示3行，超长后以“...”截断。 | 请参考[设计-通知-普通通知](../design-guides/system-features-notification-0000001793074217.md#section4182426382)。 |
...
| 描述 | 图片展示 |
| --- | --- |
| 适用于有图片预览的通知。 | 请参考[设计-通知-图片预览通知](../design-guides/system-features-notification-0000001793074217.md#section16214134103817)。 |
