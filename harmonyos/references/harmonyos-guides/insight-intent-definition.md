...
| 意图定义方式 | 说明 | 使用场景 |
| --- | --- | --- |
| [接入标准意图](insight-intent-definition.md#接入标准意图) | 由系统预定义调用规范，包含统一的意图参数格式定义与意图执行结果格式定义。系统会根据装饰器的schema字段与intentVersion字段，在标准意图列表查询是否存在匹配的意图。标准意图列表以Json Schema文件形式存储在SDK中。 | 通用功能场景，若开发者提供的功能在标准意图中已有定义，可选择接入标准意图。 |
| [创建自定义意图](insight-intent-definition.md#创建自定义意图) | 由应用开发者定义调用规范，用于满足标准意图未覆盖的场景。 | 若开发者提供的功能未在标准意图中定义，可选择创建自定义意图。 |
...
当现有标准意图定义无法覆盖应用的功能时，开发者可以自定义意图。系统入口需要解析开发者定义的意图描述和参数规范，意图匹配用户语义后，触发意图执行。自定义意图的属性通过[IntentDecoratorInfo](../harmonyos-references/js-apis-app-ability-insightintentdecorator.md#intentdecoratorinfo)及其子类定义，需要包含如下要素：
