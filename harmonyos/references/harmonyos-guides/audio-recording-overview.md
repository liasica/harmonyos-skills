---
url: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/audio-recording-overview
title: 音频录制开发概述
breadcrumb: 指南 > 媒体 > Audio Kit（音频服务） > 音频录制 > 音频录制开发概述
category: harmonyos-guides
scraped_at: 2026-09-02T14:50:14+08:00
doc_updated_at: 2026-08-29
content_hash: sha256:d55cf2f9861581f967444d985a6e6a3e08cc66653c620b612ce17e31ce8649a3
---
...
* [AVRecorder](using-avrecorder-for-recording.md)：用于音频录制的ArkTS/JS API，集成了音频采集、音频编码和媒体封装的功能。开发者可以直接调用设备硬件如麦克风录音，并生成m4a音频文件。

## 开发音频录制应用须知

* 应用可以调用麦克风录制音频，但该行为属于隐私敏感行为，在调用麦克风前，需要先[向用户申请授权](request-user-authorization.md)：ohos.permission.MICROPHONE。

  如何管理麦克风静音状态，请参考[管理麦克风静音状态](mic-management.md)。
* 如果需要持续录制或后台录制，请申请长时任务避免进入挂起（Suspend）状态。具体参考[长时任务(ArkTS)](continuous-task.md)。
...
* 如果开发者需要实现屏幕录制功能，可参考[使用AVScreenCaptureRecorder录屏写文件(ArkTS)](using-avscreencapture-arkts.md)和[使用AVScreenCapture录屏写文件(C/C++)](using-avscreencapture-for-file.md)。
