...
* HAP中支持添加UIAbility组件或ExtensionAbility组件，以及pages页面。具体操作可参考[在模块中添加Ability](ide-add-new-ability.md)和[添加Page](ide-add-page.md)。
* HAP中支持引用HAR或HSP共享包，详见[HAR的使用](har-package.md#使用)、[HSP的使用](in-app-hsp.md#使用)。

## 调试

通过DevEco Studio编译打包，生成单个或者多个HAP，即可基于HAP进行调试。如需根据不同的部署环境、目标人群、运行环境等，将同一个HAP定制编译为不同版本，请参见[定制编译指导](ide-customized-multi-targets-and-products-guides.md#section1011341611469)。

开发者可以使用DevEco Studio或者hdc工具进行调试：

* **方法一：** 使用DevEco Studio进行调试，详见[应用程序包调试方法](ide-run-debug-configurations.md)。
* **方法二：** 使用[hdc工具](hdc.md)进行调试。
