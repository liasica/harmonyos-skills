---
url: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/bm-tool
title: bm工具
breadcrumb: 指南 > 系统 > 调测调优 > 调试命令 > bm工具
category: harmonyos-guides
scraped_at: 2026-09-24T06:50:16+08:00
doc_updated_at: 2026-09-09
content_hash: sha256:4d1155e32d3cb24e47186cd8036d36accc4722b50515a95d42c43475f794f833
---
...
| -u | 可选参数，指定[用户](bm-tool.md#userid)，默认在当前活跃用户下卸载应用。仅支持在当前活跃用户或0用户下卸载应用。  **说明：**  如果当前活跃用户是100，使用命令bm uninstall -n com.ohos.app -u 102卸载时，只会在当前活跃用户100下卸载应用。 |
...
| -u | 可选参数，查询指定[用户](bm-tool.md#userid)下的应用信息，默认在当前活跃用户下查询应用信息。仅支持在当前活跃用户或0用户下查询。  **说明：**  如果当前活跃用户是100，使用命令bm dump -n com.ohos.app -u 102查询时，只会在当前活跃用户100下查询应用。 |
...
| -u | 可选参数，清理指定[用户](bm-tool.md#userid)下的数据，默认在当前活跃用户下清理数据。仅支持在当前活跃用户或0用户下清理数据。  **说明：**  如果当前活跃用户是100，使用命令bm clean -c -n com.ohos.app -u 102清理数据时，只会在当前活跃用户100下清理。 |
...
| -u | 可选参数，在指定[用户](bm-tool.md#userid)下查询OverlayModuleInfo信息，默认在当前活跃用户下查询。仅支持在当前活跃用户或0用户下查询。  **说明：**  如果当前活跃用户是100，使用命令bm dump-overlay -b com.ohos.app -u 102查询OverlayModuleInfo信息，只会返回当前活跃用户100下的OverlayModuleInfo信息。 |
...
| -u | 可选参数，在指定[用户](bm-tool.md#userid)下查询OverlayModuleInfo信息，默认在当前活跃用户下查询。仅支持在当前活跃用户或0用户下查询。  **说明：**  如果当前活跃用户是100，使用命令bm dump-target-overlay -b com.ohos.app -u 102查询目标应用com.ohos.app中的所有关联的OverlayBundleInfo信息，只会返回当前活跃用户100下的OverlayModuleInfo信息。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/3e/v3/LvyTLF9oTDCrqVRyORBPdQ/zh-cn_image_0000002769450745.png)
...
* 方法一：先通过[bm install -p](bm-tool.md#安装命令install)命令安装依赖的动态共享包（HSP）模块，再在应用运行配置页勾选Keep Application Data，点击OK保存配置，再运行/调试。
  ...
  ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/9c/v3/RTleNTheTvSf2X8nFEPF_w/zh-cn_image_0000002739891414.png)
* 方法二：在运行配置页，选择Deploy Multi Hap标签页，勾选Deploy Multi Hap Packages，选择依赖的模块，点击OK保存配置，再进行运行/调试。

  ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/6f/v3/e1M7G5L7QZOV6vW87yEvuQ/zh-cn_image_0000002739731536.png)
* 方法三：单击Run > Edit Configurations，在General中，勾选Auto Dependencies。点击OK保存配置，再运行/调试。

  ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/d9/v3/dOScpOCaS6O3uhqPCAzLog/zh-cn_image_0000002769330885.png)

场景二：依赖的HSP与HAP不在同一工程内：

在安装HAP前，使用[bm install](bm-tool.md#安装命令install)命令安装依赖的HSP。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/93/v3/mU2b30wVTdmSVrnsI_MaCA/zh-cn_image_0000002769450747.png)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/9e/v3/UjC5eeMyTc-FSMYXCgz67A/zh-cn_image_0000002739891416.png)
...
* 方法1：请参考[app.json5配置文件](app-configuration-file.md)和[module.json5配置文件](module-configuration-file.md)查看并补充必填字段。
* 方法2：通过hilog日志判断缺失字段。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/08/v3/23XDNeK6SYyV_xl-mxAvEA/zh-cn_image_0000002739731538.png)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/56/v3/_n8yQjQOQDyT_uqsSIO0_Q/zh-cn_image_0000002769330887.png)
...
* 场景一：签名时使用了[发布profile文件](../app/agc-help-release-profile-0000002248341090.md)。发布证书签名的应用不能启动调试或运行。
* 场景二：在[build-profile.json5](ide-hvigor-build-profile-app.md#section153288223224)文件中，配置的profile文件与.cer文件不匹配。
...
在启动调试或运行应用/服务时，由于安装应用的[Profile签名文件](../app/agc-help-profile-overview-0000002283260125.md)中的类型与已安装应用的不相同，导致安装HAP出现错误。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/37/v3/1ztkh2ArSEeO7ChlAuLXQA/zh-cn_image_0000002769450749.png)
...
应用APL等级为normal，只能使用APL等级为normal的权限，如果使用了system\_basic或system\_core等级的权限，将导致报错，具体请参考[权限机制中的基本概念](app-permission-mgmt-overview.md#权限机制中的基本概念)。

**处理步骤**

根据报错描述中的权限名称在[应用权限列表](app-permissions.md)中排查权限是否存在。

* 如果不存在，需要查看需要该权限的API接口的文档说明，以确保权限支持申请。例如：[setDevicePairingConfirmation](../harmonyos-references/js-apis-bluetooth-connection.md#connectionsetdevicepairingconfirmation)接口所需权限中，ohos.permission.MANAGE\_BLUETOOTH仅系统应用支持申请，此时请更换为ohos.permission.ACCESS\_BLUETOOTH权限。
* 如果存在，请查看对应的权限文档的要求，申请权限，同时请关注权限是否支持申请。例如：[受限开放权限](restricted-permissions.md)请参考文档[申请受限权限](declare-permissions-in-acl.md)。[企业类应用可用权限](permissions-for-enterprise-apps.md)仅企业类应用支持申请，三方应用不支持。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/b2/v3/iLuzH0jRRJm4Z_QL16Rx1A/zh-cn_image_0000002739891418.png)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ee/v3/iG7bAXluSOmwfAfOyNxOXg/zh-cn_image_0000002739731540.png)
...
2. 如果是因不同团队提供的HSP导致签名不一致问题，可以采用[集成态HSP](integrated-hsp.md)的方式统一提供HSP；在多HAP包的情况下，必须确保所有HAP包的签名一致。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/0b/v3/86p6d8VbQTmFchuNJEEiYg/zh-cn_image_0000002769330889.png)
...
* 场景一：用户导入了三方提供的HSP模块，且该HSP既非[集成态HSP](integrated-hsp.md)，又非同包名的HSP，造成包名不一致。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/3d/v3/c-g5egUtTX6nacpr5TujQA/zh-cn_image_0000002769450751.png)
...
1. 检查应用定义的proxyData内容是否符合要求，参考[proxyData标签](module-configuration-file.md#proxydata标签)。
...
校验唯一标识[appIdentifier](common-problem-of-application.md#什么是appidentifier)不一样，导致安装失败。
...
1. 确认签名文件p7b中apl字段是否有误。
   ...
   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/0b/v3/UVfZ2vCGQ7aYnUSkGhltOw/zh-cn_image_0000002739891420.png)
2. 若apl字段有误，修改UnsgnedReleasedProfileTemplate.json文件中apl字段，并重新签名。

   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/3e/v3/QhAYBUJOSHKL2GRRqfjsIw/zh-cn_image_0000002739731542.png)
...
当前设备不允许安装[Profile签名文件](../app/agc-help-profile-overview-0000002283260125.md)中如下两种类型的应用：enterprise\_mdm（企业MDM应用）、enterprise\_normal（普通企业应用）。Profile签名文件类型的取值及含义请参考[ApplicationInfo.appDistributionType](../harmonyos-references/js-apis-bundlemanager-applicationinfo.md#applicationinfo-1)。
...
禁止通过bm命令安装[Profile签名文件](../app/agc-help-profile-overview-0000002283260125.md)中的类型为app\_gallery并且签名证书类型为release的应用。

**可能原因**

安装应用[Profile签名文件](../app/agc-help-profile-overview-0000002283260125.md)中的类型为app\_gallery并且签名证书类型为release。

**处理步骤**

1. 使用[Profile签名文件](../app/agc-help-profile-overview-0000002283260125.md)中的类型非app\_gallery的文件对应用重新签名。
...
虽然已卸载预置应用，但在安装新应用之前，系统仍会先安装预置应用包，随后再安装新应用包。这是因为预置应用的安装签名信息中的密钥和[APP ID](../app/agc-help-create-app-0000002247955506.md#section16423184171915)，与新安装的应用对应信息不一致。
...
检查应用是否为预置的release版本。如果是，需要替换应用的[Profile签名文件](../app/agc-help-profile-overview-0000002283260125.md)的类型，重新签名并安装。
...
该[Profile签名文件](../app/agc-help-profile-overview-0000002283260125.md)中的类型被限制，禁止安装到当前设备中。
...
该应用的[Profile签名文件](../app/agc-help-profile-overview-0000002283260125.md)为调试类型，且未配置当前设备的UDID。
...
使用[bm dump -n](bm-tool.md#查询应用信息命令dump)命令查看应用配置，确认要卸载的模块已经安装。
...
检查多HAP的Abi是否一致，请参考[错误码9568347](bm-tool.md#section9568347-解析本地so文件失败)的处理步骤。
...
使用[bm dump -n 命令](bm-tool.md#查询应用信息命令dump)查询应用的信息，检查传入的插件是否安装。
...
应用[Profile签名文件](../app/agc-help-profile-overview-0000002283260125.md)中allowed-acls字段的U1Enabled配置发生变更，例如：
...
多HAP包签名时使用的[Profile签名文件](../app/agc-help-profile-overview-0000002283260125.md)不一致导致签名信息中allowed-acls的U1Enabled配置不一致。
