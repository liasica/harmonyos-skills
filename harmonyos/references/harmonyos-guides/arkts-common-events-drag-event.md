...
content_hash: sha256:93420c5970db393e5ba603d36c71ff38757c3817aef59d34b433453b3502071b
---
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/91/v3/4XKpEpuPRlSmNgWIC6oAig/zh-cn_image_0000002742002951.png)
...
| **回调事件** | **说明** |
| --- | --- |
| [onDragStart](../harmonyos-references/ts-universal-events-drag-drop.md#ondragstart) | 拖出的组件产生拖出动作时，该回调触发。  该回调可以感知拖拽行为的发起，开发者可以在onDragStart方法中设置拖拽过程中传递的数据，并自定义拖拽的背板图像。建议开发者采用[DragItemInfo](../harmonyos-references/ts-universal-events-drag-drop.md#dragiteminfo)中的pixelMap字段来返回背板图像，避免使用[CustomBuilder](../harmonyos-references/ts-types.md#custombuilder8)，因为后者可能会带来额外的性能开销。 |
...
在拖拽移动过程中显示的背板图并非组件本身，而是表示用户拖动的数据，开发者可将其设定为任意可显示的图像。具体而言，onDragStart回调中返回的[CustomBuilder](../harmonyos-references/ts-types.md#custombuilder8)或[DragItemInfo](../harmonyos-references/ts-universal-events-drag-drop.md#dragiteminfo)的pixelMap字段可以用于设置拖拽移动过程中的背板图，而浮起图则默认采用组件本身的截图。[dragPreview](../harmonyos-references/ts-universal-attributes-drag-drop.md#dragpreview11)属性中设定的CustomBuilder或DragItemInfo的pixelMap字段可以用于配置浮起和拖拽过程的背板图。若开发者未配置背板图，系统将自动采用组件本身的截图作为拖拽和浮起时的背板图。

拖拽背板图当前支持设置透明度、圆角、阴影和模糊，具体用法见[拖拽控制](../harmonyos-references/ts-universal-attributes-drag-drop.md)。

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/23/v3/_c24CrmsSICpyefs5lbsMQ/zh-cn_image_0000002712403964.png)

**约束限制：**

* 对于容器组件，如果内部内容通过[position](../harmonyos-references/ts-universal-attributes-location.md#position)、[offset](../harmonyos-references/ts-universal-attributes-location.md#offset)等接口使得绘制区域超出了容器组件范围，则系统截图无法截取到范围之外的内容。此种情况下，如果一定要浮起，即拖拽背板能够包含范围之外的内容，则可考虑通过扩大容器范围或自定义方式实现。
* 不论是使用CustomBuilder或是系统默认截图方式，截图都暂时无法应用[scale](../harmonyos-references/ts-universal-attributes-transformation.md#scale)、[rotate](../harmonyos-references/ts-universal-attributes-transformation.md#rotate)等图形变换效果。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/5d/v3/tcM65oqDQNCBHCWYBidxSw/zh-cn_image_0000002742122913.gif)

### 多选拖拽适配

从API version 12开始，[Grid](../harmonyos-references/ts-container-grid.md)组件和[List](../harmonyos-references/ts-container-list.md)组件中的[GridItem](../harmonyos-references/ts-container-griditem.md)和[ListItem](../harmonyos-references/ts-container-listitem.md)组件支持多选与拖拽功能。目前，仅支持onDragStart的触发方式。
...
1. 组件多选拖拽使能。
   ...
   多选拖拽功能默认处于关闭状态。若要启用此功能，需在[dragPreviewOptions](../harmonyos-references/ts-universal-attributes-drag-drop.md#dragpreviewoptions11)接口的options参数中，将isMultiSelectionEnabled设置为true，以表明当前组件支持多选。此外，options还包含defaultAnimationBeforeLifting参数，用于控制组件浮起前的默认效果。将该参数设置为true，组件在浮起前将展示一个默认的缩小动画效果。
   ...
   为了确保选中状态，应将GridItem子组件的[selected](../harmonyos-references/ts-container-griditem.md#selected10)属性设置为true。例如，可以通过调用[onClick](../harmonyos-references/ts-universal-events-click.md#onclick)来设置特定组件为选中状态。
...
2. 优化多选拖拽性能。
   ...
   在多选拖拽操作中，当多选触发聚拢动画效果时，系统会截取当前屏幕内显示的选中组件图像。如果选中组件数量过多，可能会造成较高的性能消耗。为了优化性能，多选拖拽功能支持从[dragPreview](../harmonyos-references/ts-universal-attributes-drag-drop.md#dragpreview11)中获取截图，用以实现聚拢动画效果，从而有效节省系统资源。
...
4. 适配数量角标。
   ...
   多选拖拽的数量角标当前需要应用使用[dragPreviewOptions](../harmonyos-references/ts-universal-attributes-drag-drop.md#dragpreviewoptions11)中的numberBadge参数设置，开发者需要根据当前选中的节点数量来设置数量角标。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/fb/v3/3e1F2H8BQwSArZQq5HYENg/zh-cn_image_0000002712244000.gif)
...
3. 拖拽落位适配动效。
   ...
   设置onDrop回调函数，接收拖拽数据。拖拽落位动效通过[executeDropAnimation](../harmonyos-references/ts-universal-events-drag-drop.md#executedropanimation18)函数执行，设置[useCustomDropAnimation](../harmonyos-references/ts-universal-events-drag-drop.md#属性)为true禁用系统默认动效。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/e7/v3/qE1HOGpUTG2EPPey-4Bvmw/zh-cn_image_0000002742002953.gif)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/7b/v3/o8ycVOgdTnaYMJjWyHIulg/zh-cn_image_0000002712403966.gif)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/68/v3/3cJiHcSiQGCVA6pYjMQyQg/zh-cn_image_0000002742122915.png)

### 触发原理

要实现这些能力，需要在组件上注册[onDragSpringLoading](../harmonyos-references/ts-universal-events-drag-drop.md#ondragspringloading20)接口，并传入一个用于处理拖拽悬停触发通知的回调。使用该接口后，该组件将如同注册了onDrop接口的组件一样，成为一个可拖入目标，并且遵循与onDrop相同的命中检测规则，即：在悬停位置下方，仅有一个组件可以接收拖拽事件响应，并且总是首个被检测到的组件。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/c6/v3/0XCaNH2rQCqg6AGV-22UKg/zh-cn_image_0000002712244002.png)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/2e/v3/3U46BvVCTW6_93O5f7l1ZA/zh-cn_image_0000002742002955.gif)
