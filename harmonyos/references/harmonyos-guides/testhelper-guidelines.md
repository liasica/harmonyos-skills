---
url: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/testhelper-guidelines
title: 辅助测试工具使用指导
breadcrumb: 指南 > 应用测试 > 单元测试和UI测试 > 自动化测试框架使用指导 > 辅助测试工具使用指导
category: harmonyos-guides
scraped_at: 2026-09-02T15:00:20+08:00
doc_updated_at: 2026-08-29
content_hash: sha256:a99288892e70209d72df121bd9e2ab04f82a07c4c77b45187607162857b3996f
---
...
搭载HarmonyOS 7.0.0及以上版本的设备支持此工具。

## 功能全景

testhelper功能架构设计图如下所示：

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ff/v3/AtVkXqi8RiGvVYNIXRV_yA/zh-cn_image_0000002701800954.png)
...
| 参数 | 必填 | 说明 |
| --- | --- | --- |
| time | 是 | 时间字符串，格式必须为YYYY-MM-DD HH:MM:SS。  **取值范围：**  - 年份：1970-2038。  - 月份：1-12。  - 日期：1-31，需符合实际日历（如2月无30日）。  - 小时：0-23。  - 分钟：0-59。  - 秒：0-59。 |
...
| 参数 | 必填 | 说明 |
| --- | --- | --- |
| timezone | 是 | 时区标识符，如Asia/Shanghai。取值参考[支持的系统时区](../harmonyos-references/js-apis-date-time.md#支持的系统时区)。 |
...
| 参数 | 必填 | 说明 |
| --- | --- | --- |
| font-path | 是 | 字体文件的绝对路径。  **路径要求：**  - 路径必须为/data/local/tmp下的绝对路径。  - 文件格式：仅支持.ttf和.ttc。  - 不支持包含..的路径。 |
...
| 场景 | 打印信息 |
| --- | --- |
| 路径无效（非/data/local/tmp下的绝对路径）。 | Error: Invalid font path. Please provide a valid absolute path under the /data/local/tmp |
| 字体文件格式无效（非.ttf或.ttc）。 | Error: Invalid font file format. Only supports .ttf and .ttc files. |
...
| 参数 | 必填 | 说明 |
| --- | --- | --- |
| font-name | 是 | 字体名称。可通过[testhelper get-fontname](testhelper-guidelines.md#testhelper-get-fontname)命令获取已安装字体文件对应的字体名称。 |
...
| 参数 | 必填 | 说明 |
| --- | --- | --- |
| mode | 是 | 显示模式，取值为dark（深色模式）或light（浅色模式）。 |
...
| 场景 | 打印信息 |
| --- | --- |
| 模式值无效（非dark或light）。 | Error: Invalid view mode. Mode must be 'dark' or 'light'. |
...
启用位置模拟功能。启用后支持使用[testhelper set-mocked-locations](testhelper-guidelines.md#testhelper-set-mocked-locations)命令设置模拟位置信息。
...
| 参数 | 必填 | 说明 |
| --- | --- | --- |
| path | 是 | GPX文件的绝对路径。  **路径要求：**  - 路径必须为/data/local/tmp下的绝对路径。  - 文件格式：仅支持.gpx扩展名。  - 不支持包含..的路径。  - 最大位置数量：1000个。 |
| interval | 否 | 位置更新间隔，单位为秒。  - 取值范围：1-3600（整数）。  - 默认值：1。 |
...
* lat和lon：经纬度坐标（必填），纬度范围-90到90，经度范围-180到180。
* <ele>：海拔高度（可选，单位：米）。
...
| 场景 | 打印信息 |
| --- | --- |
| 路径无效（非/data/local/tmp下的绝对路径）。 | Error: Invalid path. Please provide a valid gpx file path under the /data/local/tmp |
...
1. 检查参数格式是否符合要求。
2. 确认参数值在限制范围内。
3. 确保文件路径符合要求（如为/data/local/tmp下的绝对路径、拥有正确的扩展名）。
