---
url: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-layout-optimization-guidance
title: 布局优化指导
breadcrumb: 指南 > 应用框架 > ArkUI（方舟UI框架） > UI开发 (ArkTS声明式开发范式) > 组件布局 > 布局优化指导
category: harmonyos-guides
scraped_at: 2026-09-10T06:22:04+08:00
doc_updated_at: 2026-09-09
content_hash: sha256:ce5c047aa17a61b49333415a6ce79774d06ab0079859163f90b89d374e5d8c3f
---
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/91/v3/9Wj4g_YzRzCitn_99QTJQQ/zh-cn_image_0000002717610346.jpg)

应用界面更新的过程主要分为两个过程：**数据处理过程和UI更新过程**。

1. 数据处理过程中主要是对状态数据进行更新，状态数据指的是所定义的@State等相关的数据。数据变化时，会有一定的更新耗时，并且数据关联的组件数量，也影响下一步UI更新的耗时，在开发过程中需要避免无效的数据更新，从而减少冗余的UI更新耗时。关于这部分的优化措施可以参考《[状态管理最佳实践](../best-practices/bpta-status-management.md)》。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/bf/v3/jxm9RZloSaOallKnKutPFw/zh-cn_image_0000002747290297.jpg)

确定实际的脏节点数组后，根据脏节点数组来拿到对应的脏节点对象，通过递归遍历children进行Measure过程，如果该对象布局参数没有发生变化，就会跳过对应的Measure阶段。当Measure执行完成后，进行layout阶段。

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/e3/v3/O6ffNPu6RymU_iYDwbcBvA/zh-cn_image_0000002747210215.jpg)
...
建议开发者优先使用[Code Linter扫描工具](ide-code-linter.md)进行代码检查，重点关注[@performance/hp-arkui-remove-container-without-property](ide_hp-arkui-remove-container-without-property.md)规则。若扫描结果中出现该规则相关问题，可参考本章节提供的优化建议进行调整。
...
| 对比指标 | 指标细分 | 10 | 100 | 500 | 1000 |
| --- | --- | --- | --- | --- | --- |
| **嵌套/层** | 首帧绘制 | 3.2ms | 5.8ms | 17.3ms | 32ms |
| **嵌套/层** | Measure | 1.88ms | 2.89ms | 5.93ms | 10.46ms |
| **嵌套/层** | Layout | 0.38ms | 1.12ms | 5.26ms | 10.88ms |
| **平铺 /个** | 首帧绘制 | 3.6ms | 4.5ms | 14ms | 24.3ms |
| **平铺 /个** | Measure | 2.15ms | 2.31ms | 5.61ms | 9.26ms |
| **平铺 /个** | Layout | 0.39ms | 1.38ms | 4.74ms | 9.92ms |

**说明** 

以上数据来源均为版本DevEco Studio 4.0.3.415、SDK 4.0.10.9条件下测试得到，不同设备类型数据可能存在差异，测试数据旨在体现性能优化趋势，仅供参考。

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/5d/v3/u7l1jVbWRVu1S8ma7XrUOg/zh-cn_image_0000002717770282.png)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/d6/v3/olRra097Ryq4F_FKNoqKxA/zh-cn_image_0000002717610348.png)
...
* [RelativeContainer](arkts-layout-development-relative-layout.md) 通过相对布局实现扁平化。
* [绝对定位](../harmonyos-references/ts-universal-attributes-location.md#position) 通过锚点定位实现扁平化。
* [Grid](arkts-layout-development-create-grid.md) 通过二维布局实现扁平化。
...
**表 3** **使用if/else和visibility属性控制显隐的布局时间对比**
...
**表 4** **使用if条件判断和visibility控制显隐的Measure/Layout时间对比**
...
在列表场景下会采用[List](../harmonyos-references/ts-container-list.md)、[Grid](../harmonyos-references/ts-container-grid.md)、[WaterFlow](../harmonyos-references/ts-container-waterflow.md)等组件配合[ForEach](../harmonyos-references/ts-rendering-control-foreach.md)或者[LazyForEach](../harmonyos-references/ts-rendering-control-lazyforeach.md)来实现，ForEach适合内容长度确定，内容在两屏以内的列表。LazyForEach适合长度超过两屏的列表情况，并且当内容布局相对固定的情况下，配合组件复用的方式来减少滑动过程中的组件创建。在[长列表加载丢帧优化](../best-practices/bpta-best-practices-long-list.md)中，介绍了较为详细的实践案例，这里我们仅引用一些关键性能收益数据。
...
* 使用[Row](../harmonyos-references/ts-container-row.md)、[Column](../harmonyos-references/ts-container-column.md)构建线性布局。
* 使用[Stack](../harmonyos-references/ts-container-stack.md)构建层叠布局。

上述布局都属于线性布局，顾名思义，在线性布局中排布的组件是按照特定的方向线性放置，如横向/纵向/Z序方向。除上述布局类型外，还有一些复杂布局能力，如[Flex](../harmonyos-references/ts-container-flex.md)、[List](../harmonyos-references/ts-container-list.md)、[Grid](../harmonyos-references/ts-container-grid.md)、[RelativeContainer](../harmonyos-references/ts-container-relativecontainer.md)和自定义布局等。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/11/v3/cCBmbioRQx6xfOOd5l_ICw/zh-cn_image_0000002747290299.png)

**图 3** List宽高固定

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/1d/v3/jLp8lQMrQ7u45Zm81niPQA/zh-cn_image_0000002747210217.png)
...
性能是一个涉及许多方面的复杂问题。DevEco Studio提供了工具可以为您提供关于性能瓶颈发生位置的明确提示，使用工具Profiler 查看性能参数可以参考[使用Profiler进行性能调优](ide-profiler-introduction.md)，使用ArkUI Inspector查看组件树结构可以参考[布局分析](ide-arkui-inspector.md)。
