---
url: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/source-obfuscation-rule-options
...
| 功能 | 选项 | 起始API版本 |
| --- | --- | --- |
| 关闭混淆 | [-disable-obfuscation](source-obfuscation-rule-options.md#section-disable-obfuscation) | 10 |
| 开启属性名称混淆 | [-enable-property-obfuscation](source-obfuscation-rule-options.md#section-enable-property-obfuscation) | 10 |
| 开启字符串属性名称混淆 | [-enable-string-property-obfuscation](source-obfuscation-rule-options.md#section-enable-string-property-obfuscation) | 10 |
| 开启顶层作用域名称混淆 | [-enable-toplevel-obfuscation](source-obfuscation-rule-options.md#section-enable-toplevel-obfuscation) | 10 |
| 开启导入导出名称混淆 | [-enable-export-obfuscation](source-obfuscation-rule-options.md#section-enable-export-obfuscation) | 10 |
| 开启文件名混淆 | [-enable-filename-obfuscation](source-obfuscation-rule-options.md#section-enable-filename-obfuscation) | 10 |
| 代码压缩 | [-compact](source-obfuscation-rule-options.md#section-compact) | 10 |
| 声明文件注释删除 | [-remove-comments](source-obfuscation-rule-options.md#section-remove-comments) | 10 |
| 删除console.\*语句 | [-remove-log](source-obfuscation-rule-options.md#section-remove-log) | 10 |
| 名称缓存输出 | [-print-namecache](source-obfuscation-rule-options.md#section-print-namecache) | 10 |
| 名称缓存复用 | [-apply-namecache](source-obfuscation-rule-options.md#section-apply-namecache) | 10 |
| 输出未混淆名单 | [-print-kept-names](source-obfuscation-rule-options.md#section-print-kept-names) | 18 |
| 缩减语言预置白名单 | [-extra-options strip-language-default](source-obfuscation-rule-options.md#section-extra-options-strip-language-default) | 18 |
| 缩减系统预置白名单 | [-extra-options strip-system-api-args](source-obfuscation-rule-options.md#section-extra-options-strip-system-api-args) | 18 |
| 不保留未参与编译模块名称 | [-extra-options strip-not-compiled-module-name](source-obfuscation-rule-options.md#section-extra-options-strip-not-compiled-module-name) | 22 |
| 保留声明文件参数 | [-keep-parameter-names](source-obfuscation-rule-options.md#section-keep-parameter-names) | 18 |
| 合并依赖模块选项 | [-enable-lib-obfuscation-options](source-obfuscation-rule-options.md#section-enable-lib-obfuscation-options) | 18 |
| 通过注释在源码中标记白名单 | [-use-keep-in-source](source-obfuscation-rule-options.md#section-use-keep-in-source) | 19 |
| 保留对象字面量属性名称 | [-keep-object-props](source-obfuscation-rule-options.md#section-keep-object-props) | 23 |
| 删除指定的方法调用语句 | [-remove-nosideeffects-calls](source-obfuscation-rule-options.md#section-remove-nosideeffects-calls) | 23 |
...
开启该选项后，在需要[手动配置白名单的场景中](source-obfuscation-keep-options.md#section-keep-property-name)，请将对应的属性名配置到白名单中。
...
* ArkUI组件中的属性名不会被混淆。例如，下面例子中的message和data不会被混淆。
  ...
  ```
* 被保留选项[-keep-property-name](source-obfuscation-keep-options.md#section-keep-property-name)指定的属性名不会被混淆。
...
1. 如果代码里面有字符串属性名包含特殊字符（除了a-z、A-Z、0-9、\_之外的字符），例如let obj = {"\n": 123, "": 4, " ": 5}，建议不要开启-enable-string-property-obfuscation选项，因为可能无法通过[-keep-property-name](source-obfuscation-keep-options.md#section-keep-property-name)来保留这些名称。
2. SDK API的属性白名单中不包含声明文件中使用的字符串常量值，例如示例中的字符串'ohos.want.action.home'未包含在属性白名单中。
   ...
   因此，在开启-enable-string-property-obfuscation选项后，如果希望保留代码中使用的SDK API字符串常量的属性不被混淆，例如obj['ohos.want.action.home']，可以使用[-keep-property-name](source-obfuscation-keep-options.md#section-keep-property-name)选项进行保留。

## -enable-toplevel-obfuscation

**说明** 

开启该选项后，在需要[手动配置白名单的场景中](source-obfuscation-keep-options.md#section-keep-global-name)，请将对应的顶层作用域名称配置到白名单中。
...
* 当前文件找不到声明的名称不会被混淆。
* 被保留选项[-keep-global-name](source-obfuscation-keep-options.md#section-keep-global-name)指定的顶层作用域名称不会被混淆。
...
开启该选项后，在需要[手动配置白名单的场景中](source-obfuscation-keep-options.md#section-keep-file-name)，请将对应的文件夹/文件名称配置到白名单中。
...
* 被[-keep-file-name](source-obfuscation-keep-options.md#section-keep-file-name)指定的文件/文件夹名称不会被混淆。
* 非ECMAScript模块引用方式（例如：const module = require('./module')）。
* 非路径引用方式，例如import module from 'json5'中的json5不会被混淆。

**说明** 

由于系统会在应用运行时加载某些指定的文件，针对这类文件，开发者需要手动在[-keep-file-name](source-obfuscation-keep-options.md#section-keep-file-name)选项中配置相应的白名单，防止指定文件被混淆，导致运行失败。
...
若希望对部分源码路径仍保留换行（便于对照报错栈行号阅读混淆中间产物），可在开启-compact的同时，使用[-keep-uncompact](source-obfuscation-keep-options.md#section-keep-uncompact)指定不参与压缩的源码路径。
...
使用[-keep-comments](source-obfuscation-keep-options.md#section-keep-comments)配置保留声明文件中的JSDoc注释。
...
当**filepath**参数缺省时，未混淆名单（keptNames.json）和全量白名单（whitelist.json）默认输出到缓存路径build/default/cache/{...}/release/obfuscation中。
...
未混淆名单（keptNames.json）中包含未混淆的名称及其原因。未混淆的原因包括：与SDK白名单重名、与语言白名单重名、与用户配置白名单重名、与结构体白名单重名、与导出白名单重名、与字符串属性白名单重名（未开启[字符串属性混淆](source-obfuscation-rule-options.md#section-enable-string-property-obfuscation)的情况下）以及与枚举白名单重名。
...
混淆配置分为[混淆选项](source-obfuscation-rule-options.md#混淆选项汇总)和[保留选项](source-obfuscation-keep-options.md)：
...
被标记的名称根据以下规则添加到混淆白名单，被// @KeepAsConsumer保留的名称还会生成到[obfuscation.txt](source-obfuscation-guide.md#混淆配置文件)文件中。
