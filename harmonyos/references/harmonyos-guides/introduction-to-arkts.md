...
number类型在表示大整数（即超过Number.MIN\_SAFE\_INTEGER（-9007199254740991）~`Number.MAX\_SAFE\_INTEGER（9007199254740991））时会造成精度丢失。在开发时可以按需使用bigint`类型来确保精度：
...
开发者可以通过使用ArkTS提供的[Retention](../harmonyos-references/js-apis-arkts-lang.md#retention24)接口来构造自定义源码态注解。源码态注解有更广的使用范围，支持在以下声明上使用：
