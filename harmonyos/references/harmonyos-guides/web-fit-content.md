...
2. 建议配置[overScrollMode](../harmonyos-references/arkts-basic-components-web-attributes.md#overscrollmode11)（过滚动模式）为关闭状态。当过滚动模式开启时，当用户在Web界面上滑动到边缘时，Web会通过弹性动画弹回界面，会与Scroll组件的回弹相互冲突，影响体验。
3. [keyboardAvoidMode](../harmonyos-references/arkts-basic-components-web-attributes.md#keyboardavoidmode12)（键盘避让）属性配置为RESIZE\_CONTENT时，该避让模式不生效。
4. 不支持对页面进行手势缩放。
