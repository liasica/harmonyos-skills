---
url: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/app-stop
title: 应用退出
breadcrumb: 指南 > 应用框架 > Ability Kit（程序框架服务） > 应用生命周期 > 应用退出
category: harmonyos-guides
scraped_at: 2026-09-02T14:59:10+08:00
doc_updated_at: 2026-08-29
content_hash: sha256:f5599095438db29a18f8f9b3355a79f7b4897d711eae209eb21869c6afb4f389
---
...
| 退出类型 | 具体触发场景 | 是否触发[onDestroy](../harmonyos-references/js-apis-app-ability-uiability.md#ondestroy) |
| --- | --- | --- |
...
1. **UIAbility销毁**：当某个UIAbility实例被销毁时，系统依次回调[onWindowStageWillDestroy()](../harmonyos-references/js-apis-app-ability-uiability.md#onwindowstagewilldestroy12)、[onWindowStageDestroy()](../harmonyos-references/js-apis-app-ability-uiability.md#onwindowstagedestroy)，最后回调[onDestroy()](../harmonyos-references/js-apis-app-ability-uiability.md#ondestroy)生命周期。开发者可以在onDestroy()中执行该UIAbility实例的资源释放操作，如关闭网络连接、释放文件句柄等。
2. **AbilityStage销毁**：当某个AbilityStage下所有的UIAbility对象均被销毁后，系统回调该AbilityStage的[onDestroy()](../harmonyos-references/js-apis-app-ability-abilitystage.md#ondestroy12)生命周期。开发者可以在该回调中执行Module级别的资源释放操作，如释放Module级别的缓存、关闭Module级别的数据库连接等。
3. **进程退出**：当应用进程内所有AbilityStage均被销毁后，应用进程退出，操作系统回收该进程的全部内存与线程资源。

**说明** 

完整的UIAbility生命周期回调顺序，请参见[UIAbility组件生命周期](uiability-lifecycle.md)。

## 用户主动退出

### 用户通过返回退出

**场景示例**：在手机类设备上，用户按下返回键时，系统回调UIAbility的[onBackPressed()](../harmonyos-references/js-apis-app-ability-uiability.md#onbackpressed10)回调，该回调默认返回true，系统将UIAbility置于后台而不是销毁。

**开发实现**：开发者可以通过重写[onBackPressed()](../harmonyos-references/js-apis-app-ability-uiability.md#onbackpressed10)方法返回true来改变返回键的默认行为，例如在onBackPressed()中调用terminateSelf()，实现按下返回键时直接退出UIAbility。
...
通过“一键清理”退出UIAbility时可能不会触发[onDestroy()](uiability-lifecycle.md#ondestroy)回调，而是直接终止进程。
...
**开发实现**：开发者可以在业务逻辑执行完毕后，通过调用[terminateSelf()](../harmonyos-references/js-apis-inner-application-uiabilitycontext.md#terminateself-1)接口主动退出当前UIAbility。
