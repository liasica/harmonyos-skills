...
5. 设备A将数字信封数据发送给设备B。
6. 设备B使用[importWrappedKeyItem](../harmonyos-references/js-apis-huks.md#huksimportwrappedkeyitem9)导入数字信封密钥。若导入密钥是对称密钥，此步骤只需对裸密钥进行加密。若导入非对称密钥的密钥对，则将公钥以DER格式封装，并放入[HUKS\_TAG\_ASYMMETRIC\_PUBLIC\_KEY\_DATA](../harmonyos-references/js-apis-huks.md#hukstag)中。
