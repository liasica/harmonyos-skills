...
如果应用涉及获取受限权限，在应用发布上架时，[AppGallery Connect（AGC）](https://developer.huawei.com/consumer/cn/service/josp/agc/index.html#/)将根据应用的使用场景审核是否可以使用对应的受限权限。如不符合，应用的上架申请将被驳回，审核方式请见[发布HarmonyOS应用](../app/agc-help-release-app-0000002271695230.md)。
...
* 数据克隆备份：应用需要克隆、备份或同步本地图片/视频类文件。
* 相册整理：应用需要批量读取系统图库内的图片/视频，并进行自动分类整理、去重删除等操作。
* 智能视频生成：应用需要自动选择图库内的图片/视频文件并生成短视频文件。

例如：

* 应用需要批量读取本地媒体库的文件（图片、视频），用于内容同步云端服务器，或是在设备间进行内容同步或者克隆。
* 应用需要自动读取本地媒体库的文件（图片、视频），对这些文件按日期、类型、大小、是否重复等规则进行分类后，供用户操作处理。
...
2. 用户在应用内手动选择图片/视频并进行处理，如：
   ...
   1）用户将选择的图片/视频投屏到大屏等其他设备。

   2）用户将选择的图片/视频发送、共享给其他应用或设备。

   3）用户对选择的图片/视频进行加密、美化等操作。

   4）应用处理用户手动选择的图片/视频文件并生成短视频。

   5）应用对用户手动选择的图片/视频进行去重、删除等操作。
...
**无需权限的建议方案：** 使用安全控件或授权弹窗的方式，将用户指定的媒体资源保存到图库中，使用方式请参考：[保存媒体库资源](photoaccesshelper-savebutton.md)。
...
应用申请此权限后，系统引擎可申请带MAP\_FORT的匿名可执行内存，做即时编译，提高运行时执行效率。
...
仅元服务小游戏可申请，申请方式请参考：[存储空间管理服务](../app/agc-help-release-minigame-acl-and-ability-0000002425276004.md#section17374114165216)。
...
**申请条件**：需要与闪控球权限[ohos.permission.USE\_FLOAT\_BALL](restricted-permissions.md#ohospermissionuse_float_ball)一起，才可申请此权限。
...
## ohos.permission.GET\_NETWORK\_STATS
...
## ohos.permission.ACCESS\_GAME\_BUDDY\_SERVICE
...
应用提供游戏AI陪玩类的功能，需要在用户玩游戏时通过分析游戏应用截图来为用户提供辅助建议。
...
## ohos.permission.kernel.SUPPORT\_LOCAL\_PLUGIN
...
## ohos.permission.ALLOW\_USE\_BM
...
## ohos.permission.ACCESS\_DIGITAL\_IDENTITY
...
## ohos.permission.CAPTURE\_PLAYBACK\_DOWNLINK
...
## ohos.permission.MANAGE\_SANDBOX\_BUNDLE
...
## ohos.permission.DETECT\_DEVICE
...
## ohos.permission.ACCESS\_SE\_KEY

允许应用生成、使用、删除安全芯片密钥。

**可申请此权限的特殊场景与功能：**

1. 该权限仅面向数字盾相关场景，如金融类、企业类应用中的基于数字盾的转账支付或数字签章的等高安场景使用，应用需具备数字盾服务开放能力。
2. 该权限涉及安全芯片密钥管理能力，依赖硬件支持，请先确保使用设备支持相应能力。
