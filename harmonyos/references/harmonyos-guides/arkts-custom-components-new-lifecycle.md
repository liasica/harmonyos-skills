...
content_hash: sha256:c5958db01ae362c5591d1c16c8e8b3089f04bc9c99ad307bd8cdbaa2a7aec354
---

## 概述

已有的[自定义组件生命周期](arkts-page-custom-components-lifecycle.md)回调函数触发只取决于事件的触发，在某些特定的情况下，会出现自定义组件生命周期回调函数的触发顺序不符合预期。比如：[aboutToDisappear在特定情况下会误调用aboutToAppear、组件未展开被复用时，会误调用aboutToReuse](arkts-custom-components-new-lifecycle.md#生命周期回调函数的区别)。新的自定义组件生命周期回调函数受状态机限制，生命周期回调函数调用时机符合预期。
...
* [@ComponentReuse](../harmonyos-references/ts-custom-component-new-lifecycle.md#componentreuse)：当可复用的自定义组件从复用池中重新添加到节点树时调用该装饰器装饰的函数，以接收组件的构造入参。最后，@ComponentReuse装饰的函数会递归遍历所有子组件，对每个完成复用的组件调用@ComponentReuse装饰的函数。
...
* [@ComponentActive](../harmonyos-references/ts-custom-component-new-lifecycle.md#componentactive)：当组件从非激活状态变为激活状态时，调用该装饰器装饰的函数。自定义组件的激活和非激活概念与[组件冻结](arkts-custom-components-freeze.md)中的组件激活（active）和非激活（inactive）概念相同，详见[自定义组件的激活与非激活生命周期](arkts-custom-components-new-lifecycle.md#自定义组件的激活与非激活生命周期)。
* [@ComponentInactive](../harmonyos-references/ts-custom-component-new-lifecycle.md#componentinactive)：当组件从激活状态变为非激活状态时，调用该装饰器装饰的函数。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/23/v3/TA3NuaztRpqMJE3jHPqEtA/zh-cn_image_0000002769329559.png)
...
自定义组件生命周期装饰器@ComponentActive和@ComponentInactive自API版本26.0.0起可用，用于监听组件的激活状态变化，不受状态机约束。

当组件由非激活转为激活状态时（例如应用从后台切回前台、页面重新显示时），会触发@ComponentActive装饰的函数；反之，当组件从激活转为非激活状态时（例如应用退至后台、页面隐藏或组件预创建时），则调用@ComponentInactive装饰的函数。组件激活/非激活并不等同于其可见性。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/83/v3/jjHIeSJcSlKee6SDsAu71g/zh-cn_image_0000002769449421.gif)
...
当开发者同时使用回收复用和激活生命周期时，自定义组件触发相关组件的时序有以下约束：@ComponentInactive触发时机早于@ComponentRecycle和aboutToRecycle，@ComponentActive触发时机晚于@ComponentReuse和aboutToReuse。

### 懒创建场景下监听激活状态变化

懒创建的含义为，容器中有多个组件，只有选定的组件完成创建，其余组件需要通过翻页跳转等方法选中时触发创建。当切换到创建完成的组件时，该组件转变为激活态；组件不再被选中时，该组件转变为非激活态。

懒创建场景包含[Tabs](arkts-navigation-tabs.md)和[Navigation](arkts-navigation-introduction.md)，以下示例展示Navigation和Tabs场景中，@ComponentActive和@ComponentInactive生命周期装饰器的触发时机。
...
在系统配置文件route\_map.json中配置子页信息如下：
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ea/v3/I3lFgQPFSlevzdncZMOxfg/zh-cn_image_0000002739890090.gif)
...
以[LazyForEach](arkts-rendering-control-lazyforeach.md)为例，LazyForEach预加载区域的组件创建完毕转变为非激活态。[List](../harmonyos-references/ts-container-list.md)、[Swiper](../harmonyos-references/ts-container-swiper.md)、[Grid](../harmonyos-references/ts-container-grid.md)、[WaterFlow](../harmonyos-references/ts-container-waterflow.md)等组件使用LazyForEach时，可以通过[cachedCount](../harmonyos-references/ts-container-list.md#cachedcount14)的count属性设置预加载区域的节点数量。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/b3/v3/WpSiawt2Qs6r7vrRCEr-Qg/zh-cn_image_0000002739730212.gif)
...
当List的cachedCount的show属性设为true时，预加载节点创建完毕触发@ComponentInactive后，执行上树并触发@ComponentActive。

### 页面可见性变化场景下监听激活状态变化

[Router](arkts-router-to-navigation.md)页面每次隐藏时触发onPageHide，页面中的组件转变为非激活态；页面每次显示时触发onPageShow，页面中的组件转变为激活态。同理，亮屏时页面会触发onPageShow，页面中的组件转变为激活态；息屏时页面会触发onPageHide，页面中的组件转变为非激活态。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/65/v3/ZSuh88dlQv6ZrNz5JTjYYQ/zh-cn_image_0000002769329561.png)
...
* 新增生命周期装饰器装饰方法时，自定义组件对应事件发生时会回调该方法。新增生命周期装饰器建议单独使用，不与其他状态变量装饰器联合使用。比如生命周期装饰器和[@Computed](arkts-new-computed.md)联合使用时，生命周期装饰器不生效。
  ...
  ```
* 当自定义组件没有使用生命周期装饰器，且没有注册监听，使用[getCurrentState](../harmonyos-references/ts-custom-component-new-lifecycle.md#getcurrentstate)查询自定义组件当前生命周期状态时，返回值永远为[CustomComponentLifecycleState.INIT](../harmonyos-references/ts-custom-component-new-lifecycle.md#customcomponentlifecyclestate)。当自定义组件没有使用生命周期装饰器时，自定义组件注册监听成功时才开启状态机且当前生命周期状态为INIT。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/7c/v3/ETPOAVCBTJaNwjGDau_05w/zh-cn_image_0000002769449423.gif)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/91/v3/iaMzqmxMTWKfKbVCNvhZBA/zh-cn_image_0000002739890092.png)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/5b/v3/jKq1h7IfRGmO4EwCGCt2Ww/zh-cn_image_0000002739730214.gif)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/e1/v3/kPnNIdCQSHm-88Ft7Wx0iA/zh-cn_image_0000002769329563.gif)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/d0/v3/f6MDYA9ZQESwNA_GrB4W-g/zh-cn_image_0000002769449425.gif)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/3a/v3/cSmkPLF5T7uVWQZfIv-P0w/zh-cn_image_0000002739890094.gif)
