...
content_hash: sha256:e72fe32b7693750adfc74a1d4b1d4affedb6a5fc1117f136486bf1d490d2839d
---
...
请参考[APP核准（APP备案）指引](../App/50130.md)完成游戏备案。
...
* 调试阶段，配置签名信息的具体操作请参见[手动签名](ide-signing-manual.md)。
* 发布阶段，配置签名信息的具体操作请参见[发布应用](ide-publish-app.md)。
...
1. 登录[AppGallery Connect](https://developer.huawei.com/consumer/cn/service/josp/agc/index.html)，在“开发与服务”下选择项目及项目下的游戏，获取“应用”下的APP ID和Client ID。
   ...
   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/45/v3/5brpBh1qRyKbE_SRQBcgiQ/zh-cn_image_0000002762994709.png)
...
1. 登录[AppGallery Connect](https://developer.huawei.com/consumer/cn/service/josp/agc/index.html)，在“开发与服务”下选择项目及项目下的游戏，左侧菜单选择“构建 > 游戏服务”，在右侧点击“新增配置”。
   ...
   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/74/v3/A-Uyn665SlOyBLwlywbrjw/zh-cn_image_0000002762834823.png)
2. 在弹出的“新增配置信息”窗口中选择HAP游戏和APK游戏，完成后点击“下一步”。
   ...
   **说明** 
   ...
   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/88/v3/x8ZSPWvuRuSxskT6nqzAJg/zh-cn_image_0000002733275308.png)
...
3. 在弹出的窗口中继续填写信息，完成后点击“下一步”。
   ...
   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/aa/v3/Jbd5iXavSpaNpNXLfZ4aGg/zh-cn_image_0000002733435186.png)

   | 信息项 | 说明 |
   | --- | --- |
   | 支持数据继承的玩家标识类型 | 请选择HarmonyOS 4及以下游戏的玩家标识类型：  - **playerId**：老系统游戏确认使用**playerId**作为玩家标识，请选择“**playerId**”。选择后，新系统游戏的玩家标识gamePlayerId=playerId。  - **openId**：如下情况请选择“**openId**”  - 情况一：老系统游戏确认使用**openId**作为玩家标识。  - 情况二：老系统游戏确认使用**unionId**作为玩家标识。此时，请同时勾选“使用了unionId”，并通过[通过gamePlayerId获取playerId/openId/unionId](../harmonyos-references/gameservice-convertid.md)用gamePlayerId（openId）换取unionId，若unionId未在游戏侧找到玩家记录，则当前玩家为新系统游戏的新用户。  - 情况三：部分游戏由于处于playerId替换为openId方案的过渡期，导致老系统游戏的玩家标识存在**playerId与openId混用**的情况，例如A玩家使用openId，B玩家使用playerId。此时，若无法通过gamePlayerId在游戏侧找到玩家记录，可通过[通过gamePlayerId获取playerId/openId/unionId](../harmonyos-references/gameservice-convertid.md)接口用gamePlayerId（openId）换取playerId，若playerId能在游戏侧找到玩家记录，表明该玩家是使用playerId作为玩家标识的老用户，否则该玩家为新系统游戏的新用户。  - 情况四：HarmonyOS 4及以下游戏未上架时。 |
...
4. （可选）填写开发者服务器的回调地址，回调地址要求支持HTTPS协议，且具有合法商用证书，完成后点击“确定”提交APP ID映射关系的审批申请。
   ...
   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/45/v3/E3hnnfbUTpCVriacQ6AZww/zh-cn_image_0000002762994711.png)

   若用户注销华为账号，华为游戏服务器向开发者服务器发送事件通知，通知游戏自行清理账号数据。
5. 若出现异常情况，将在如下提示框以红字提醒。建议点击“取消”并重新配置映射关系，若忽略异常情况点击“确定”继续提交申请，可能会造成映射关系审批不通过。

   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/13/v3/HbrxgAg3TlCJfj_nJoh0CQ/zh-cn_image_0000002762834825.png)
6. 提交申请后，华为工作人员完成审核需要1-3个工作日，请耐心等待。APP ID映射关系生效后如需重新配置，请先提交映射关系的删除申请。

   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ed/v3/hsuYJyAcSe6EWHJ45N6ZMA/zh-cn_image_0000002733275310.png)

   配置/删除APP ID映射关系的审核结果将通过互动中心或邮件进行通知。

   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/4a/v3/lu4ffrwMQ2KDHNz97Da5jw/zh-cn_image_0000002733435188.png)

## 业务流程

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/e3/v3/pp3aZZycS6msuJXlkIIwDQ/zh-cn_image_0000002762994715.png)
...
9. 游戏调用[verifyLocalPlayer](../harmonyos-references/gameservice-gameplayer.md#gameplayerverifylocalplayer)接口，校验当前用户设备登录华为账号的实名认证、未成年人防沉迷信息。校验通过后，玩家进入游戏。若校验未通过请根据返回的[错误码](../harmonyos-references/errorcode-gameservice.md)进行相应处理。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/91/v3/VJibknExRHe7cRErpYYGTg/zh-cn_image_0000002762834827.png)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/6e/v3/0v6MH2LTSS6yvacbZDb1ng/zh-cn_image_0000002733275312.png)
...
| 场景二 | 用户在设备上切换华为账号后，游戏重启但没有调用unionLogin，直接调用verifyLocalPlayer。此时，verifyLocalPlayer接口会返回[1002000015](../harmonyos-references/errorcode-gameservice.md#section1002000015-当前玩家信息无效)错误码，同时触发on回调。 | 若监听到账号变化，游戏应先清除本地缓存信息（session或其他用户缓存），再重新调用[unionLogin](../harmonyos-references/gameservice-gameplayer.md#gameplayerunionlogin)接口（showLoginDialog设置为true），重新拉起联合登录面板。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/3e/v3/BWKkGr4BTryU-lVoEJJd8w/zh-cn_image_0000002733275306.png)

用户完成登录流程后，游戏顶部弹出欢迎横幅，并向游戏返回accountName（选择华为账号登录返回值为hw\_account）、accountIdentifier（选择华为账号登录返回值为hw\_account）、gamePlayerId等信息。

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/7c/v3/m_lyAnbeQIm03NMK55CdIA/zh-cn_image_0000002733435190.png)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/95/v3/DWTUbpOdST2bg_uJsHrNrA/zh-cn_image_0000002762994717.png)
...
| 合规校验 | 校验项 | 国家政策 | 解决方案 |
| --- | --- | --- | --- |
| 华为账号实名认证 | 校验用户设备登录的华为账号是否已实名认证。 | 根据相关法律法规要求，所有网络游戏玩家必须使用真实有效身份信息注册并登录网络游戏。 | 若玩家使用未实名认证的华为账号登录游戏时，基础游戏服务向玩家弹出实名认证窗口，要求玩家进行实名认证。若玩家取消实名认证，则返回[1002000004](../harmonyos-references/errorcode-gameservice.md#section1002000004-实名认证返回强制实名但用户取消或需要强制实名但没有实名)错误码。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/34/v3/Z8vyLmMPTNGboneZ5X6gUg/zh-cn_image_0000002762834829.png)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/2d/v3/zPIQfLyVQM2pGqi8loonPg/zh-cn_image_0000002733275314.png)
