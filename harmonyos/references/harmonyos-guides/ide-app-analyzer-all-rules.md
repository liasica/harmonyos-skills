...
content_hash: sha256:c9c652e875234c720c6594006f6128bc4a596a4efb27772f26927986c2ce19bf
---
...
| [前台场景内存峰值占用](ide-peak-foreground-memory-usage-0418.md) | 应用/元服务前台场景峰值内存占用：应用在前台且亮屏使用规程的内存占用应≤ 1500MB。 | 应用，元服务 |
| [后台CPU占用峰值](ide-peak-background-cpu-usage-0420.md) | 应用/元服务后台CPU占用峰值：应用/元服务切换到后台等待3min后，开始采集3min内CPU Load < 5%。 | 应用，元服务 |
| [点击操作完成快](ide-quick-completion-for-click-0404.md) | 应用内点击操作完成时延应≤ 900毫秒；时间起点：点击离手；时间终点：转场页面所有占位符加载完成。 | 应用，元服务 |
| [滑动过程流畅](ide-smooth-for-swipe-0413.md) | 应用的滑动过程卡顿率≤ 5ms/s；满帧30FPS的游戏类、地图类和视频类的应用帧率应≥ 29FPS。 | 应用，元服务 |
| [转场操作流畅](ide-smooth-for-transition-0414.md) | 应用的应用内转场过程卡顿率≤ 0ms/s；滑动过程卡顿率：动效时间内累计丢帧时间/动效时长。 | 应用，元服务 |
| [节点数超过500过多](ide-render-node-limit-0430.md) | 后端Render Server在每帧数据里处理的节点数不应该超过500，否则会造成CPU使用过高，引发帧时延过高，从而导致丢帧。 | 应用，元服务 |
| [点击操作响应快](ide-quick-response-for-click-0403.md) | 应用内点击操作响应时延应≤ 100毫秒；时间起点：点击离手；时间终点：界面发生变化。 | 应用，元服务 |
| [滑动操作响应快](ide-quick-response-for-swipe-0405.md) | 应用内滑动操作响应时延应≤ 80毫秒；时间起点：手指滑动；时间终点：界面发生变化。 | 应用，元服务 |
| 序列化反序列化耗时长 | 使用TaskPool/Worker并发能力时候，会检测对象和方法在跨线程传递时序列化和反序列化的耗时；序列化和反序列化耗时应 ≤ 8ms。 | 应用，元服务 |
| [启动加载完成快](ide-quick-completion-for-boot.md) | 各类应用的冷启动首帧完成时延应 ≤ 1100毫秒；时间起点：桌面图标点击离手；时间终点：应用首页铺满全屏并且所有占位符加载完成。 | 应用，元服务 |
...
| 体检规则 | 规则详情 | 应用或元服务规则 | 权重 |
| --- | --- | --- | --- |
| [应用/元服务内点击操作响应快](ide-quick-response-for-click-0403.md) | 起点：点击离手；  终点：界面发生变化；  应用/元服务内点击操作响应时延应≤ 100毫秒。 | 应用，元服务 | 3 |
| [应用/元服务内点击操作完成快](ide-quick-completion-for-click-0404.md) | 时间起点：点击离手；  时间终点：转场页面所有占位符加载完成；  应用/元服务内点击操作完成时延应≤ 1600毫秒。 | 应用，元服务 | 3 |
| [应用/元服务内滑动操作响应快](ide-quick-response-for-swipe-0405.md) | 时间起点：手指滑动；  时间终点：界面发生变化；  应用/元服务内滑动操作响应时延应≤ 80毫秒。 | 应用，元服务 | 3 |
