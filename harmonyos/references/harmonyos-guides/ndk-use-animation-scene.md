---
url: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ndk-use-animation-scene
title: 使用动画开发指导
breadcrumb: 指南 > 应用框架 > ArkUI（方舟UI框架） > UI开发 (基于NDK构建UI) > 使用动画 > 使用动画开发指导
category: harmonyos-guides
scraped_at: 2026-09-05T06:14:05+08:00
doc_updated_at: 2026-08-29
content_hash: sha256:f41e36beb40db62ac3d507db41c424b521d4c9bb151308f7426d8ff62d707f4a
---
...
该示例主要演示了如何通过全局[animateTo](../harmonyos-references/capi-arkui-nativemodule-arkui-nativeanimateapi-1.md#animateto)接口添加属性动画。NDK接口开发的UI界面挂载到ArkTS主页面的完整流程可参考[接入ArkTS页面](ndk-access-the-arkts-page.md)。
...
1. 在.ets文件中创建[NodeContent](../harmonyos-references/js-apis-arkui-nodecontent.md)，把NodeContent作为参数输出到Native方法中。
   ...
   ```
2. 解析NodeContent，转换为C中的[ArkUI\_NodeContentHandle](../harmonyos-references/capi-arkui-nativemodule-arkui-nodecontent8h.md)对象。
...
5. 设置回调参数。
   ...
   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/58/v3/C4WOXtWNQWi6IftCwwbw_g/zh-cn_image_0000002712404102.gif)

## 组件出现/消失转场

该示例主要演示了如何通过[ArkUI\_NodeAttributeType](../harmonyos-references/capi-native-node-h.md#arkui_nodeattributetype)中的NODE\_ROTATE\_TRANSITION，NODE\_SCALE\_TRANSITION，NODE\_TRANSLATE\_TRANSITION属性配置转场参数，并通过[ArkUI\_NodeAttributeType](../harmonyos-references/capi-native-node-h.md#arkui_nodeattributetype)中的NODE\_TRANSFORM\_CENTER属性设置NODE\_SCALE\_TRANSITION和NODE\_ROTATE\_TRANSITION动效的中心点坐标，实现组件的插入和删除时显示过渡动效。

1. 创建可交互界面，界面中包含一个Button组件，点击可以控制转场节点的添加和移除。其中[ArkUI\_NodeContentHandle](../harmonyos-references/capi-arkui-nativemodule-arkui-nodecontent8h.md)类型节点的获取与使用可参考[接入ArkTS页面](ndk-access-the-arkts-page.md)。
   ...
   ```
2. 创建一个设置了[ArkUI\_NodeAttributeType](../harmonyos-references/capi-native-node-h.md#arkui_nodeattributetype)中的NODE\_ROTATE\_TRANSITION，NODE\_SCALE\_TRANSITION属性的节点，当目标节点上下树时会播放转场动画。
...
3. 在Button的监听回调里添加转场节点上下树逻辑，以此控制转场节点的入场和出场。
   ...
   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/05/v3/OfhtPDmrQ8K8odwBzwuhbg/zh-cn_image_0000002742123051.gif)

## 一镜到底转场

该示例主要演示如何通过[ArkUI\_NodeAttributeType](../harmonyos-references/capi-native-node-h.md#arkui_nodeattributetype)中的NODE\_GEOMETRY\_TRANSITION属性来实现一镜到底转场，NDK接口开发的UI界面挂载到ArkTS主页面的完整流程可参考[接入ArkTS页面](ndk-access-the-arkts-page.md)。

1. 创建一个设置了[ArkUI\_NodeAttributeType](../harmonyos-references/capi-native-node-h.md#arkui_nodeattributetype)中的NODE\_GEOMETRY\_TRANSITION属性的image节点，同时设置该节点的宽度、高度和位置，用于区别于另一个image节点，然后将当前image节点挂载到一个父节点，当前示例中父节点为stack。
   ...
   ```
2. 再创建一个设置了[ArkUI\_NodeAttributeType](../harmonyos-references/capi-native-node-h.md#arkui_nodeattributetype)中的NODE\_GEOMETRY\_TRANSITION属性的image节点，属性值与第一个image节点相同，同时为该节点设置不同的宽度、高度和位置。
   ...
   ```
3. 在stack的OnImageTransitionClicked监听回调函数中添加两个节点的上下树逻辑，控制节点的上下树，确保同时只有一个节点存在，同时，当其中一个节点挂载到父节点时需要重置[ArkUI\_NodeAttributeType](../harmonyos-references/capi-native-node-h.md#arkui_nodeattributetype)中的NODE\_GEOMETRY\_TRANSITION属性的值，并重新设置为原来的值。
   ...
   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ea/v3/a4wLxsLRRJOoeHvnvweRGg/zh-cn_image_0000002712244138.gif)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/2d/v3/bb-5eGw2SNKq3VQETZcycw/zh-cn_image_0000002742003091.gif)

## 使用帧动画

该示例主要演示如何通过[createAnimator](../harmonyos-references/capi-arkui-nativemodule-arkui-nativeanimateapi-1.md#createanimator)来设置帧动画。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/1d/v3/LgBdPKPZTWKDnuRWoiJs1A/zh-cn_image_0000002712404104.gif)
