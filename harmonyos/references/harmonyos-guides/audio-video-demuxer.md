...
3. 创建解封装器实例。
   ...
   ```
4. 注册DRM信息监听函数，接口参考[Demuxer\_MediaKeySystemInfoCallback()](../harmonyos-references/capi-native-avdemuxer-h.md#demuxer_mediakeysysteminfocallback)（可选）。如果不是DRM码流或已获得DRM信息，可跳过此步骤。DRM信息内容参考[DRM\_MediaKeySystemInfo](../harmonyos-references/capi-drm-drm-mediakeysysteminfo.md)。
