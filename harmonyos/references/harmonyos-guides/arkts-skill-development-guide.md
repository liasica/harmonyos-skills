---
url: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-skill-development-guide
...
以下是基于ArkTS脚本开发应用Skill使用的主要接口，更多接口及使用方式请见[@ohos.app.ability.scriptManager](../harmonyos-references/js-apis-app-ability-scriptmanager.md)。

| 接口名 | 描述 |
| --- | --- |
| [ExecuteResult](../harmonyos-references/js-apis-app-ability-scriptmanager.md#executeresult) | ArkTS脚本执行结果。 |
| [ArkTSScriptInfo](../harmonyos-references/js-apis-app-ability-scriptmanager.md#arktsscriptinfo) | 应用的ArkTS脚本入口函数的第一个参数，用于接收系统传递的脚本上下文信息。 |
| [completeArkTSScriptInApp(context: Context, requestCode: string, result: ExecuteResult): Promise<void>](../harmonyos-references/js-apis-app-ability-scriptmanager.md#scriptmanagercompletearktsscriptinapp) | 完成应用的ArkTS脚本执行，上报执行结果。使用Promise异步回调。 |
...
2. 配置module.json5文件。
   ...
   在entry/src/main/module.json5的module标签下新增[skillProfiles标签](module-configuration-file.md#skillprofiles标签)，将Skill注册到模块。
...
3. 实现ArkTS脚本。
   ...
   * **方法签名约定**：第一个参数类型固定为[ArkTSScriptInfo](../harmonyos-references/js-apis-app-ability-scriptmanager.md#arktsscriptinfo)。
   ...
   业务执行完成后，需将结果封装为[ExecuteResult](../harmonyos-references/js-apis-app-ability-scriptmanager.md#executeresult)，并通过调用[completeArkTSScriptInApp](../harmonyos-references/js-apis-app-ability-scriptmanager.md#scriptmanagercompletearktsscriptinapp)回传给系统智能体，回包内容应与SKILL.md中“执行返回值”声明的分支保持一致。
