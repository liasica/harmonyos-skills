...
3. 本地调试的签名证书配置是否为手动签名并且是从[AppGallery Connect](https://developer.huawei.com/consumer/cn/service/josp/agc/index.html)下载的，参见[应用/服务手动签名](ide-signing-manual.md)。证书下载后，可打开调试Profile（.p7b）文件，搜索“app-identifier”字段，如果对应的值和预下单请求或orderStr中传递的appId不一致，则证书生成错误，需重新生成证书及配置。
...
7. 使用“hdc hilog > 日志路径”抓取运行日志，参考[错误码](../harmonyos-references/errorcode-payment.md)及日志来分析具体的报错异常。
