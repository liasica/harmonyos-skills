---
url: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-global-reuse-pool
...
content_hash: sha256:0742e0beed42580218953f1e1766dcbf1307323d96db1c875ed4035bb7b28f5e
---
...
* 开发者可以选择组件类的所有实例共享单个复用池（[shared](arkts-global-reuse-pool.md#复用池所有权模式)）还是每个实例拥有自己的池（[perInstance](arkts-global-reuse-pool.md#复用池所有权模式)）。
* [IReusableInfo](../harmonyos-references/js-apis-statemanagement.md#ireusableinfo)接口允许应用程序查询和限制缓存组件的数量，包括reuseId等信息。
* [preRender](../harmonyos-references/js-apis-statemanagement.md#prerender)接口允许提前创建可复用组件并将其放入复用池，加快初始渲染速度。
...
| 读取复用池状态 | 不支持 | [getReusableInfo](../harmonyos-references/js-apis-statemanagement.md#getreusableinfo)可获取当前自定义组件的全局复用池的信息。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ce/v3/zvxW0xgLRl2-ySZcMVaY4A/zh-cn_image_0000002712403326.png)
...
| 参数 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| reusePool | [ReusePoolOwnership](arkts-global-reuse-pool.md#复用池所有权模式) | 否 | 如果使用全局复用功能，该参数的值必须为"shared"或"perInstance"。决定此组件类的所有实例是共享单个复用池还是每个实例拥有自己的池。 |
| poolAccepts | Function[] | 否 | 全局复用池接纳的可复用组件的数组，可以同时包含@Reusable和@ReusableV2装饰的自定义组件。 |
| freezeWhenInactive | boolean | 是 | 配置自定义组件支持组件冻结。true：开启组件冻结，false：不开启组件冻结。  从API version 11开始，支持通过此参数配置@Component组件冻结。例子可见[自定义组件冻结](arkts-custom-components-freeze.md)。  从API version 12开始，支持通过此参数配置@ComponentV2组件冻结。例子可见[自定义组件冻结](arkts-custom-components-freezev2.md)。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/1d/v3/nuaEd9_IQcG2BkiIc8Bmzg/zh-cn_image_0000002742122277.png)
...
具体shared复用池示例代码，参考使用场景：[多个父组件间共享复用池](arkts-global-reuse-pool.md#多个父组件间共享复用池)。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/d7/v3/KjZrUDmOTpmg_UczMw9NJw/zh-cn_image_0000002712243362.png)
...
有关包括类型定义、参数表、返回值和示例在内的完整API参考，请参阅[@ohos.arkui.StateManagement (状态管理)](../harmonyos-references/js-apis-statemanagement.md)。

以下接口可用于全局复用池：

| 接口 | 说明 |
| --- | --- |
| [UIUtils.getCustomComponentContext(this).getReusePool()](../harmonyos-references/js-apis-statemanagement.md#getreusepool) | 获取当前组件的[IReusePool](../harmonyos-references/js-apis-statemanagement.md#ireusepool)。如果该组件或其上层组件未配置全局复用池，则返回undefined。 |
| [IReusePool.getReusableInfo(reusableComp, reuseId?)](../harmonyos-references/js-apis-statemanagement.md#getreusableinfo) | 检索池中给定可复用组件类型的回收实例信息。支持按reuseId查询。 |
| [IReusePool.preRender(builder, n)](../harmonyos-references/js-apis-statemanagement.md#prerender) | 调度空闲任务以预创建可复用组件并在首次使用前将其放入复用池。 |
| [IReusableInfo](../harmonyos-references/js-apis-statemanagement.md#ireusableinfo).count | 池中当前回收的组件数（只读）。 |
| [IReusableInfo](../harmonyos-references/js-apis-statemanagement.md#ireusableinfo).maxCount | 允许的最大回收组件数。设置此项可控制缓存大小。 |
| [IReusableInfo](../harmonyos-references/js-apis-statemanagement.md#ireusableinfo).reuseId | 全局复用池基于reuseId分区存放复用组件，该分区对应的reuseId（只读）。 |

## 使用限制

* reusePool和poolAccepts参数必须同时提供。仅指定其中一个会导致编译错误。
* poolAccepts必须是非空数组，否则会导致编译报错。poolAccepts的成员必须是@Reusable或@ReusableV2装饰的自定义组件，使用普通（不可复用）组件、[@Builder](arkts-builder.md)函数或非组件类会导致编译错误。
* reusePool和poolAccepts配置仅在@Component和@ComponentV2上支持。在[@CustomDialog](arkts-common-components-custom-dialog.md)上不受支持。
* @Component和@ComponentV2在配置reusePool和poolAccepts启用全局复用的同时，需要额外配置freezeWhenInactive参数，freezeWhenInactive参数的取值请参考[自定义组件冻结（V1）](arkts-custom-components-freeze.md)或[自定义组件冻结（V2）](arkts-custom-components-freezev2.md)。
...
* 建议不要在[aboutToRecycle](../harmonyos-references/ts-custom-component-lifecycle.md#abouttorecycle10)中修改会触发重新渲染的状态变量，因为组件此时正从UI树中移除。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/9c/v3/Y494787hTUmTf75u32z2Sw/zh-cn_image_0000002742002319.gif)
...
此示例演示与特定父实例绑定的perInstance池。它还展示了[@Consumer](arkts-new-provider-and-consumer.md)在复用周期后如何重连到[@Provider](arkts-new-provider-and-consumer.md)。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/cc/v3/l2QH3XAWTamqHYFmJttUqQ/zh-cn_image_0000002712403328.gif)
...
复用后，aboutToReuse回调设置this.provide = 150。[@Consumer](arkts-new-provider-and-consumer.md)重连到Parent组件的[@Provider](arkts-new-provider-and-consumer.md)。后续@Provider更新正确传播到复用的组件。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ba/v3/7HsKxLm3SjWv9kmVg_YIig/zh-cn_image_0000002742122279.gif)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/b8/v3/pGQWNV00SeSzKX29PUrf4A/zh-cn_image_0000002712243364.gif)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/f5/v3/y3_XUgIqTd29erlQXHPGOg/zh-cn_image_0000002742002321.gif)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/69/v3/lZYHhkBpRj2dVZzevoK9VQ/zh-cn_image_0000002712403330.gif)
