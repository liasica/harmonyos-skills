---
url: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-layout-development-container-reader
...
content_hash: sha256:12c7caf86dbdfe9fe73e1518e423d5aecef5b6df157d7943ca8b82ab7dbef0bd
---
...
ContainerReader在实际开发中常用于[Flex](../harmonyos-references/ts-container-flex.md)、[Row](../harmonyos-references/ts-container-row.md)或[Column](../harmonyos-references/ts-container-column.md)容器内部、[Navigation](../harmonyos-references/ts-basic-components-navigation.md)以及[自定义组件](arkts-create-custom-components.md)内部场景，提供实时尺寸获取、断点值获取和自定义断点阈值核心功能。
...
* **自定义断点阈值**：通过[breakpointConfig](../harmonyos-references/ts-container-containerreader.md#breakpointconfig)属性灵活配置不同尺寸区间对应的布局策略。

## 布局规格

从API版本26.0.0开始，[ContainerReader](../harmonyos-references/ts-container-containerreader.md)组件在不同父容器类型下的布局规格如下。
...
[ContainerReader](../harmonyos-references/ts-container-containerreader.md)作为子组件时，其尺寸由父容器决定。当父容器为Flex、Row或Column时，ContainerReader会根据父容器的布局方向自动撑满父容器剩余空间。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/a4/v3/2Dspf7kKRee-5Nn1tJ42Ng/zh-cn_image_0000002736432611.png)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/89/v3/JjDLdn9rRHyfYnKcVDLr7Q/zh-cn_image_0000002706833456.png)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/8c/v3/OPCXLhDsSQqBilICHtBrdg/zh-cn_image_0000002736312565.png)
...
[ContainerReader](../harmonyos-references/ts-container-containerreader.md)的尺寸由父容器和自身布局确定，不受子组件影响。在ContainerReader组件尺寸测算时，先根据父组件和自身设置确认自身尺寸，然后再做子组件的展开尺寸测算。为防止在节点未尺寸测算前的生命周期中使用ContainerReader组件双向绑定的size状态变量，需要给双向绑定状态变量一个初始值。
...
* ContainerReader接口
  ...
  [ContainerReader](../harmonyos-references/ts-container-containerreader.md)是实现容器断点的核心组件，其使用要求如下。[ContainerReaderInfo](../harmonyos-references/ts-container-containerreader.md#containerreaderinfo)所有参数都必须通过状态变量进行双向绑定。ContainerReader通过双向绑定机制，将后端计算的尺寸和断点值实时更新到状态变量中。不能通过改变此处[ContainerReaderInfo](../harmonyos-references/ts-container-containerreader.md#containerreaderinfo)的size来试图设置ContainerReader尺寸。在使用状态变量时需要添加[!!](arkts-new-binding.md)后缀，触发双向绑定更新。
* breakpointConfig属性

  通过[breakpointConfig](../harmonyos-references/ts-container-containerreader.md#breakpointconfig)属性可以自定义断点阈值。断点数组必须为单调递增数组。其中，宽度断点最多支持5个，即数组最大长度为4；高度断点最多支持3个，即数组最大长度为2。断点区间为左闭右开区间[breakpoint[i], breakpoint[i+1])。宽度断点值单位为vp；高度断点值为组件高度与宽度的比值，无单位。
...
3. 确定容器尺寸。
   ...
   * **按比例分配剩余空间**：当父容器Flex、Row或Column有多个ContainerReader子组件时，ContainerReader通过layoutWeight分配剩余空间。
   ...
   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/c9/v3/pFF7b5czRbCbVWsitf-koQ/zh-cn_image_0000002736432611.png)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/aa/v3/YyqOK4lqSUmZy1FSyyOKSA/zh-cn_image_0000002706673522.png)

## 网格组件根据自身容器断点设置列数

在多设备开发中，网格组件（如[Grid](../harmonyos-references/ts-container-grid.md)）可以根据自身容器尺寸设置不同的列数，实现自适应的列表布局。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/60/v3/7Gh3cUKHT7iBwFCIxyZbuQ/zh-cn_image_0000002736432613.png)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/21/v3/8FWeEZz7THGW_LkWYFP20w/zh-cn_image_0000002706833458.png)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/e6/v3/eHodFnTXQHGo4m74KZqoww/zh-cn_image_0000002736312567.png)

宽屏时左右排列。

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ec/v3/czpwRfaaRgWeIaf3u2PJvQ/zh-cn_image_0000002706673524.png)
