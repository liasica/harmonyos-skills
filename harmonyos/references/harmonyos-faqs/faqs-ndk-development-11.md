---
url: https://developer.huawei.com/consumer/cn/doc/harmonyos-faqs/faqs-ndk-development-11
title: 从napi导出的C++类如何在ArkTS侧被派生
breadcrumb: FAQ > 应用框架开发 > NDK开发 > NDK开发 > 从napi导出的C++类如何在ArkTS侧被派生
category: harmonyos-faqs
scraped_at: 2026-09-24T06:56:29+08:00
doc_updated_at: 2026-09-23
content_hash: sha256:fea880e7a0722aed1f487b6179f441de9a9ce65caa16bd31807bca78eddb2251
---

## 问题现象

* 场景一：从napi导出的C++类，支持在ArkTS侧被派生吗?

  如果支持的话，如何在ArkTS侧派生使用从napi导出的C++类，并提供示例Demo。
* 场景二：在.ts文件中继承从.so导入的C++类时，编译报错"Property 'test' does not exist on type 'BB'"。
...
* 场景一：
  ...
  + **方法一：使用[AKI](https://gitcode.com/openharmony-sig/aki)的JSBIND\_CLASS接口将C++类绑定到JavaScript环境，再在ArkTS侧用extends继承**：
    ...
    5. ArkTS侧：index.ets中使用派生类。
       ...
       ```
  + **方法二：通过napi\_define\_class建立ArkTS类与C++侧的映射关系，并将对应的对象挂载到export上导出，后用extends继承。**
    1. C++侧：napi\_init.cpp中定义使用的类结构。
       ...
       static napi_value Init(napi_env env, napi_value exports)
       {
           ...
           // 通过napi_define_class建立ArkTS类和C++端的映射关系，然后将相应的对象挂载到export上
...
* 场景二：
  ...
  在[build-profile.json5](../harmonyos-guides/ide-hvigor-build-profile.md#section511142752919)中设置tsImportSoCheck字段为true，开启IDE对.ts文件中来自.so符号的解析支持。该功能从7.0Beta1版本开始支持。
