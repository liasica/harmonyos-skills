...
async def probe(url: str, headed: bool) -> None:
    selectors = yaml.safe_load((SCRAPER_ROOT / "config" / "selectors.yaml").read_text(encoding="utf-8"))
    settings = yaml.safe_load((SCRAPER_ROOT / "config" / "whitelist.yaml").read_text(encoding="utf-8"))["settings"]
